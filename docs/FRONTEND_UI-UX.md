...
- **`ProjectRaciMatrix`** — grille **actions × personnes** (`identityKey` roster ∪ équipes convocables) ; cellule = une lettre `R` / `A` / `S` / `C` / `I` (`ProjectRaciKind`) ; règle métier **un seul A par action** (côté API) ; 8 actions BPM par défaut au premier accès ; cycle UI avec confirmation ~1,8 s avant de remplacer un A existant ; libellés métier uniquement (jamais d’UUID en colonnes) ; `readOnly` si fiche verrouillée. API : `GET|PATCH /api/projects/:projectId/team-raci` (body `{ actionId, identityKey, kind }`), `POST|DELETE …/raci-actions`. Permission édition : `projects.update`. Équipes convocables : onglet workspace + `GET|POST|PATCH|DELETE /api/projects/:projectId/teams` (RFC-PROJ-023).
