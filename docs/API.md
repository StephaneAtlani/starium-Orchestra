...
- **POST /api/procedures** — Création brouillon. Body : `{ title, code?, description?, categoryId?, ownerUserId?, templateId? }`. `code` **optionnel** (unique par client si fourni) — généré depuis le titre à la **1ʳᵉ publication** si absent. Si `templateId` : modèle **ACTIVE** du client → draft initial = outline matérialisé (titres + `p` vides) + `sourceTemplateId` / `sourceTemplateName` ; sinon `EMPTY_PROCEDURE_DOC`. `categoryId` catégorie **active** (défaut `PILOTAGE`). Code déjà pris → **409**. Audit `procedure.created`. Permission **`procedures.create`**.
