...
- **GET /api/compliance/frameworks/catalog** — Cadres plateforme actifs non archivés. Champs : `id`, `name`, `version`, `description`, `provider`, `requirementCount`, `domainCount` (catégories distinctes), `familyLabel` (NIS2, ISO 27001, RGPD… dérivé déterministe), `scope: platform`. Permission **`compliance.read`**.
- **GET /api/compliance/frameworks** — Instances client (mêmes métadonnées catalogue + `isActive`, `nextAuditAt`). Permission **`compliance.read`**.
- **POST /api/compliance/frameworks/activate** — Body `{ platformFrameworkId }` : copie framework + exigences (+ `description` / `provider`) vers le client actif. **409** si déjà **actif** (même `name`+`version`). Si une instance **inactive** existe, la **réactive** (historique conservé, pas de nouvelle copie). Permission **`compliance.update`**. Audit client `compliance.framework.activated` (création) ou `compliance.framework.updated` (réactivation).
...
- **PATCH /api/procedures/:id/draft** — Body `{ contentJson, expectedUpdatedAt? }`. Met à jour le brouillon TipTap (whitelist nodes/marks ; liens `https` only). Optimistic lock → **409**. Archivée → **400**. Audit `procedure.draft.updated` (taille, pas le corps). Permission **`procedures.update`**.
...
UI : `/procedures` (catalogue + création), `/procedures/[id]/edit` (métadonnées + éditeur TipTap + archive).
...
- **POST /api/projects/:projectId/documents/upload** — Multipart `file` (+ `name?`, `category?`, `description?`). Écriture via **stockage documents client** (LOCAL/S3 plateforme, domaine `projets`) ; persiste `storageBucket` + `storageKey`. MIME allowlist projet (PDF, Office, images, txt/csv, zip). Taille max = réglage plateforme (`PlatformMaxFileInterceptor`). Audit **`project.document.created`** (`via: upload`). **`projects.update`**.
- **GET /api/projects/:projectId/documents/:documentId/download** — Stream fichier STARIUM uniquement (422 si EXTERNAL/MICROSOFT ou bucket/clé absents). **`projects.read`**
...
- **PUT** — Création / mise à jour (`UpdateProjectMicrosoftLinkDto`) : `isEnabled`, `teamId`, `channelId`, `plannerPlanId` (**obligatoire non vide** si `isEnabled=true` — flux manuel INT-007 ; un lien créé par provisioning Teams peut exister sans plan Planner tant que l’utilisateur n’active pas le lien manuellement), `syncTasksEnabled`, `syncDocumentsEnabled`, `useMicrosoftPlannerBuckets` (remplace les buckets Starium par l’import des buckets du plan Planner — RFC-PROJ-OPT-001), `filesDriveId`, `filesFolderId`, libellés optionnels. **`projects.update`**
...
- **POST /api/projects/:projectId/microsoft-link/sync-documents** — Sync one-way des `ProjectDocument` **STARIUM** (lecture via stockage documents client `storageBucket`/`storageKey`, domaine `projets`) vers le dossier `starium-project-{projectId}` du drive configuré. Réponse `{ total, synced, failed, skipped }`. Audits **`project.microsoft_documents.synced`** ou **`project.microsoft_sync.failed`**. **`projects.update`**
...
**Worker / file (lot 2)** : queue `project_microsoft_teams_provisioning` ; backoff **30 s** (`PROJECT_MICROSOFT_TEAMS_PROVISIONING_QUEUE_BACKOFF_MS`) ; timeout job processor **8 min** (`PROJECT_MICROSOFT_TEAMS_PROVISIONING_JOB_TIMEOUT_MS`) ; job maintenance stale toutes les **5 min** (`PROJECT_MICROSOFT_TEAMS_PROVISIONING_STALE_MAINTENANCE_INTERVAL_MS`, seuil heartbeat **15 min**). Redémarrer le worker API après déploiement.
