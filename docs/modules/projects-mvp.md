...
| GET | `/projects/:projectId/reviews/:reviewId` | `projects.read` — détail (+ agenda, participants avec `invitedAt`/`lastInvitedAt`, `externalEmail`/`lastEmailedAt` ; champs Graph `microsoftOnlineMeetingId`, `microsoftEventId` ; `snapshotPayload` `null` si non finalisé) |
...
| POST | `/projects/:projectId/reviews/:reviewId/invite` | `projects.update` — invitations (RFC-PROJ-013-1 Phases 2–3) : canaux `in_app`/`email` + actions opt-in Teams/calendrier — voir [API.md](../API.md) §21 |
...
- **Points projet démo** (RFC-PROJ-013 / RFC-PROJ-013-1) : `apps/api/prisma/seed-project-demo-reviews.ts` — statuts `PLANNED` / `IN_REVIEW` / `FINALIZED` / `CANCELLED`, jeux cohérents sur `{prefix}-SEED-01` … `10`, dont des **retours d’expérience** (`POST_MORTEM`, `contentPayload.postMortem`) sur certains projets (**SEED-03**, **SEED-06**, **SEED-09**)
