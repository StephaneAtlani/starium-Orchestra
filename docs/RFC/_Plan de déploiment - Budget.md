...
_Dernière mise à jour : avril 2026 (alignée sur le dépôt `starium-Orchestra` : apps web `features/budgets`, API `modules/budget-*`). **Statuts budget (`BudgetStatus`)** : cycle DRAFT → SUBMITTED → REVISED → VALIDATED → LOCKED → ARCHIVED (migration `20260412120000_budget_status_workflow_cycle`, ancien `ACTIVE` → `VALIDATED`). Aucune **machine à états** imposée côté API (pas de règles de transition ni rôles DAF/DG) — voir ligne « Workflow budgétaire » ci-dessous. Grille prévisionnel : **Écart prév. / rév.** (€) et **% écart prév.** — [RFC-024 — Budget UI](./RFC-024%20%E2%80%94%20Budget%20UI.md) §5._
