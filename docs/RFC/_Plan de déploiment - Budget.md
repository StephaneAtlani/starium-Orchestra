...
| RFC-019 + RFC-015-3 + RFC-033 + UI compare | Versioning & snapshots exploitables     | **Page Versions** et **cycles** T1/T2/clôture ([RFC-033](./RFC-033%20%E2%80%94%20Mise%20en%20place%20des%20versions%20budg%C3%A9taires%20(produit).md)) : `/budgets/[budgetId]/versions`, bloc sur `/budgets/workflow-settings`, permission `budgets.versioning_cycle.manage`. Comparaison forecast/baseline/snapshot/version : UI MVP ([RFC-FE-BUD-030](./RFC-FE-BUD-030%20%E2%80%94%20Forecast%20et%20Comparaison%20budg%C3%A9taire%20UI.md)), pages `/budgets/[budgetId]/reporting`, `/budgets/[budgetId]/snapshots`. Reste : timeline dédiée, écarts avancés, finitions hors MVP | ⚠️ Partiel (comparaison + versions UI MVP ; perfectionnements hors MVP) |
...
**Routes utiles (vérification code)** : liste `/budgets`, cockpit `/budgets/dashboard`, détail budget `/budgets/[budgetId]`, **versions** `/budgets/[budgetId]/versions`, reporting & comparaison `/budgets/[budgetId]/reporting`, import `/budgets/[budgetId]/import`, enveloppe `/budget-envelopes/[envelopeId]`, exercices `/budgets/exercises`, configuration `/budgets/configuration`, réglages cockpit `/budgets/cockpit-settings`, réglage workflow budget (garde « enveloppes non brouillon » avant `VALIDATED` + options cycle versioning) `/budgets/workflow-settings`.
...
| RFC-033    | Versions budgétaires (produit) | UI page Versions, client API, cycles + permission `budgets.versioning_cycle.manage` | ✅ MVP |
