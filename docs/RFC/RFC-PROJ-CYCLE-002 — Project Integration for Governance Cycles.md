...
**Prérequis backend** : [RFC-PROJ-CYCLE-001](./RFC-PROJ-CYCLE-001%20%E2%80%94%20Governance%20Cycles%20Core%20Backend.md) — CRUD cycles + items + scoring §4.5 + KPI global B7 livrés (lots B1–B7) ; endpoint `GET /api/governance-cycles/by-project/:projectId` **non encore implémenté** (RFC-002 / lot B9).
