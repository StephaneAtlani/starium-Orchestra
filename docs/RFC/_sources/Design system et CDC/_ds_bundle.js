...
// design_handoff_demandes_projet/reference-code/demandes.js
try { (() => {
...
const DEM_TYPES = {
  transformation: {
    l: 'Transformation',
    m: 'Refonte, digitalisation métier',
    bg: 'var(--brand-gold-050)',
    fg: 'var(--brand-gold-700)',
    ico: '<rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><path d="M6.5 10v4h11v-4"/><rect x="14" y="14" width="7" height="7"/>'
  },
  infra: {
    l: 'Infrastructure',
    m: 'Cloud, réseau, résilience',
  ...
  reglementaire: {
    l: 'Réglementaire',
    m: 'RGPD, DORA, obligation légale',
    bg: 'var(--state-success-bg)',
    fg: 'var(--state-success)',
    ico: '<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>'
  },
  produit: {
    l: 'Produit & innovation',
    m: 'Nouvelle offre, expérimentation',
    bg: 'var(--purple-bg)',
    fg: 'var(--purple)',
    ico: '<path d="M9 18h6M10 22h4M12 2a7 7 0 0 0-4 12.7c.6.5 1 1.3 1 2.1h6c0-.8.4-1.6 1-2.1A7 7 0 0 0 12 2z"/>'
  },
  evolution: {
    l: 'Évolution applicative',
    m: 'Amélioration d\'un existant',
    bg: 'var(--teal-bg)',
    fg: 'var(--teal)',
    ico: '<path d="M21 2v6h-6"/><path d="M3 12a9 9 0 0 1 15-6.7L21 8"/><path d="M3 22v-6h6"/><path d="M21 12a9 9 0 0 1-15 6.7L3 16"/>'
  }
};
const DEM_ST = {
  brouillon: {
...
function demSeanceOptions(inst) {
  ...
  try {
    if (typeof MTG_LIST !== 'undefined') MTG_LIST.filter(m => m.st === 'plan').forEach(function (m) {
      ...
      out.push({
        v: 'mtg:' + m.id,
        l: m.title + ' — ' + (typeof mtgFDate === 'function' ? mtgFDate(m.date) : m.date),
        m: !!i && i.name === inst
      });
    });
  } catch (e) {}
...
let DEM = [{
  id: 'DP-2026-018',
  t: 'Portail fournisseurs self-care',
  type: 'transformation',
  dir: 'Direction Achats',
  who: 'Nadia Cherif',
  ini: 'NC',
  sponsor: 'Marc Delaunay — DSI',
  date: '2026-09-02',
  ...
  st: 'cycle',
  instance: 'COPIL',
  seance: 'COPIL Transformation — 24 sept. 2026',
  ...
  id: 'DP-2026-017',
  t: 'Mise en conformité DORA — registre des prestataires',
  type: 'reglementaire',
  dir: 'Direction Risques',
  who: 'Amélie Rousseau',
  ini: 'AR',
  sponsor: 'Isabelle Fournier — DG',
  date: '2026-08-28',
  ...
  id: 'DP-2026-016',
  t: 'Refonte du parcours de souscription mobile',
  type: 'produit',
  dir: 'Direction Marketing',
  who: 'Sophie Marchand',
  ini: 'SM',
  sponsor: 'Antoine Roy — Dir. Marketing',
  date: '2026-09-08',
  ...
  id: 'DP-2026-015',
  t: 'Espace documentaire RH unifié',
  type: 'evolution',
  dir: 'Direction RH',
  who: 'Farida Haddad',
  ini: 'FH',
  sponsor: 'Sophie Marchand — DRH adj.',
  date: '2026-09-11',
  ...
  id: 'DP-2026-014',
  t: 'Supervision applicative temps réel',
  type: 'infra',
  dir: 'Direction Technique',
  who: 'Marc Lefèvre',
  ini: 'ML',
  sponsor: 'Marc Delaunay — DSI',
  date: '2026-09-12',
  ...
  koAt: 'arb',
  instance: 'COPIL',
  seance: 'COPIL Transformation — 27 août 2026',
  ...
  id: 'DP-2026-009',
  t: 'Migration du parc bureautique Windows 12',
  type: 'infra',
  dir: 'Direction Technique',
  who: 'Jean Tissot',
  ini: 'JT',
  ...
  prio: 'moyenne',
  st: 'projet',
  proj: 'Migration Poste de travail',
  instance: 'CODIR',
  seance: 'CODIR — 2 juil. 2026',
  ...
  id: 'DP-2026-006',
  t: 'Application de covoiturage interne',
  type: 'produit',
  dir: 'Direction RSE',
  who: 'Hugo Petit',
  ini: 'HP',
  ...
  koAt: 'arb',
  instance: 'COPIL',
  seance: 'COPIL Transformation — 28 mai 2026',
...
function demIco(p, sz) {
  return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"' + (sz ? ' style="width:' + sz + 'px;height:' + sz + 'px"' : '') + '>' + p + '</svg>';
}
function demK(n) {
...
function demBadge(name, size) {
  return typeof avBadge === 'function' ? avBadge(name, size || 'av-sm') : demPerson(name, size);
}
...
function demToday() {
  return new Date('2026-09-14').toLocaleDateString('fr-FR', {
    day: 'numeric',
    month: 'short',
    year: 'numeric'
  });
...
function demSteps(d) {
  const cyc = demNeedsCycle(d),
    inst = demInstance(d) || d.instance;
  const s = [{
    k: 'sub',
    l: 'Soumission',
    s: 'Demandeur'
  }];
  if (DEM_CFG.n1) s.push({
    k: 'n1',
    l: 'Validation N+1',
    s: 'Responsable de direction'
  });
  if (DEM_CFG.instr) s.push({
    k: 'ins',
    l: 'Instruction',
    s: 'PMO · ' + DEM_CFG.delai + ' j ouvrés'
  });
  s.push(cyc ? {
    k: 'arb',
    l: 'Arbitrage ' + inst,
    ...
    k: 'arb',
    l: 'Validation PMO',
    s: 'Hors cycle de pilotage'
  });
  s.push({
    k: 'prj',
    l: 'Création du projet',
    s: DEM_CFG.autoProj ? 'Automatique' : 'Sur action du PMO'
  });
  return s;
}
...
function demLog(d, txt, who) {
  d.jrn.push([txt, who || 'Julie Fontaine — PMO', demToday()]);
}
...
function demShowList() {
  ...
  const kAInstruire = n(d => d.st === 'soumise' || d.st === 'instruction'),
    kCycle = n(d => d.st === 'cycle'),
    kValid = n(d => d.st === 'validee'),
    kKo = n(d => d.st === 'refusee' || d.st === 'ajournee');
  const enveloppe = DEM.filter(d => ['soumise', 'instruction', 'cycle', 'validee'].indexOf(d.st) >= 0).reduce((s, d) => s + d.budget, 0);
  r.innerHTML = '<div class="pg-head">' + '<div><h1>Demandes de projet</h1><p class="np-sub">Point d\'entrée unique des besoins métiers. Chaque demande suit le circuit défini dans la configuration, passe en cycle de pilotage lorsque les seuils l\'exigent, et devient un projet du portefeuille une fois validée.</p></div>' + '<div class="head-actions">' + '<button class="btn btn-secondary" onclick="demOpenConfig()">' + demIco('<path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.6 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.6h.09A1.65 1.65 0 0 0 10 3.09V3a2 2 0 1 1 4 0v.09A1.65 1.65 0 0 0 15 4.6a1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9v.09c.36.14.68.38.91.7"/>') + 'Configuration du circuit</button>' + '<button class="btn btn-primary" onclick="demNew()">' + demIco('<circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/>') + 'Nouvelle demande</button>' + '</div></div>' + '<div class="list-kpis" style="grid-template-columns:repeat(4,1fr)">' + demKpi('À instruire', kAInstruire, 'En attente du PMO ou du N+1', 'var(--state-info-bg)', 'var(--state-info)', '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>') + demKpi('En cycle de pilotage', kCycle, 'Inscrites à un comité', 'var(--brand-gold-050)', 'var(--brand-gold-700)', '<path d="M3 12a9 9 0 1 0 18 0 9 9 0 0 0-18 0"/><polyline points="12 8 12 12 14 14"/>') + demKpi('Validées à convertir', kValid, 'Prêtes à devenir un projet', 'var(--state-success-bg)', 'var(--state-success)', '<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>') + demKpi('Enveloppe en circuit', demK(enveloppe), kKo + ' demande' + (kKo > 1 ? 's' : '') + ' refusée' + (kKo > 1 ? 's' : '') + ' ou ajournée' + (kKo > 1 ? 's' : ''), 'var(--purple-bg)', 'var(--purple)', '<line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>') + '</div>' + '<div class="card tablecard">' + '<div class="dem-bar">' + '<div class="dem-search">' + demIco('<circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>') + '<input id="dem-q" placeholder="Rechercher une demande, une direction…" oninput="demRenderRows()"></div>' + '<div class="seg-toggle" id="dem-seg">' + ['all', 'En circuit', 'Terminées'].map(function (x, i) {
    const k = ['all', 'circuit', 'done'][i];
    return '<button class="seg-btn' + (DEM_FILTER === k ? ' active' : '') + '" onclick="demFilter(\'' + k + '\',this)">' + (k === 'all' ? 'Toutes' : x) + '</button>';
  }).join('') + '</div>' + '<div style="flex:1"></div>' + '<span style="font-size:12px;color:var(--neutral-500)">Seuil de passage en cycle : <b style="color:var(--brand-ink)">' + demK(DEM_CFG.seuilCopil) + '</b></span>' + '</div>' + '<div class="table-wrap"><table class="dem-table"><thead><tr>' + '<th>Réf.</th><th>Demande</th><th>Type</th><th>Demandeur</th><th class="right">Budget estimé</th><th>Circuit</th><th>Statut</th><th></th>' + '</tr></thead><tbody id="dem-rows"></tbody></table></div>' + '</div>' + '<div class="dem-rules">' + '<div class="card dem-rule"><div class="dem-rule-k">Passage en cycle de pilotage</div><div class="dem-rule-v">À partir de ' + demK(DEM_CFG.seuilCopil) + '</div><div class="dem-rule-m">En dessous du seuil, la demande est validée par le PMO sans arbitrage en comité. Au-delà de ' + demK(DEM_CFG.seuilCodir) + ', l\'arbitrage remonte au CODIR.</div></div>' + '<div class="card dem-rule"><div class="dem-rule-k">Étapes actives</div><div class="dem-rule-v">' + demSteps({
    budget: DEM_CFG.seuilCopil,
    type: 'transformation'
  }).length + ' étapes</div><div class="dem-rule-m">' + demSteps({
    budget: DEM_CFG.seuilCopil,
    type: 'transformation'
  }).map(s => s.l).join(' → ') + '</div></div>' + '<div class="card dem-rule"><div class="dem-rule-k">Exemptions</div><div class="dem-rule-v">' + (DEM_CFG.exempt.length ? DEM_CFG.exempt.map(t => DEM_TYPES[t].l).join(', ') : 'Aucune') + '</div><div class="dem-rule-m">Ces types de demandes sont validés hors cycle quel que soit leur montant — typiquement les obligations réglementaires à échéance contrainte.</div></div>' + '</div>' + demModals();
  demRenderRows();
}
...
function demRenderRows() {
  const tb = document.getElementById('dem-rows');
  if (!tb) return;
  ...
  tb.innerHTML = rows.length ? rows.map(function (d) {
    const ty = DEM_TYPES[d.type],
      ...
      cyc = demNeedsCycle(d),
      inst = demInstance(d) || d.instance;
    const route = cyc ? '<span class="dem-route" style="background:var(--brand-gold-050);color:var(--brand-gold-700)">' + demIco('<path d="M3 12a9 9 0 1 0 18 0 9 9 0 0 0-18 0"/><polyline points="12 8 12 12 14 14"/>') + inst + '</span>' : '<span class="dem-route" style="background:var(--neutral-100);color:var(--neutral-600)">' + demIco('<polyline points="20 6 9 17 4 12"/>') + 'Hors cycle</span>';
    return '<tr onclick="demOpen(\'' + d.id + '\')">' + '<td><span class="dem-ref">' + d.id + '</span></td>' + '<td><div class="dem-title">' + demEsc(d.t) + '</div><div class="dem-sub">' + demEsc(d.dir) + ' · déposée le ' + demDate(d.date) + '</div></td>' + '<td><span class="dem-route" style="background:' + ty.bg + ';color:' + ty.fg + '">' + ty.l + '</span></td>' + '<td>' + demPerson(d.who) + '</td>' + '<td class="right"><span class="dem-num">' + demK(d.budget) + '</span></td>' + '<td>' + route + (d.seance && d.st === 'cycle' ? '<div class="dem-sub" style="margin-top:4px">' + demEsc(d.seance.replace(/^[A-Z]+ ?[A-Za-z]* — /, '')) + '</div>' : '') + '</td>' + '<td><span class="badge ' + st.bdg + '">' + st.l + '</span>' + (d.st === 'projet' ? '<div class="dem-sub" style="margin-top:4px">' + demEsc(d.proj) + '</div>' : '') + '</td>' + '<td class="dem-chev">' + demIco('<polyline points="9 18 15 12 9 6"/>') + '</td>' + '</tr>';
...
function demOpen(id) {
  ...
  const ty = DEM_TYPES[d.type],
    st = DEM_ST[d.st],
    steps = demSteps(d),
    reached = demReached(d);
  const ki = steps.map(s => s.k).indexOf(reached);
  const ko = d.st === 'refusee' || d.st === 'ajournee';
  r.innerHTML = '<div style="display:flex;align-items:center;gap:10px;margin-bottom:16px">' + '<button class="btn btn-secondary" onclick="demShowList()">' + demIco('<polyline points="15 18 9 12 15 6"/>') + 'Toutes les demandes</button>' + '<span style="font-size:12.5px;color:var(--neutral-500)">' + d.id + ' · déposée le ' + demDate(d.date) + '</span>' + '</div>' + '<div class="card dem-hero">' + '<div class="dem-hero-ico" style="background:' + ty.bg + ';color:' + ty.fg + '">' + demIco(ty.ico) + '</div>' + '<div class="dem-hero-mid"><h2>' + demEsc(d.t) + '</h2>' + '<div class="dem-hero-meta"><span class="badge ' + st.bdg + '">' + st.l + '</span><span class="dem-route" style="background:' + ty.bg + ';color:' + ty.fg + '">' + ty.l + '</span><span>' + demEsc(d.dir) + '</span><span>·</span>' + demPerson(d.who, 'av-xs') + '<span>·</span><span>Sponsor</span>' + demPerson(d.sponsor.split(' — ')[0], 'av-xs') + '</div></div>' + '<div class="dem-hero-acts">' + demActions(d) + '</div>' + '</div>' + '<div class="dem-flow">' + steps.map(function (s, i) {
    let cls = '';
    if (ko && i === ki) cls = 'ko';else if (d.st === 'projet' || i < ki) cls = 'done';else if (i === ki) cls = 'cur';
  ...
  }).join('') + '</div>' + '<div class="dem-grid">' + '<div class="card dem-card">' + '<div class="dem-sec"><div class="dem-sec-t">Besoin exprimé</div><div class="dem-body">' + demEsc(d.need) + '</div></div>' + '<div class="dem-sec"><div class="dem-sec-t">Objectifs visés</div><div class="dem-obj">' + d.obj.map(o => '<div class="dem-obj-i">' + demIco('<polyline points="20 6 9 17 4 12"/>') + '<span>' + demEsc(o) + '</span></div>').join('') + '</div></div>' + '<div class="dem-sec"><div class="dem-sec-t">Bénéfices attendus</div><div class="dem-body muted">' + demEsc(d.benef) + '</div></div>' + '<div class="dem-sec"><div class="dem-sec-t">Cadrage estimatif</div><div class="dem-kv">' + '<div class="dem-kv-i"><div class="dem-kv-k">Budget estimé</div><div class="dem-kv-v">' + fmtEur(d.budget) + '</div></div>' + '<div class="dem-kv-i"><div class="dem-kv-k">Charge interne</div><div class="dem-kv-v">' + d.charge + ' j·h</div></div>' + '<div class="dem-kv-i"><div class="dem-kv-k">Échéance souhaitée</div><div class="dem-kv-v" style="font-size:14px">' + demDate(d.deadline) + '</div></div>' + '<div class="dem-kv-i"><div class="dem-kv-k">Priorité demandée</div><div class="dem-kv-v" style="font-size:14px">' + {
    haute: 'Haute',
    moyenne: 'Moyenne',
    basse: 'Basse'
  }[d.prio] + '</div></div>' + '</div></div>' + '</div>' + '<div>' + '<div class="card dem-panel">' + demDecisionPanel(d) + '</div>' + '<div class="card dem-panel"><div class="dem-panel-t">Journal de la demande</div><div class="dem-panel-s">Toutes les étapes sont horodatées et reprises dans le dossier d\'arbitrage.</div>' + '<div class="dem-jrn">' + d.jrn.slice().reverse().map(function (j, i) {
  ...
  }).join('') + '</div>' + '</div>' + '</div>' + '</div>' + demModals();
}
function demActions(d) {
  ...
  if (s === 'cycle') return d.seance ? '<button class="btn btn-secondary" onclick="showView(\'cycles\')">Voir le cycle</button><button class="btn btn-primary" onclick="demOpenDec()">' + demIco('<polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/>') + 'Enregistrer la décision</button>' : '<button class="btn btn-primary" onclick="demOpenOdj()">' + demIco('<rect x="3" y="4" width="18" height="18" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/>') + 'Inscrire à l\'ordre du jour</button>';
...
function demDecisionPanel(d) {
  ...
  let t, s;
  if (d.st === 'brouillon') {
    t = 'Demande en cours de rédaction';
    s = 'Complétez le dossier puis soumettez-le. Le circuit sera déterminé automatiquement à la soumission.';
  } else if (d.st === 'soumise') {
    t = 'En attente de validation N+1';
    s = 'Le responsable de ' + d.dir + ' doit confirmer l\'opportunité avant instruction par le PMO.';
  } else if (d.st === 'instruction') {
    t = 'Instruction par le PMO';
    s = 'Vérification de la faisabilité, du chiffrage et de l\'alignement au schéma directeur. Délai cible : ' + DEM_CFG.delai + ' jours ouvrés.';
  } else if (d.st === 'cycle') {
    t = d.seance ? 'Inscrite au ' + inst : 'À inscrire en ' + inst;
    s = d.seance ? 'Point programmé : ' + d.seance + '. La décision du comité clôturera l\'arbitrage.' : 'Le montant (' + demK(d.budget) + ') dépasse le seuil de ' + demK(DEM_CFG.seuilCopil) + ' : un arbitrage en ' + inst + ' est requis.';
  } else if (d.st === 'validee') {
    t = 'Validée — prêt à devenir un projet';
    s = 'La création du projet reprend le cadrage, le budget et le sponsor de la demande.';
  } else if (d.st === 'projet') {
    t = 'Projet créé';
    s = 'La demande est clôturée ; le suivi se poursuit dans la fiche projet « ' + (d.proj || d.t) + ' ».';
  } else {
    t = d.st === 'refusee' ? 'Demande refusée' : 'Demande ajournée';
    s = d.jrn[d.jrn.length - 1][0];
  }
  return '<div class="dem-panel-t">' + t + '</div><div class="dem-panel-s">' + demEsc(s) + '</div>' + '<div class="dem-prev-row"><span class="k">Circuit appliqué</span><span class="v">' + (cyc ? inst : 'Hors cycle') + '</span></div>' + '<div class="dem-prev-row"><span class="k">Validation N+1</span><span class="v">' + (DEM_CFG.n1 ? 'Requise' : 'Désactivée') + '</span></div>' + '<div class="dem-prev-row"><span class="k">Instruction PMO</span><span class="v">' + (DEM_CFG.instr ? 'Requise' : 'Désactivée') + '</span></div>' + '<div class="dem-prev-row"><span class="k">Séance d\'arbitrage</span><span class="v">' + (d.seance ? demEsc(d.seance) : '—') + '</span></div>' + '<div class="dem-note">' + demIco('<circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/>') + '<span>Le circuit est recalculé à chaque modification du budget ou du type. Règles modifiables dans <a class="cyc-link" style="display:inline" onclick="demOpenConfig()">la configuration</a>.</span></div>';
}

/* ─── Transitions ─── */
function demRefresh() {
  if (DEM_CUR) demOpen(DEM_CUR);else demShowList();
}
function demSubmit(id) {
  ...
  if (d.st === 'cycle') {
    d.instance = demInstance(d);
    demLog(d, 'Circuit sans instruction — arbitrage en ' + d.instance + ' requis');
  }
...
function demReopen(id) {
  ...
  showToast('Demande rouverte en brouillon');
  demRefresh();
}
...
function demAddProjectRow(d) {
  const tb = document.querySelector('#plTableCard tbody');
  if (!tb) return;
  const ty = DEM_TYPES[d.type];
  const tr = document.createElement('tr');
  tr.style.cursor = 'pointer';
  tr.setAttribute('onclick', "showView('detail')");
  tr.innerHTML = '<td><div class="cat-cell"><div class="cat-ico ti-gold">' + demIco(ty.ico) + '</div><span class="cat-name">' + ty.l + '</span></div></td>' + '<td><div class="pname">' + demEsc(d.t) + '</div><div class="psub2">Issu de la demande ' + d.id + '</div></td>' + '<td><span class="nature">Build</span></td>' + '<td><span class="sante">Non évaluée</span></td>' + '<td><span class="badge bdg-info">Cadrage</span></td>' + '<td><span class="mrole">Sponsor</span></td>' + '<td><div class="prog-cell"><span class="prog-val">0%</span></div></td>' + '<td>' + demDate(d.deadline) + '</td>' + '<td><span class="dem-sub">— · — · —</span></td>' + '<td><span class="dem-sub">Nouveau</span></td>' + '<td><span class="badge bdg-neutral">Demande ' + d.id + '</span></td>';
  tb.insertBefore(tr, tb.firstChild);
}

/* ─── Modales ─── */
function demModals() {
  return '' + '<div class="modal-overlay" id="demCfgModal" onclick="if(event.target===this)demCloseCfg()"><div class="modal modal-dem">' + '<div class="modal-head"><div class="modal-head-ico">' + demIco('<path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/><path d="M4 12h2M18 12h2M12 4v2M12 18v2"/>') + '</div>' + '<div><div class="modal-title">Configuration du circuit de demande</div><div class="modal-sub">Détermine les étapes imposées et le déclenchement du passage en cycle de pilotage.</div></div>' + '<button class="modal-close" onclick="demCloseCfg()">' + demIco('<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>') + '</button></div>' + '<div class="modal-body">' + '<div class="field-row3" style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px">' + '<div class="field"><label class="field-label">Seuil COPIL (€)</label><input class="input" id="cfg-copil" type="number" step="5000" value="' + DEM_CFG.seuilCopil + '"></div>' + '<div class="field"><label class="field-label">Seuil CODIR (€)</label><input class="input" id="cfg-codir" type="number" step="10000" value="' + DEM_CFG.seuilCodir + '"></div>' + '<div class="field"><label class="field-label">Délai d\'instruction (j)</label><input class="input" id="cfg-delai" type="number" value="' + DEM_CFG.delai + '"></div>' + '</div>' + '<div class="dem-sec-t" style="margin-top:6px">Étapes du circuit</div>' + demSw('n1', 'Validation hiérarchique (N+1)', 'Le responsable de la direction demandeuse valide avant instruction.') + demSw('instr', 'Instruction par le PMO', 'Étude de faisabilité, chiffrage et alignement au schéma directeur.') + demSw('autoProj', 'Création automatique du projet', 'Le projet est créé dès la validation, sans action manuelle du PMO.') + '<div class="dem-sec-t" style="margin-top:18px">Types exemptés de cycle de pilotage</div>' + '<div class="dem-pillrow">' + Object.keys(DEM_TYPES).map(k => '<button class="dem-pill' + (DEM_CFG.exempt.indexOf(k) >= 0 ? ' sel' : '') + '" data-t="' + k + '" onclick="this.classList.toggle(\'sel\')">' + DEM_TYPES[k].l + '</button>').join('') + '</div>' + '<div class="dem-note" style="margin-top:14px">' + demIco('<circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/>') + '<span>Les demandes déjà engagées conservent leur historique ; le circuit restant est recalculé avec les nouvelles règles.</span></div>' + '</div>' + '<div class="modal-foot"><button class="btn btn-secondary" onclick="demCloseCfg()">Annuler</button><button class="btn btn-primary" onclick="demSaveCfg()">Enregistrer la configuration</button></div>' + '</div></div>' + '<div class="modal-overlay" id="demOdjModal" onclick="if(event.target===this)demCloseOdj()"><div class="modal">' + '<div class="modal-head"><div class="modal-head-ico">' + demIco('<rect x="3" y="4" width="18" height="18" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/>') + '</div>' + '<div><div class="modal-title">Inscrire la demande à un cycle de pilotage</div><div class="modal-sub">Le point est ajouté à l\'ordre du jour avec le dossier d\'instruction.</div></div>' + '<button class="modal-close" onclick="demCloseOdj()">' + demIco('<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>') + '</button></div>' + '<div class="modal-body"><div class="field"><label class="field-label">Séance</label><select class="input" id="odj-seance"></select></div>' + '<div class="field"><label class="field-label">Type de point</label><select class="input" id="odj-kind"><option>Décision — Go / No Go</option><option>Arbitrage budgétaire</option><option>Information</option></select></div>' + '<div class="field"><label class="field-label">Temps demandé</label><select class="input" id="odj-time"><option>10 min</option><option selected>15 min</option><option>20 min</option><option>30 min</option></select></div></div>' + '<div class="modal-foot"><button class="btn btn-secondary" onclick="demCloseOdj()">Annuler</button><button class="btn btn-primary" onclick="demSaveOdj()">Inscrire à l\'ordre du jour</button></div>' + '</div></div>' + '<div class="modal-overlay" id="demDecModal" onclick="if(event.target===this)demCloseDec()"><div class="modal">' + '<div class="modal-head"><div class="modal-head-ico">' + demIco('<polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/>') + '</div>' + '<div><div class="modal-title">Décision du comité</div><div class="modal-sub" id="dec-sub">—</div></div>' + '<button class="modal-close" onclick="demCloseDec()">' + demIco('<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>') + '</button></div>' + '<div class="modal-body">' + '<label class="field-label" style="display:block;margin-bottom:10px">Issue de l\'arbitrage</label>' + '<div class="type-pills" id="dec-pills"><button class="type-pill sel" data-k="validee" onclick="demPick(this)">Validée</button><button class="type-pill" data-k="ajournee" onclick="demPick(this)">Ajournée</button><button class="type-pill" data-k="refusee" onclick="demPick(this)">Refusée</button></div>' + '<div class="field" style="margin-top:18px"><label class="field-label">Motivation / conditions</label><textarea class="textarea" id="dec-txt" placeholder="Ex. : validée sous réserve d\'un lot 1 limité au périmètre facturation."></textarea></div>' + '</div>' + '<div class="modal-foot"><button class="btn btn-secondary" onclick="demCloseDec()">Annuler</button><button class="btn btn-primary" onclick="demSaveDec()">Enregistrer la décision</button></div>' + '</div></div>' + '<div class="modal-overlay" id="demInstrModal" onclick="if(event.target===this)demCloseInstr()"><div class="modal">' + '<div class="modal-head"><div class="modal-head-ico">' + demIco('<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>') + '</div>' + '<div><div class="modal-title">Conclusion de l\'instruction</div><div class="modal-sub">Le circuit aval est déterminé par le budget retenu et la configuration.</div></div>' + '<button class="modal-close" onclick="demCloseInstr()">' + demIco('<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>') + '</button></div>' + '<div class="modal-body">' + '<label class="field-label" style="display:block;margin-bottom:10px">Avis du PMO</label>' + '<div class="type-pills" id="ins-pills"><button class="type-pill sel" data-k="fav" onclick="demPick(this)">Favorable</button><button class="type-pill" data-k="res" onclick="demPick(this)">Réservé</button><button class="type-pill" data-k="def" onclick="demPick(this)">Défavorable</button></div>' + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:18px">' + '<div class="field"><label class="field-label">Budget retenu (€)</label><input class="input" id="ins-budget" type="number" step="5000"></div>' + '<div class="field"><label class="field-label">Charge retenue (j·h)</label><input class="input" id="ins-charge" type="number"></div>' + '</div>' + '<div class="field"><label class="field-label">Synthèse d\'instruction</label><textarea class="textarea" id="ins-txt" placeholder="Faisabilité, dépendances, alignement au schéma directeur…"></textarea></div>' + '<div class="dem-note" id="ins-route"></div>' + '</div>' + '<div class="modal-foot"><button class="btn btn-secondary" onclick="demCloseInstr()">Annuler</button><button class="btn btn-primary" onclick="demSaveInstr()">Conclure l\'instruction</button></div>' + '</div></div>';
}
function demSw(key, label, meta) {
  return '<div class="dem-cfg-row"><div class="dem-cfg-b"><div class="dem-cfg-l">' + label + '</div><div class="dem-cfg-m">' + meta + '</div></div>' + '<button class="dem-sw' + (DEM_CFG[key] ? ' on' : '') + '" id="cfg-' + key + '" onclick="this.classList.toggle(\'on\')" aria-label="' + label + '"></button></div>';
}
function demPick(el) {
  el.parentElement.querySelectorAll('.type-pill').forEach(p => p.classList.remove('sel'));
  el.classList.add('sel');
  if (el.parentElement.id === 'ins-pills') demInsRoute();
}
function demSel(id) {
  const w = document.getElementById(id);
  const s = w && w.querySelector('.type-pill.sel');
  return s ? s.dataset.k : null;
}
function demOpenConfig() {
  document.getElementById('demCfgModal').classList.add('open');
}
function demCloseCfg() {
  document.getElementById('demCfgModal').classList.remove('open');
}
function demSaveCfg() {
...
function demOpenOdj() {
  ...
  const inst = demInstance(d) || 'COPIL';
...
function demCloseOdj() {
  document.getElementById('demOdjModal').classList.remove('open');
}
function demSaveOdj() {
  const d = DEM.find(x => x.id === DEM_CUR);
  const sel = document.getElementById('odj-seance');
  const kind = document.getElementById('odj-kind').value,
    time = document.getElementById('odj-time').value;
...
function demPushAgenda(d, kind, time) {
  const ref = d.seanceRef || '';
  const label = 'Demande ' + d.id + ' — ' + d.t;
  if (ref.indexOf('prep:') === 0 && typeof INST_PREP === 'object') {
    const k = ref.slice(5),
      x = INST_PREP[k];
    if (!x) return;
    x.agenda.push([kind.split(' — ')[0] + ' — ' + label, time, null]);
    if (typeof curPrepKey !== 'undefined' && curPrepKey === k && typeof prepRerender === 'function' && document.getElementById('prepModal') && document.getElementById('prepModal').classList.contains('open')) prepRerender();
  } else if (ref.indexOf('mtg:') === 0 && typeof MTG_LIST !== 'undefined') {
    const m = MTG_LIST.find(x => x.id === ref.slice(4));
    if (!m) return;
    m.ag = m.ag || [];
    m.ag.push({
      t: label,
      d: parseInt(time, 10) || 15,
      o: d.who,
      k: kind.indexOf('Information') >= 0 ? 'info' : kind.indexOf('Arbitrage') >= 0 ? 'arb' : 'deci'
    });
    if (typeof mtgRenderList === 'function' && document.getElementById('mtg-reunions')) try {
      mtgRenderList();
    } catch (e) {}
  }
}
...
function demCloseDec() {
  document.getElementById('demDecModal').classList.remove('open');
}
function demSaveDec() {
  const d = DEM.find(x => x.id === DEM_CUR),
    k = demSel('dec-pills') || 'validee';
...
function demCloseInstr() {
  document.getElementById('demInstrModal').classList.remove('open');
}
function demInsRoute() {
  ...
  const cyc = demNeedsCycle(probe),
    inst = demInstance(probe);
  const avis = demSel('ins-pills');
  const el = document.getElementById('ins-route');
  if (!el) return;
  el.innerHTML = demIco('<circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/>') + '<span>' + (avis === 'def' ? 'Avis défavorable : la demande sera refusée à l\'issue de l\'instruction.' : cyc ? 'Avec ' + demK(b) + ', la demande passera en arbitrage <b>' + inst + '</b> (seuil ' + demK(DEM_CFG.seuilCopil) + (inst === 'CODIR' ? ' / CODIR ' + demK(DEM_CFG.seuilCodir) : '') + ').' : 'Avec ' + demK(b) + ', la demande sera <b>validée hors cycle de pilotage</b>' + (DEM_CFG.exempt.indexOf(d.type) >= 0 ? ' (type exempté)' : '') + '.') + '</span>';
}
function demSaveInstr() {
  const d = DEM.find(x => x.id === DEM_CUR),
    avis = demSel('ins-pills') || 'fav';
  ...
  if (avis === 'def') {
    d.st = 'refusee';
    d.koAt = 'ins';
    showToast('Instruction conclue · demande refusée');
    demRefresh();
    return;
  }
  d.st = demNext(d, 'ins');
  if (d.st === 'cycle') {
    d.instance = demInstance(d);
    showToast('Instruction conclue · arbitrage en ' + d.instance + ' requis');
  } else if (DEM_CFG.autoProj) {
    demRefresh();
    demConvert(d.id);
    return;
...
function demForm(d) {
  ...
  r.innerHTML = '<div class="pg-head" style="margin-bottom:20px"><div><h1>' + (d ? 'Modifier la demande' : 'Nouvelle demande de projet') + '</h1><p class="np-sub">' + (d ? d.id : 'Décrivez le besoin : le circuit de validation est déduit du type et du budget estimé.') + '</p></div>' + '<div class="head-actions"><button class="btn btn-secondary" onclick="' + (d ? 'demOpen(\'' + d.id + '\')' : 'demShowList()') + '">' + demIco('<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>') + 'Annuler</button></div></div>' + '<div class="dem-form-grid">' + '<div class="card dem-card">' + '<div class="dem-sec" style="margin-top:0"><div class="dem-sec-t">Nature de la demande</div>' + '<div class="type-grid">' + Object.keys(DEM_TYPES).map(function (k) {
    const t = DEM_TYPES[k];
  ...
  }).join('') + '</div></div>' + '<div class="dem-sec"><div class="dem-sec-t">Identification</div>' + '<div class="field"><label class="field-label">Intitulé de la demande <span class="req">*</span></label><input class="input" id="df-t" value="' + demEsc(v.t) + '" oninput="demFormSync()" placeholder="Ex. : Portail fournisseurs self-care"></div>' + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">' + '<div class="field"><label class="field-label">Direction demandeuse <span class="req">*</span></label><input class="input" id="df-dir" value="' + demEsc(v.dir) + '" oninput="demFormSync()" placeholder="Direction Achats"></div>' + '<div class="field"><label class="field-label">Demandeur</label><input class="input" id="df-who" value="' + demEsc(v.who) + '" oninput="demFormSync()" placeholder="Prénom Nom"></div>' + '</div>' + '<div class="field"><label class="field-label">Sponsor pressenti</label><input class="input" id="df-sponsor" value="' + demEsc(v.sponsor) + '" placeholder="Marc Delaunay — DSI"></div>' + '</div>' + '<div class="dem-sec"><div class="dem-sec-t">Besoin &amp; objectifs</div>' + '<div class="field"><label class="field-label">Situation actuelle et besoin <span class="req">*</span></label><textarea class="textarea" id="df-need" rows="4" placeholder="Décrivez le problème constaté, ses impacts et son origine.">' + demEsc(v.need) + '</textarea></div>' + '<div class="field"><label class="field-label">Objectifs visés — un par ligne</label><textarea class="textarea" id="df-obj" rows="3" placeholder="Réduire le délai de traitement de 5 à 1 jour">' + demEsc(v.obj.join('\n')) + '</textarea></div>' + '<div class="field"><label class="field-label">Bénéfices attendus</label><textarea class="textarea" id="df-benef" rows="2" placeholder="Gains quantifiés, conformité, réduction de risque…">' + demEsc(v.benef) + '</textarea></div>' + '</div>' + '<div class="dem-sec"><div class="dem-sec-t">Cadrage estimatif</div>' + '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px">' + '<div class="field"><label class="field-label">Budget estimé (€) <span class="req">*</span></label><input class="input" id="df-budget" type="number" step="5000" value="' + v.budget + '" oninput="demFormSync()" placeholder="120000"></div>' + '<div class="field"><label class="field-label">Charge interne (j·h)</label><input class="input" id="df-charge" type="number" value="' + v.charge + '" placeholder="120"></div>' + '<div class="field"><label class="field-label">Échéance souhaitée</label><input class="input" id="df-deadline" type="date" value="' + (v.deadline || '') + '"></div>' + '</div>' + '<label class="field-label" style="display:block;margin:6px 0 10px">Priorité demandée</label>' + '<div class="type-pills" id="df-prio">' + [['haute', 'Haute'], ['moyenne', 'Moyenne'], ['basse', 'Basse']].map(p => '<button class="type-pill' + (v.prio === p[0] ? ' sel' : '') + '" data-k="' + p[0] + '" onclick="demPick(this)">' + p[1] + '</button>').join('') + '</div>' + '</div>' + '<div style="display:flex;justify-content:flex-end;gap:12px;margin-top:22px">' + '<button class="btn btn-secondary" onclick="demFormSave(false)">' + demIco('<path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/>') + 'Enregistrer en brouillon</button>' + '<button class="btn btn-primary" onclick="demFormSave(true)">' + demIco('<line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/>') + 'Soumettre la demande</button>' + '</div>' + '</div>' + '<div class="card dem-prev" id="df-prev"></div>' + '</div>' + demModals();
  demFormSync();
}
function demFormType(el) {
  el.parentElement.querySelectorAll('.typecard').forEach(t => t.classList.remove('sel'));
  el.classList.add('sel');
  demFormSync();
}
...
function demFormSync() {
  const p = document.getElementById('df-prev');
  if (!p) return;
  const f = demFormRead(),
    steps = demSteps(f),
    cyc = demNeedsCycle(f),
    inst = demInstance(f);
  p.innerHTML = '<div class="dem-prev-k">Circuit qui sera appliqué</div>' + '<div class="dem-jrn">' + steps.map(function (s, i) {
    return '<div class="dem-jrn-i"><div class="dem-jrn-d" style="background:' + (i === 0 ? 'var(--brand-gold)' : 'var(--neutral-300)') + '"></div><div class="dem-jrn-b"><div class="dem-jrn-t">' + s.l + '</div><div class="dem-jrn-m">' + s.s + '</div></div></div>';
  }).join('') + '</div>' + '<div class="dem-prev-row"><span class="k">Type</span><span class="v">' + DEM_TYPES[f.type].l + '</span></div>' + '<div class="dem-prev-row"><span class="k">Budget estimé</span><span class="v">' + (f.budget ? demK(f.budget) : '—') + '</span></div>' + '<div class="dem-prev-row"><span class="k">Cycle de pilotage</span><span class="v" style="color:' + (cyc ? 'var(--brand-gold-700)' : 'var(--neutral-600)') + '">' + (cyc ? 'Requis · ' + inst : 'Non requis') + '</span></div>' + '<div class="dem-note">' + demIco('<circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/>') + '<span>' + (cyc ? 'Au-delà de ' + demK(DEM_CFG.seuilCopil) + ', la demande est arbitrée en ' + inst + ' avant création du projet.' : 'En dessous du seuil' + (DEM_CFG.exempt.indexOf(f.type) >= 0 ? ' ou pour un type exempté' : '') + ', le PMO valide sans passer en comité.') + '</span></div>';
}
function demFormSave(submit) {
  const f = demFormRead();
  if (!f.t) {
    showToast('Donnez un intitulé à la demande');
    return;
  }
  if (!f.dir) {
    showToast('Indiquez la direction demandeuse');
    return;
  }
  ...
  if (d) {
    ...
    d = {
      id: 'DP-2026-0' + DEM_SEQ++,
      ini: ini,
      date: '2026-09-14',
      st: 'brouillon',
      instance: null,
      seance: null,
      jrn: [['Demande créée', f.who || 'Demandeur', demToday()]]
    };
    Object.assign(d, f);
    DEM.unshift(d);
  }
  DEM_CUR = d.id;
  if (submit && d.st === 'brouillon') {
    demSubmit(d.id);
    return;
  }
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "design_handoff_demandes_projet/reference-code/demandes.js", error: String((e && e.message) || e) }); }
...
// ui_kits/app/modules/demandes.js
try { (() => {
/* Demandes de projet — entonnoir amont du portefeuille
   Saisie d'une demande → circuit de validation (avec ou sans passage en cycle de
   pilotage, selon la configuration) → création du projet.
...
let DEM = [{
  ...
  scope: 'Service Achats (14 personnes), comptabilité fournisseurs (6), et les 480 fournisseurs référencés. Tous sites.',
  users: 500,
  ...
  objective: 'SO-1',
  val: 'Claire Besson — Dir. Achats',
  swot: {
    s: "Équipe Achats mobilisée, processus déjà cartographié, référentiel fournisseurs à jour.",
    w: "Aucune compétence interne sur les portails externes ; dépendance à l'ERP comptable.",
    o: "Mise en conformité au devoir de vigilance ; socle d'authentification externe déjà acquis en 2026.",
    t: "Fenêtre de clôture comptable d'avril 2027 ; résistance possible des petits fournisseurs au dépôt en ligne."
  },
  tows: {
    so: "Capitaliser sur la cartographie existante pour livrer un lot 1 « dépôt de factures » avant la clôture d'avril.",
    wo: "Faire porter l'authentification externe par le socle existant plutôt que de recruter la compétence.",
    st: "Embarquer les 30 principaux fournisseurs en pilote pour sécuriser l'adhésion avant généralisation.",
    wt: "Limiter le périmètre du lot 1 à la facturation pour ne pas dépendre d'une évolution de l'ERP."
  },
  docs: [{
    n: 'Cartographie du processus actuel.pdf',
    s: '1,4 Mo'
  }, {
    n: 'Pénalités de retard 2025.xlsx',
    s: '82 Ko'
  }],
  budget: 180000,
  charge: 210,
  ...
  scope: 'Direction Risques, achats IT, 120 contrats prestataires.',
  users: 35,
  deadlineWhy: 'Échéance réglementaire DORA au 17 janvier 2027.',
  constraints: 'Réglementaire (DORA) · données contractuelles sensibles.',
  solutions: 'Suivi Excel actuel, non auditable.',
  objective: 'SO-2',
  val: 'Isabelle Fournier — DG',
  docs: [{
    n: 'Exigences DORA — registre TIC.pdf',
    s: '640 Ko'
  }],
  budget: 40000,
  charge: 60,
  ...
  scope: 'Parcours de souscription mobile, tous produits ; 120 000 visiteurs mensuels.',
  users: 120000,
  deadlineWhy: 'Campagne d\'acquisition prévue à la rentrée 2027.',
  constraints: 'Conformité DSP2 sur le paiement ; refonte à iso-back-office.',
  solutions: 'Optimisations ponctuelles du tunnel en 2025 (gain de 3 pts seulement).',
  objective: 'SO-3',
  val: 'Antoine Roy — Dir. Marketing',
  docs: [],
  budget: 260000,
  charge: 340,
  deadline: '2027-06-30',
...
const DEM_OBJ = [{
  id: 'SO-1',
  l: 'Industrialiser les processus de gestion'
}, {
  id: 'SO-2',
  l: 'Sécuriser la conformité et la maîtrise des risques'
}, {
  id: 'SO-3',
  l: 'Améliorer l\'expérience client'
}, {
  id: 'SO-4',
  l: 'Moderniser le socle technique'
}, {
  id: 'SO-5',
  l: 'Développer l\'engagement des collaborateurs'
}];
const DEM_URG = {
  haute: {
    l: 'Haute',
    bdg: 'bdg-danger',
    m: 'Bloquant à court terme'
  },
  moyenne: {
    l: 'Moyenne',
    bdg: 'bdg-warn',
    m: 'À traiter dans l\'année'
  },
  basse: {
    l: 'Basse',
    bdg: 'bdg-neutral',
    m: 'Sans échéance contrainte'
  }
};
function demPeople() {
  const out = [];
  try {
    if (typeof DIR === 'object' && DIR) Object.keys(DIR).forEach(function (k) {
      out.push(DIR[k][0] + ' — ' + DIR[k][1]);
    });
  } catch (e) {}
  ['Marc Delaunay — DSI', 'Isabelle Fournier — DG', 'Claire Besson — Dir. Achats', 'Antoine Roy — Dir. Marketing', 'Julie Fontaine — PMO'].forEach(function (n) {
    if (out.indexOf(n) < 0) out.push(n);
  });
  return out;
}
function demObjLabel(id) {
  const o = DEM_OBJ.find(x => x.id === id);
  return o ? o.l : null;
}
...
DEM.forEach(function (d) {
  ...
  if (!d.docs) d.docs = [];
  ['outcome', 'risk', 'scope', 'deadlineWhy', 'constraints', 'solutions', 'objective'].forEach(function (k) {
...
function demShowList() {
  ...
  r.innerHTML = '<div class="pg-head">' + '<div><h1>Demandes de projet</h1><p class="np-sub">Point d\'entrée unique des besoins métiers. Chaque demande suit le circuit défini dans la configuration, passe en cycle de pilotage lorsque les seuils l\'exigent, et devient un projet du portefeuille une fois validée.</p></div>' + '<div class="head-actions">' + '<button class="btn btn-secondary" onclick="demOpenConfig()">' + demIco('<path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.6 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.6h.09A1.65 1.65 0 0 0 10 3.09V3a2 2 0 1 1 4 0v.09A1.65 1.65 0 0 0 15 4.6a1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9v.09c.36.14.68.38.91.7"/>') + 'Configuration du circuit</button>' + '<button class="btn btn-primary" onclick="demNew()">' + demIco('<circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/>') + 'Nouvelle demande</button>' + '</div></div>' + '<div class="list-kpis" style="grid-template-columns:repeat(4,1fr)">' + demKpi('À instruire', kAInstruire, 'En attente du PMO ou du N+1', 'var(--state-info-bg)', 'var(--state-info)', '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>') + demKpi('En cycle de pilotage', kCycle, 'Inscrites à un comité', 'var(--brand-gold-050)', 'var(--brand-gold-700)', '<path d="M3 12a9 9 0 1 0 18 0 9 9 0 0 0-18 0"/><polyline points="12 8 12 12 14 14"/>') + demKpi('Validées à convertir', kValid, 'Prêtes à devenir un projet', 'var(--state-success-bg)', 'var(--state-success)', '<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/>') + demKpi('Enveloppe en circuit', demK(enveloppe), kKo + ' demande' + (kKo > 1 ? 's' : '') + ' refusée' + (kKo > 1 ? 's' : '') + ' ou ajournée' + (kKo > 1 ? 's' : ''), 'var(--purple-bg)', 'var(--purple)', '<line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>') + '</div>' + '<div class="card tablecard">' + '<div class="dem-bar">' + '<div class="dem-search">' + demIco('<circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>') + '<input id="dem-q" placeholder="Rechercher une demande, une direction…" oninput="demRenderRows()"></div>' + '<div class="seg-toggle" id="dem-seg">' + ['all', 'En circuit', 'Terminées'].map(function (x, i) {
    const k = ['all', 'circuit', 'done'][i];
    return '<button class="seg-btn' + (DEM_FILTER === k ? ' active' : '') + '" onclick="demFilter(\'' + k + '\',this)">' + (k === 'all' ? 'Toutes' : x) + '</button>';
  }).join('') + '</div>' + '<div style="flex:1"></div>' + '<span style="font-size:12px;color:var(--neutral-500)">Seuil de passage en cycle : <b style="color:var(--brand-ink)">' + demK(DEM_CFG.seuilCopil) + '</b></span>' + '</div>' + '<div class="table-wrap"><table class="dem-table"><thead><tr>' + '<th>Réf.</th><th>Demande</th><th>Urgence</th><th>Demandeur</th><th>Validateur</th><th class="right">Budget estimé</th><th>Échéance</th><th>Circuit</th><th>Statut</th><th></th>' + '</tr></thead><tbody id="dem-rows"></tbody></table></div>' + '</div>' + '<div class="dem-rules">' + '<div class="card dem-rule"><div class="dem-rule-k">Passage en cycle de pilotage</div><div class="dem-rule-v">À partir de ' + demK(DEM_CFG.seuilCopil) + '</div><div class="dem-rule-m">En dessous du seuil, la demande est validée par le PMO sans arbitrage en comité. Au-delà de ' + demK(DEM_CFG.seuilCodir) + ', l\'arbitrage remonte au CODIR.</div></div>' + '<div class="card dem-rule"><div class="dem-rule-k">Étapes actives</div><div class="dem-rule-v">' + demSteps({
    budget: DEM_CFG.seuilCopil,
    type: 'transformation'
  }).length + ' étapes</div><div class="dem-rule-m">' + demSteps({
    budget: DEM_CFG.seuilCopil,
...
function demRenderRows() {
  ...
  tb.innerHTML = rows.length ? rows.map(function (d) {
    ...
    const route = cyc ? '<span class="dem-route" style="background:var(--brand-gold-050);color:var(--brand-gold-700)">' + demIco('<path d="M3 12a9 9 0 1 0 18 0 9 9 0 0 0-18 0"/><polyline points="12 8 12 12 14 14"/>') + inst + '</span>' : '<span class="dem-route" style="background:var(--neutral-100);color:var(--neutral-600)">' + demIco('<polyline points="20 6 9 17 4 12"/>') + 'Hors cycle</span>';
    return '<tr onclick="demOpen(\'' + d.id + '\')">' + '<td><span class="dem-ref">' + d.id + '</span></td>' + '<td><div style="display:flex;align-items:flex-start;gap:10px"><span class="dem-tyi" style="background:' + ty.bg + ';color:' + ty.fg + '" title="' + ty.l + '">' + demIco(ty.ico) + '</span><span style="min-width:0"><span class="dem-title">' + demEsc(d.t) + '</span><div class="dem-sub">' + demEsc(d.dir) + ' · créée le ' + demDate(d.date) + '</div></span></div></td>' + '<td><span class="badge ' + urg.bdg + '">' + urg.l + '</span></td>' + '<td>' + demPerson(d.who) + '</td>' + '<td>' + (d.val ? demPerson(d.val.split(' — ')[0]) : '<span class="dem-sub">—</span>') + '</td>' + '<td class="right"><span class="dem-num">' + (d.budget ? demK(d.budget) : '<span class="dem-sub">non chiffré</span>') + '</span></td>' + '<td><span class="dem-sub" style="font-size:12.5px;color:var(--neutral-600)">' + demDate(d.deadline) + '</span></td>' + '<td>' + route + (d.seance && d.st === 'cycle' ? '<div class="dem-sub" style="margin-top:4px">' + demEsc(d.seance.replace(/^[A-Z]+ ?[A-Za-z]* — /, '')) + '</div>' : '') + '</td>' + '<td><span class="badge ' + st.bdg + '">' + st.l + '</span>' + (d.st === 'projet' ? '<div class="dem-sub" style="margin-top:4px">' + demEsc(d.proj) + '</div>' : '') + '</td>' + '<td class="dem-chev">' + demIco('<polyline points="9 18 15 12 9 6"/>') + '</td>' + '</tr>';
...
function demOpen(id) {
  ...
  r.innerHTML = '<div style="display:flex;align-items:center;gap:10px;margin-bottom:16px">' + '<button class="btn btn-secondary" onclick="demShowList()">' + demIco('<polyline points="15 18 9 12 15 6"/>') + 'Toutes les demandes</button>' + '<span style="font-size:12.5px;color:var(--neutral-500)">' + d.id + ' · déposée le ' + demDate(d.date) + '</span>' + '</div>' + '<div class="card dem-hero">' + '<div class="dem-hero-ico" style="background:' + ty.bg + ';color:' + ty.fg + '">' + demIco(ty.ico) + '</div>' + '<div class="dem-hero-mid"><h2>' + demEsc(d.t) + '</h2>' + '<div class="dem-hero-meta"><span class="badge ' + st.bdg + '">' + st.l + '</span><span class="dem-route" style="background:' + ty.bg + ';color:' + ty.fg + '">' + ty.l + '</span><span>' + demEsc(d.dir) + '</span><span>·</span>' + demPerson(d.who, 'av-xs') + '<span>·</span><span>Sponsor</span>' + demPerson(d.sponsor.split(' — ')[0], 'av-xs') + '</div></div>' + '<div class="dem-hero-acts">' + demActions(d) + '</div>' + '</div>' + '<div class="dem-flow">' + steps.map(function (s, i) {
    ...
    const num = cls === 'done' ? demIco('<polyline points="20 6 9 17 4 12"/>') : cls === 'ko' ? demIco('<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>') : i + 1;
    return '<div class="dem-step ' + cls + '"><div class="dem-step-n">' + num + '</div><div class="dem-step-l">' + s.l + '</div><div class="dem-step-s">' + s.s + '</div></div>';
  }).join('') + '</div>' + '<div class="card dem-panel" style="margin-bottom:16px"><div class="dem-panel-t" style="margin-bottom:12px">Synthèse pour le validateur</div>' + demSynth(d) + '</div>' + '<div class="dem-grid">' + '<div class="card dem-card">' + '<div class="dem-sec"><div class="dem-sec-t">Besoin / problème à résoudre</div><div class="dem-body">' + demEsc(d.need) + '</div></div>' + (d.outcome ? '<div class="dem-sec"><div class="dem-sec-t">Résultat attendu</div><div class="dem-body">' + demEsc(d.outcome) + '</div></div>' : '') + (d.obj && d.obj.length ? '<div class="dem-sec"><div class="dem-sec-t">Objectifs visés</div><div class="dem-obj">' + d.obj.map(o => '<div class="dem-obj-i">' + demIco('<polyline points="20 6 9 17 4 12"/>') + '<span>' + demEsc(o) + '</span></div>').join('') + '</div></div>' : '') + (d.scope || d.users ? '<div class="dem-sec"><div class="dem-sec-t">Périmètre concerné</div><div class="dem-body muted">' + demEsc(d.scope || '—') + (d.users ? ' <b style="color:var(--brand-ink)">' + d.users.toLocaleString('fr-FR') + ' utilisateurs concernés.</b>' : '') + '</div></div>' : '') + '<div class="dem-sec"><div class="dem-sec-t">Bénéfices attendus</div><div class="dem-body muted">' + demEsc(d.benef) + '</div></div>' + (d.risk ? '<div class="dem-sec"><div class="dem-sec-t">Risques si on ne fait rien</div><div class="dem-body muted">' + demEsc(d.risk) + '</div></div>' : '') + (d.constraints ? '<div class="dem-sec"><div class="dem-sec-t">Contraintes connues</div><div class="dem-body muted">' + demEsc(d.constraints) + '</div></div>' : '') + (d.solutions ? '<div class="dem-sec"><div class="dem-sec-t">Solutions déjà envisagées</div><div class="dem-body muted">' + demEsc(d.solutions) + '</div></div>' : '') + demSwotView(d) + '<div class="dem-sec"><div class="dem-sec-t">Cadrage estimatif</div><div class="dem-kv">' + '<div class="dem-kv-i"><div class="dem-kv-k">Budget estimé</div><div class="dem-kv-v">' + (d.budget ? fmtEur(d.budget) : 'Non chiffré') + '</div></div>' + '<div class="dem-kv-i"><div class="dem-kv-k">Charge interne</div><div class="dem-kv-v">' + (d.charge ? d.charge + ' J/H' : 'Non chiffrée') + '</div></div>' + '<div class="dem-kv-i"><div class="dem-kv-k">Échéance souhaitée</div><div class="dem-kv-v" style="font-size:14px">' + demDate(d.deadline) + '</div>' + (d.deadlineWhy ? '<div class="dem-syn-m">' + demEsc(d.deadlineWhy) + '</div>' : '') + '</div>' + '<div class="dem-kv-i"><div class="dem-kv-k">Urgence</div><div class="dem-kv-v" style="font-size:14px">' + (DEM_URG[d.prio] || DEM_URG.moyenne).l + '</div><div class="dem-syn-m">' + (DEM_URG[d.prio] || DEM_URG.moyenne).m + '</div></div>' + '</div></div>' + (d.docs && d.docs.length ? '<div class="dem-sec"><div class="dem-sec-t">Pièces jointes</div><div class="dem-files">' + d.docs.map(function (f) {
    return '<div class="dem-file">' + demIco('<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>') + '<span class="dem-file-n">' + demEsc(f.n) + '</span><span class="dem-file-s">' + demEsc(f.s) + '</span></div>';
  }).join('') + '</div></div>' : '') + '</div>' + '<div>' + '<div class="card dem-panel">' + demDecisionPanel(d) + '</div>' + '<div class="card dem-panel"><div class="dem-panel-t">Journal de la demande</div><div class="dem-panel-s">Toutes les étapes sont horodatées et reprises dans le dossier d\'arbitrage.</div>' + '<div class="dem-jrn">' + d.jrn.slice().reverse().map(function (j, i) {
    return '<div class="dem-jrn-i"><div class="dem-jrn-d"' + (i === 0 ? ' style="background:var(--brand-gold)"' : '') + '></div><div class="dem-jrn-b"><div class="dem-jrn-t">' + demEsc(j[0]) + '</div><div class="dem-jrn-m" style="display:flex;align-items:center;gap:6px">' + demAuthor(j[1], 'av-xs') + '<span>' + demEsc(j[1]) + ' · ' + demEsc(j[2]) + '</span></div></div></div>';
  }).join('') + '</div>' + '</div>' + '</div>' + '</div>' + demModals();
}
...
function demSynth(d) {
  const urg = DEM_URG[d.prio] || DEM_URG.moyenne,
    obj = demObjLabel(d.objective);
  const i = [['Urgence', '<span class="badge ' + urg.bdg + '">' + urg.l + '</span>', urg.m, true], ['Budget estimé', d.budget ? demK(d.budget) : 'Non chiffré', d.budget ? fmtEur(d.budget) : 'À préciser à l\'instruction'], ['Charge interne', d.charge ? d.charge + ' J/H' : 'Non chiffrée', 'Estimation du demandeur'], ['Échéance souhaitée', demDate(d.deadline), d.deadlineWhy ? demEsc(d.deadlineWhy) : 'Aucune justification'], ['Direction demandeuse', demEsc(d.dir), d.users ? d.users.toLocaleString('fr-FR') + ' utilisateurs concernés' : 'Périmètre non chiffré'], ['Sponsor métier', d.sponsor ? demPerson(d.sponsor.split(' — ')[0], 'av-xs') : '—', d.sponsor ? demEsc(d.sponsor.split(' — ')[1] || '') : 'Non désigné', true], ['Validateur', d.val ? demPerson(d.val.split(' — ')[0], 'av-xs') : '—', d.val ? demEsc(d.val.split(' — ')[1] || '') : 'Défini par le circuit', true], ['Objectif stratégique', obj ? demEsc(obj) : 'Aucun', obj ? 'Objectif rattaché' : 'Non rattachée', !!obj]];
...
function demSwotView(d) {
  const sw = d.swot || {},
    tw = d.tows || {};
  const hasSw = ['s', 'w', 'o', 't'].some(k => sw[k]),
    hasTw = ['so', 'wo', 'st', 'wt'].some(k => tw[k]);
  if (!hasSw && !hasTw) return '';
  const q = function (k, b, t, m) {
    return '<div class="dem-q ' + k + '"><div class="dem-q-h"><span class="dem-q-b">' + b + '</span><span class="dem-q-t">' + t + '</span><span class="dem-q-m">' + m + '</span></div>' + '<div class="dem-q-v' + (sw[k] ? '' : ' empty') + '">' + (sw[k] ? demEsc(sw[k]) : 'Non renseigné') + '</div></div>';
  };
  ...
  return '<div class="dem-sec"><div class="dem-sec-t">Analyse SWOT' + (tows.length ? ' / TOWS' : '') + '</div>' + (hasSw ? '<div class="dem-swot">' + q('s', 'S', 'Forces', 'Atouts internes') + q('w', 'W', 'Faiblesses', 'Limites internes') + q('o', 'O', 'Opportunités', 'Externe favorable') + q('t', 'T', 'Menaces', 'Externe défavorable') + '</div>' : '') + (tows.length ? '<div class="dem-tows">' + tows.map(function (x) {
...
function demSecHead(n, t, m) {
  return '<div class="dem-sh"><div class="dem-sh-n">' + n + '</div><div><div class="dem-sh-t">' + t + '</div><div class="dem-sh-m">' + m + '</div></div></div>';
}
function demOptTag() {
  return '<span class="dem-opt">facultatif</span>';
}
function demQ(k, b, t, m, val, ph) {
  return '<div class="dem-q ' + k + '"><div class="dem-q-h"><span class="dem-q-b">' + b + '</span><span class="dem-q-t">' + t + '</span><span class="dem-q-m">' + m + '</span></div>' + '<textarea class="textarea" id="df-sw-' + k + '" rows="3" placeholder="' + ph + '">' + demEsc(val || '') + '</textarea></div>';
}
function demTw(k, b, t, val, ph) {
  return '<div class="dem-tw"><div class="dem-tw-k">' + b + '</div><div class="dem-tw-t">' + t + '</div>' + '<textarea class="textarea" id="df-tw-' + k + '" rows="2" placeholder="' + ph + '">' + demEsc(val || '') + '</textarea></div>';
}
function demSelect(id, opts, val, ph, oninput) {
  return '<select class="input" id="' + id + '"' + (oninput ? ' onchange="' + oninput + '"' : '') + '>' + '<option value=""' + (val ? '' : ' selected') + '>' + ph + '</option>' + opts.map(function (o) {
    const v = typeof o === 'string' ? o : o.id,
      l = typeof o === 'string' ? o : o.l;
    return '<option value="' + demEsc(v) + '"' + (val === v ? ' selected' : '') + '>' + demEsc(l) + '</option>';
  }).join('') + '</select>';
}
function demForm(d) {
  DEM_FORM = d ? d.id : null;
  ...
  r.innerHTML = '<div class="pg-head" style="margin-bottom:20px"><div><h1>' + (d ? 'Modifier la demande' : 'Nouvelle demande de projet') + '</h1><p class="np-sub">' + (d ? demEsc(d.id) + ' · les informations obligatoires sont demandées à la soumission, pas à l\'enregistrement du brouillon.' : 'Décrivez le besoin en langage métier. Le circuit de validation est déduit du type et du budget estimé — vous n\'avez pas à le choisir.') + '</p></div>' + '<div class="head-actions"><button class="btn btn-secondary" onclick="' + (d ? 'demOpen(\'' + d.id + '\')' : 'demShowList()') + '">' + demIco('<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>') + 'Annuler</button></div></div>' + '<div class="dem-form-grid">' + '<div class="card dem-card">'

  /* ── 1. Votre demande ── */ + '<div class="dem-sec" style="margin-top:0">' + demSecHead(1, 'Votre demande', 'Ce que vous constatez et ce que vous attendez, sans vocabulaire projet.') + '<div class="field"><label class="field-label">Titre de la demande <span class="req">*</span></label><input class="input" id="df-t" value="' + demEsc(v.t) + '" oninput="demFormSync()" placeholder="Ex. : Portail fournisseurs self-care"></div>' + '<div class="field"><label class="field-label">Catégorie de la demande <span class="req">*</span></label>' + '<select class="input" id="df-type" onchange="demFormSync()">' + Object.keys(DEM_TYPES).map(function (k) {
    const t = DEM_TYPES[k];
    return '<option value="' + k + '"' + (v.type === k ? ' selected' : '') + '>' + t.l + ' — ' + t.m + '</option>';
  }).join('') + '</select>' + '<div class="dem-help">La catégorie détermine les exemptions de passage en comité.</div></div>' + '<div class="field"><label class="field-label">Besoin / problème à résoudre <span class="req">*</span></label><textarea class="textarea" id="df-need" rows="4" placeholder="Décrivez la situation actuelle, ce qui ne fonctionne pas et depuis quand.">' + demEsc(v.need) + '</textarea><div class="dem-help">Ce que vous vivez aujourd\'hui : le constat, ses impacts concrets, son origine.</div></div>' + '<div class="field"><label class="field-label">Résultat attendu <span class="req">*</span></label><textarea class="textarea" id="df-outcome" rows="3" placeholder="Ce qui doit être obtenu concrètement si la demande est réalisée.">' + demEsc(v.outcome) + '</textarea><div class="dem-help">Formulez-le comme une situation atteinte, pas comme une solution technique.</div></div>' + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">' + '<div class="field"><label class="field-label">Direction / service demandeur <span class="req">*</span></label>' + demSelect('df-org', DEM_ORG, v.org || v.dir, 'Sélectionner une direction', 'demFormSync()') + '<div class="dem-help">Prérempli d\'après votre rattachement.</div></div>' + '<div class="field"><label class="field-label">Nombre d\'utilisateurs concernés' + demOptTag() + '</label><input class="input" id="df-users" type="number" min="0" value="' + (v.users == null ? '' : v.users) + '" placeholder="Ex. : 120"></div>' + '</div>' + '<div class="field"><label class="field-label">Périmètre concerné' + demOptTag() + '</label><textarea class="textarea" id="df-scope" rows="2" placeholder="Service, sites, utilisateurs, processus concernés.">' + demEsc(v.scope) + '</textarea></div>' + '</div>'

  /* ── 2. Pourquoi maintenant ? ── */ + '<div class="dem-sec">' + demSecHead(2, 'Pourquoi maintenant ?', 'Ce que la demande apporte, et ce qu\'il en coûte de ne rien faire.') + '<div class="field"><label class="field-label">Bénéfices attendus <span class="req">*</span></label><textarea class="textarea" id="df-benef" rows="3" placeholder="Ce que l\'organisation y gagne, si possible chiffré.">' + demEsc(v.benef) + '</textarea><div class="dem-help">Gain de temps, réduction de coûts, chiffre d\'affaires, qualité, conformité, sécurité…</div></div>' + '<div class="field"><label class="field-label">Risques si on ne fait rien <span class="req">*</span></label><textarea class="textarea" id="df-risk" rows="3" placeholder="Ce qui se passe si la demande n\'est pas retenue.">' + demEsc(v.risk) + '</textarea><div class="dem-help">Impact métier, financier, réglementaire, sécurité, continuité d\'activité…</div></div>' + '<label class="field-label" style="display:block;margin-bottom:10px">Urgence <span class="req">*</span></label>' + '<div class="type-pills" id="df-prio" style="margin-bottom:18px">' + Object.keys(DEM_URG).map(function (k) {
    return '<button class="type-pill' + (v.prio === k ? ' sel' : '') + '" data-k="' + k + '" onclick="demPick(this)">' + DEM_URG[k].l + '</button>';
  }).join('') + '</div>' + '<div style="display:grid;grid-template-columns:1fr 1.4fr;gap:14px">' + '<div class="field"><label class="field-label">Échéance souhaitée' + demOptTag() + '</label><input class="input" id="df-deadline" type="date" value="' + (v.deadline || '') + '"></div>' + '<div class="field"><label class="field-label">Pourquoi cette échéance ?' + demOptTag() + '</label><input class="input" id="df-deadlineWhy" value="' + demEsc(v.deadlineWhy) + '" placeholder="Contrainte réglementaire, saisonnalité, fin de support…"></div>' + '</div>' + '</div>'

  /* ── 3. Première estimation ── */ + '<div class="dem-sec">' + demSecHead(3, 'Première estimation', 'Un ordre de grandeur suffit. Le chiffrage sera repris à l\'instruction.') + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">' + '<div class="field"><label class="field-label">Budget estimé (€)' + demOptTag() + '</label><input class="input" id="df-budget" type="number" step="5000" value="' + (v.budget || '') + '" oninput="demFormSync()" placeholder="120000"' + (v.budget ? '' : '') + '><label class="dem-chk"><input type="checkbox" id="df-budget-x"' + (d && !v.budget ? ' checked' : '') + ' onchange="demFormUnknown(\'budget\')"> Budget inconnu à ce stade</label></div>' + '<div class="field"><label class="field-label">Charge interne estimée (J/H)' + demOptTag() + '</label><input class="input" id="df-charge" type="number" step="0.5" value="' + (v.charge || '') + '" placeholder="120"><label class="dem-chk"><input type="checkbox" id="df-charge-x" onchange="demFormUnknown(\'charge\')"> Charge inconnue à ce stade</label></div>' + '</div>' + '<div class="field"><label class="field-label">Contraintes connues' + demOptTag() + '</label><textarea class="textarea" id="df-constraints" rows="2" placeholder="Sécurité, réglementaire, contractuel, technique, calendrier, organisation.">' + demEsc(v.constraints) + '</textarea></div>' + '<div class="field"><label class="field-label">Solutions déjà envisagées' + demOptTag() + '</label><textarea class="textarea" id="df-solutions" rows="2" placeholder="Ce qui a déjà été tenté ou étudié, et pourquoi cela n\'a pas suffi.">' + demEsc(v.solutions) + '</textarea></div>' + '<label class="field-label" style="display:block;margin-bottom:8px">Pièces jointes' + demOptTag() + '</label>' + '<label class="dem-drop">' + demIco('<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/>') + '<div class="dem-drop-t">Déposer un document ou parcourir</div><div class="dem-drop-m">Cartographie de processus, chiffrage fournisseur, note de cadrage… PDF, Office, images.</div>' + '<input type="file" multiple onchange="demAddDocs(this)"></label>' + '<div class="dem-files" id="df-docs"></div>' + '</div>'

  /* ── 4. Analyse SWOT / TOWS ── */ + '<div class="dem-sec">' + demSecHead(4, 'Analyse SWOT / TOWS', 'Facultatif, mais très utile en comité : ce qui joue pour la demande, ce qui joue contre, et ce qu\'on en déduit.') + '<div class="dem-swot">' + demQ('s', 'S', 'Forces', 'Atouts internes', v.swot.s, 'Équipe mobilisée, processus connu, brique technique déjà en place…') + demQ('w', 'W', 'Faiblesses', 'Limites internes', v.swot.w, 'Compétence manquante, dette technique, disponibilité des équipes…') + demQ('o', 'O', 'Opportunités', 'Facteurs externes favorables', v.swot.o, 'Fenêtre réglementaire, socle mutualisé, financement disponible…') + demQ('t', 'T', 'Menaces', 'Facteurs externes défavorables', v.swot.t, 'Échéance contrainte, dépendance fournisseur, résistance au changement…') + '</div>' + '<div class="dem-help" style="margin-top:12px">Croisé TOWS — ce que l\'on fait de cette analyse. Une ligne par case suffit.</div>' + '<div class="dem-tows">' + demTw('so', 'SO', 'Forces × Opportunités · attaquer', v.tows.so, 'Sur quoi s\'appuyer pour aller vite') + demTw('wo', 'WO', 'Faiblesses × Opportunités · renforcer', v.tows.wo, 'Quelle faiblesse combler grâce à une opportunité') + demTw('st', 'ST', 'Forces × Menaces · défendre', v.tows.st, 'Comment neutraliser une menace avec un atout') + demTw('wt', 'WT', 'Faiblesses × Menaces · éviter', v.tows.wt, 'Ce que l\'on retire du périmètre pour limiter le risque') + '</div>' + '</div>'

  /* ── 5. Gouvernance ── */ + '<div class="dem-sec">' + demSecHead(5, 'Gouvernance', 'Qui porte la demande et à quel objectif elle contribue.') + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">' + '<div class="field"><label class="field-label">Demandeur</label><div class="dem-ro">' + demPerson(v.who || DEM_ME, 'av-sm') + '<span class="dem-ro-m">' + (v.who && v.who !== DEM_ME ? 'Demandeur' : 'Vous') + ' · lecture seule</span></div></div>' + '<div class="field"><label class="field-label">Sponsor métier' + demOptTag() + '</label>' + demSelect('df-sponsor', people, v.sponsor, 'Sélectionner un sponsor') + '</div>' + '</div>' + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">' + '<div class="field"><label class="field-label">Objectif stratégique' + demOptTag() + '</label>' + demSelect('df-objective', DEM_OBJ, v.objective, 'Aucun objectif rattaché') + '<div class="dem-help">Rattacher la demande à un objectif facilite son arbitrage.</div></div>' + '<div class="field"><label class="field-label">Validateur</label>' + demSelect('df-val', people, v.val, 'Défini par le circuit de validation') + '<div class="dem-help">Proposé par le circuit ; seul le validateur désigné pourra décider.</div></div>' + '</div>' + '</div>' + '<div id="df-errs"></div>' + '<div style="display:flex;justify-content:flex-end;gap:12px;margin-top:22px">' + '<button class="btn btn-secondary" onclick="demFormSave(false)">' + demIco('<path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/>') + 'Enregistrer en brouillon</button>' + '<button class="btn btn-primary" onclick="demFormSave(true)">' + demIco('<line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/>') + 'Soumettre la demande</button>' + '</div>' + '</div>' + '<div class="card dem-prev" id="df-prev"></div>' + '</div>' + demModals();
  demRenderDocs();
  demFormSync();
}
function demFormType(el) {
...
function demFormUnknown(k) {
  const cb = document.getElementById('df-' + k + '-x'),
    inp = document.getElementById('df-' + k);
  if (cb.checked) {
    inp.value = '';
    inp.disabled = true;
  } else {
...
function demAddDocs(inp) {
  Array.prototype.slice.call(inp.files || []).forEach(function (f) {
    DEM_DOCS.push({
      n: f.name,
      s: f.size > 1048576 ? (f.size / 1048576).toFixed(1) + ' Mo' : Math.max(1, Math.round(f.size / 1024)) + ' Ko'
    });
  });
  inp.value = '';
  demRenderDocs();
}
function demDelDoc(i) {
  DEM_DOCS.splice(i, 1);
  demRenderDocs();
}
function demRenderDocs() {
  const el = document.getElementById('df-docs');
  if (!el) return;
  el.innerHTML = DEM_DOCS.map(function (f, i) {
    return '<div class="dem-file">' + demIco('<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>') + '<span class="dem-file-n">' + demEsc(f.n) + '</span><span class="dem-file-s">' + demEsc(f.s) + '</span>' + '<span class="dem-file-x" onclick="demDelDoc(' + i + ')">' + demIco('<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>') + '</span></div>';
  }).join('');
}
function demVal(id) {
...
function demMissing(f) {
  return DEM_REQ.filter(function (r) {
    const x = f[r[0]];
...
function demFormSync() {
  ...
  const miss = demMissing(f),
    unknownBudget = !f.budget;
  p.innerHTML = '<div class="dem-prev-k">Circuit qui sera appliqué</div>' + '<div class="dem-jrn">' + steps.map(function (s, i) {
    return '<div class="dem-jrn-i"><div class="dem-jrn-d" style="background:' + (i === 0 ? 'var(--brand-gold)' : 'var(--neutral-300)') + '"></div><div class="dem-jrn-b"><div class="dem-jrn-t">' + s.l + '</div><div class="dem-jrn-m">' + s.s + '</div></div></div>';
  }).join('') + '</div>' + '<div class="dem-prev-row"><span class="k">Catégorie</span><span class="v">' + DEM_TYPES[f.type].l + '</span></div>' + '<div class="dem-prev-row"><span class="k">Direction</span><span class="v">' + (f.org ? demEsc(f.org) : '—') + '</span></div>' + '<div class="dem-prev-row"><span class="k">Budget estimé</span><span class="v">' + (f.budget ? demK(f.budget) : 'Non renseigné') + '</span></div>' + '<div class="dem-prev-row"><span class="k">Cycle de pilotage</span><span class="v" style="color:' + (cyc ? 'var(--brand-gold-700)' : 'var(--neutral-600)') + '">' + (cyc ? 'Requis · ' + inst : 'Non requis') + '</span></div>' + '<div class="dem-note">' + demIco('<circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/>') + '<span>' + (unknownBudget ? 'Sans budget renseigné, le routage sera déterminé à l\'instruction par le PMO.' : cyc ? 'Au-delà de ' + demK(DEM_CFG.seuilCopil) + ', la demande est arbitrée en ' + inst + ' avant création du projet.' : 'En dessous du seuil' + (DEM_CFG.exempt.indexOf(f.type) >= 0 ? ' ou pour un type exempté' : '') + ', le PMO valide sans passer en comité.') + '</span></div>' + '<div class="dem-prev-k" style="margin-top:18px">Prêt à soumettre</div>' + (miss.length ? '<div class="dem-jrn">' + miss.map(function (m) {
    return '<div class="dem-jrn-i" style="padding-bottom:9px"><div class="dem-jrn-d" style="background:var(--state-warning);box-shadow:none"></div><div class="dem-jrn-b"><div class="dem-jrn-m" style="margin-top:1px">' + m[1] + '</div></div></div>';
  }).join('') + '</div>' + '<div class="dem-note"><span>' + miss.length + ' information' + (miss.length > 1 ? 's' : '') + ' à compléter avant soumission. L\'enregistrement en brouillon reste possible.</span></div>' : '<div class="dem-note" style="color:var(--state-success)">' + demIco('<polyline points="20 6 9 17 4 12"/>') + '<span style="color:var(--neutral-600)">Toutes les informations obligatoires sont renseignées.</span></div>');
}
function demFormSave(submit) {
  const f = demFormRead();
  const ids = {
  ...
  const errBox = document.getElementById('df-errs');
  if (errBox) errBox.innerHTML = '';
  /* brouillon : seuls le titre et la direction sont nécessaires pour identifier la demande */
  if (!f.t) {
    document.getElementById('df-t').classList.add('err');
    showToast('Donnez un titre à la demande');
    return;
  }
  if (!f.org) {
    document.getElementById('df-org').classList.add('err');
    showToast('Indiquez la direction demandeuse');
    return;
  }
  if (submit) {
    ...
    if (miss.length) {
      miss.forEach(function (m) {
        const el = document.getElementById(ids[m[0]]);
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "ui_kits/app/modules/demandes.js", error: String((e && e.message) || e) }); }

// ui_kits/app/modules/docs.js
try { (() => {
...
let DOCS = [{
  id: 1,
  name: 'Cahier des charges v3.pdf',
  k: 'pdf',
  d: '2025-04-28T14:32',
  by: 'MD',
  size: 2516582
  ...
  id: 4,
  name: 'Maquette espace client.fig',
  k: 'fig',
  d: '2025-04-16T11:47',
  by: 'AB',
  size: 12897484
...
let DOC_ACTS = [{
  ...
  who: 'Marc D.',
  verb: 'a ajouté',
  f: 'Cahier des charges v3.pdf',
  t: 'Il y a 1 h'
}, {
  k: 'xlsx',
  who: 'Sophie L.',
  verb: 'a mis à jour',
  f: 'Planning détaillé.xlsx',
  t: 'Il y a 4 h'
}, {
  k: 'docx',
  who: 'Julien T.',
  verb: 'a ajouté',
  f: 'Compte-rendu COPIL.docx',
  t: 'Hier à 16:05'
}, {
  k: 'fig',
  who: 'Alice B.',
  verb: 'a ajouté',
  f: 'Maquette espace client.fig',
  t: '16 avr. à 11:47'
}, {
  k: 'folder',
  who: 'Marc D.',
  verb: 'a créé le dossier',
  f: 'Documents projet',
  t: '15 avr. à 10:12'
}];
let DOC_Q = '',
  ...
  DOC_MENU = null;
function docSize(b) {
  return b >= 1048576 ? (b / 1048576).toFixed(1).replace('.', ',') + ' Mo' : Math.round(b / 1024) + ' Ko';
}
function docDate(iso) {
  const d = new Date(iso);
  const m = d.toLocaleDateString('fr-FR', {
    month: 'short'
  }).replace('.', '');
  return d.getDate() + ' ' + m + '. ' + d.getFullYear() + ' ' + String(d.getHours()).padStart(2, '0') + ':' + String(d.getMinutes()).padStart(2, '0');
}
function docAvCls(ini) {
  return typeof DIR !== 'undefined' && DIR[ini] && DIR[ini][3] || 'av-1';
}
function docWho(ini) {
  return typeof dirName === 'function' ? dirName(ini) : ini;
}
function docKindFromName(n) {
...
function docRows() {
  ...
  const cmp = {
    ...
    date: (a, b) => a.d < b.d ? -1 : a.d > b.d ? 1 : 0,
...
function docRender() {
  ...
  tb.innerHTML = rows.length ? rows.map(d => {
    const K = DOC_KINDS[d.k];
    return '<tr>' + '<td><div class="tname"><div class="doc-ico ' + K.ico + '">' + K.ext + '</div><span class="cell-strong">' + d.name + '</span></div></td>' + '<td><span class="badge ' + (K.badge || '') + ' nodot"' + (K.style ? ' style="' + K.style + '"' : '') + '>' + K.lbl + '</span></td>' + '<td style="font-variant-numeric:tabular-nums;color:var(--neutral-600)">' + docDate(d.d) + '</td>' + '<td><div class="assignee"><div class="av ' + docAvCls(d.by) + '">' + d.by + '</div><span>' + docWho(d.by) + '</span></div></td>' + '<td style="font-variant-numeric:tabular-nums">' + docSize(d.size) + '</td>' + '<td class="right"><div class="doc-act"><button class="dots-btn" onclick="docMenu(event,' + d.id + ')"><svg viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg></button></div></td>' + '</tr>';
  }).join('') : '<tr><td colspan="6"><div class="doc-empty">Aucun document ne correspond' + (DOC_Q ? ' à « ' + DOC_Q + ' »' : '') + (DOC_TYPE ? ' pour le type ' + DOC_KINDS[DOC_TYPE].lbl : '') + '.<button class="ov-btn" onclick="docReset()">Réinitialiser les filtres</button></div></td></tr>';
  ...
  const tl = document.getElementById('doc-type-lbl');
  if (tl) tl.textContent = DOC_TYPE ? DOC_KINDS[DOC_TYPE].lbl : 'Type';
  const sl = document.getElementById('doc-sort-lbl');
  if (sl) sl.textContent = {
    name: 'Nom',
    date: 'Modifié le',
    size: 'Taille'
  }[DOC_SORT] + (DOC_DIR < 0 ? ' (récent)' : ' (croissant)').replace('(récent)', DOC_SORT === 'date' ? '(récent)' : '(décroissant)');
  const act = document.getElementById('doc-acts');
  if (act) act.innerHTML = DOC_ACTS.map(a => {
    const K = DOC_KINDS[a.k];
    ...
    return '<div class="act-item">' + ico + '<div class="act-body"><div class="act-line1"><b>' + a.who + '</b> ' + a.verb + '</div><div class="act-file">' + a.f + '</div><div class="act-time">' + a.t + '</div></div><div class="act-dot" style="background:' + (K ? K.dot : 'var(--brand-gold)') + '"></div></div>';
  }).join('');
}
function docSearch(v) {
  DOC_Q = v;
  docRender();
}
function docReset() {
...
function docSetType(k) {
  DOC_TYPE = k;
  docClosePop();
  docRender();
}
function docSetSort(k) {
  if (DOC_SORT === k) DOC_DIR *= -1;else {
    DOC_SORT = k;
    DOC_DIR = k === 'name' ? 1 : -1;
  }
  docClosePop();
...
function docView(v, el) {
  DOC_VIEW = v;
  document.querySelectorAll('#doc-seg .seg-btn').forEach(b => b.classList.toggle('active', b === el));
  const g = document.getElementById('doc-grid');
  if (g) g.classList.toggle('doc-actonly', v === 'acts');
}

/* popovers légers (type, tri, actions de ligne) */
function docClosePop() {
  document.querySelectorAll('.doc-pop').forEach(p => p.remove());
  DOC_MENU = null;
}
function docPop(ev, items) {
  ...
  const open = DOC_MENU;
  docClosePop();
  if (open === ev.currentTarget) return;
...
function docTypePop(ev) {
  docPop(ev, [{
    t: 'Tous les types',
    act: "docSetType('')",
    on: !DOC_TYPE
  }].concat(Object.keys(DOC_KINDS).map(k => ({
    t: DOC_KINDS[k].lbl,
    act: "docSetType('" + k + "')",
    on: DOC_TYPE === k
  }))));
}
function docSortPop(ev) {
  docPop(ev, [{
    t: 'Nom',
    act: "docSetSort('name')",
    on: DOC_SORT === 'name'
...
function docOpen(id) {
  const d = docById(id);
  docClosePop();
  showToast('Ouverture de ' + d.name);
}
function docDownload(id) {
  const d = docById(id);
  docClosePop();
  showToast(d.name + ' · téléchargement lancé (' + docSize(d.size) + ')');
}
function docRename(id) {
  const d = docById(id);
  docClosePop();
  const n = prompt('Nouveau nom du document', d.name);
  if (!n || !n.trim() || n === d.name) return;
  const old = d.name;
  d.name = n.trim();
  d.k = docKindFromName(d.name);
  d.d = new Date().toISOString().slice(0, 16);
  DOC_ACTS.unshift({
    k: d.k,
    who: docWho('MD'),
    verb: 'a renommé',
  ...
  docRender();
  showToast('Document renommé · ' + d.name);
}
function docDelete(id) {
  const d = docById(id);
  docClosePop();
  if (!confirm('Supprimer « ' + d.name + ' » ? Cette action retire le document du projet.')) return;
...
function docAddSubmit() {
  ...
  docAddClose();
  docRender();
  showToast(name + ' ajouté au projet');
}
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', docRender);else docRender();
})(); } catch (e) { __ds_ns.__errors.push({ path: "ui_kits/app/modules/docs.js", error: String((e && e.message) || e) }); }
...
// ui_kits/app/modules/strategie-consol.js
try { (() => {
...
function stgCell(v) {
  const h = stgHeat(v);
  return '<td><div class="stg-heat' + h.cls + '"' + (h.bg ? ' style="background:' + h.bg + ';color:' + h.c + '"' : '') + '>' + h.t + '</div></td>';
}
...
function stgTok(s) {
  return s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9 ]/g, ' ').split(/\s+/).filter(w => w.length > 3 && !STG_STOP.includes(w));
}
function stgDups() {
  const all = [];
  STG_DIRS.forEach(d => d.chantiers.forEach(c => all.push({
    d,
    c,
    tk: stgTok(c.n)
  })));
...
function stgRenderConsol() {
  const host = document.getElementById('vs-consol');
  if (!host) return;
  const ax = stgGAxes();
  const chAll = [];
  STG_DIRS.forEach(d => d.chantiers.forEach(c => chAll.push({
    d,
    c
  })));
  ...
  host.innerHTML = '<div class="stg-kpis">' + '<div class="stg-kpi"><div class="l">Directions</div><div class="v">' + STG_DIRS.length + '</div><div class="d">' + STG_DIRS.filter(d => d.status === 'valide').length + ' schéma(s) validé(s)</div></div>' + '<div class="stg-kpi"><div class="l">Chantiers consolidés</div><div class="v">' + chAll.length + '</div><div class="d">' + chAll.filter(x => x.c.pct > 0 && x.c.pct < 100).length + ' en cours</div></div>' + '<div class="stg-kpi"><div class="l">Budget horizon ' + STG_Y0 + '–' + (STG_Y0 + STG_NY - 1) + '</div><div class="v">' + stgEur(budTot) + '</div><div class="d">toutes directions</div></div>' + '<div class="stg-kpi"><div class="l">Alignement moyen</div><div class="v">' + scAvg + '%</div><div class="d">' + (dups.length ? dups.length + ' recouvrement(s) détecté(s)' : 'aucun recouvrement') + '</div></div>' + '</div>'
  /* matrice */ + '<div class="stg-sec"><div class="stg-sec-head"><div><div class="stg-sec-t">Matrice directions × axes du groupe</div>' + '<div class="stg-sec-sub">Contribution déclarée de chaque direction aux axes de la vision groupe. Le nombre de chantiers rattachés est indiqué sous chaque valeur.</div></div>' + '<button class="btn btn-secondary" onclick="stgOpenCompare()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round"><line x1="12" y1="3" x2="12" y2="21"/><polyline points="7 8 4 11 7 14"/><polyline points="17 8 20 11 17 14"/></svg>Comparer deux directions</button></div>' + '<div class="card card-pad"><div class="table-wrap"><table class="stg-mx"><thead><tr><th class="rowh">Direction</th>' + ax.map(a => '<th>' + stgEsc(a.name) + '</th>').join('') + '<th>Score</th></tr></thead><tbody>' + STG_DIRS.map(d => {
    const T = stgTone(d.tone);
    return '<tr><td class="rowh"><span class="stg-chip" style="background:' + T.bg + ';color:' + T.c + ';margin-right:7px">' + stgEsc(d.sigle) + '</span>' + stgEsc(d.name) + '<div class="s">' + stgEsc(d.dir) + '</div></td>' + ax.map(a => {
      const v = d.align[a.id] || 0,
        n = d.chantiers.filter(c => (c.ga || []).includes(a.id)).length,
        h = stgHeat(v);
      return '<td><div class="stg-heat' + h.cls + '"' + (h.bg ? ' style="background:' + h.bg + ';color:' + h.c + '"' : '') + ' title="' + n + ' chantier(s) rattaché(s)">' + h.t + '</div>' + '<div style="text-align:center;font-size:10.5px;font-weight:700;color:var(--neutral-400);margin-top:2px">' + (n ? n + ' ch.' : '—') + '</div></td>';
    }).join('') + stgCell(stgScore(d)) + '</tr>';
  }).join('') + '</tbody></table></div></div></div>'
  /* heatmap maturité */ + '<div class="stg-sec"><div><div class="stg-sec-t">Maturité des schémas directeurs</div>' + '<div class="stg-sec-sub" style="margin-bottom:12px">Complétude calculée sur six dimensions : ambition formulée, objectifs mesurables, chantiers planifiés, budget couvert, risques déclarés, fraîcheur de la dernière revue.</div></div>' + '<div class="card card-pad"><div class="table-wrap"><table class="stg-mx"><thead><tr><th class="rowh">Direction</th>' + dims.map(x => '<th>' + x + '</th>').join('') + '</tr></thead><tbody>' + STG_DIRS.map(d => {
    const m = stgMaturity(d),
      T = stgTone(d.tone);
    return '<tr><td class="rowh"><span class="stg-chip" style="background:' + T.bg + ';color:' + T.c + ';margin-right:7px">' + stgEsc(d.sigle) + '</span>' + stgEsc(d.name) + '</td>' + dims.map(x => stgCell(m[x])).join('') + '</tr>';
  }).join('') + '</tbody></table></div></div></div>'
  /* timeline groupe */ + '<div class="stg-sec"><div><div class="stg-sec-t">Timeline groupe — tous les schémas directeurs superposés</div>' + '<div class="stg-sec-sub" style="margin-bottom:12px">Une ligne par chantier, regroupée par direction. Permet de repérer les pics de charge simultanés entre directions.</div></div>' + '<div class="card stg-tl">' + '<div class="stg-tl-row" style="--stg-ny:' + STG_NY + '"><div></div><div class="stg-tl-years" style="--stg-ny:' + STG_NY + '">' + Array.from({
    length: STG_NY
  }, (_, i) => '<div class="stg-tl-year">' + (STG_Y0 + i) + '</div>').join('') + '</div></div>' + STG_DIRS.map(d => {
    const T = stgTone(d.tone);
    return '<div class="stg-tl-lane-h"><i style="background:' + T.c + '"></i>' + stgEsc(d.sigle) + ' — ' + stgEsc(d.name) + '</div>' + d.chantiers.slice().sort((a, b) => a.s - b.s).map(c => '<div class="stg-tl-row stg-tl-lane"><div><div class="stg-tl-name" onclick="stgOpenDir(\'' + d.id + '\')">' + stgEsc(c.n) + '</div><div class="stg-tl-nsub">' + stgEsc(c.own) + ' · ' + stgEur(c.bud) + '</div></div>' + '<div class="stg-tl-track" style="--stg-nq:12">' + '<div class="stg-tl-bar" style="left:' + pc(c.s).toFixed(2) + '%;width:' + pc(Math.max(1, c.e - c.s)).toFixed(2) + '%;background:' + T.c + '" onclick="stgOpenDir(\'' + d.id + '\')" title="' + stgEsc(d.sigle) + ' — ' + stgEsc(c.n) + ' · ' + stgQlbl(c.s) + ' → ' + stgQlbl(c.e) + '"><i style="width:' + c.pct + '%"></i><span>' + c.pct + '%</span></div>' + (c.ms || []).map(m => '<div class="stg-tl-ms" style="left:' + pc(m.m).toFixed(2) + '%" title="' + stgEsc(m.l) + ' — ' + stgQlbl(m.m) + '"></div>').join('') + '<div class="stg-tl-now" style="left:' + pc(STG_NOW).toFixed(2) + '%"></div></div></div>').join('');
  }).join('') + '<div class="stg-tl-legend">' + STG_DIRS.map(d => '<span class="k"><i style="background:' + stgTone(d.tone).c + '"></i>' + stgEsc(d.sigle) + '</span>').join('') + '<span class="k"><span class="dia"></span>Jalon structurant</span></div></div></div>'
  /* portefeuille consolidé */ + '<div class="stg-sec"><div><div class="stg-sec-t">Portefeuille consolidé de chantiers</div>' + '<div class="stg-sec-sub" style="margin-bottom:12px">' + chAll.length + ' chantiers issus des schémas directeurs, avec leur rattachement aux axes du groupe.</div></div>' + '<div class="card tablecard"><div class="table-wrap"><table class="dt"><thead><tr><th>Chantier</th><th>Direction</th><th>Fenêtre</th><th>Pilote</th><th>Budget</th><th>Axes groupe</th><th class="right">Avancement</th></tr></thead><tbody>' + chAll.sort((a, b) => a.c.s - b.c.s).map(x => {
    const T = stgTone(x.d.tone);
    return '<tr style="cursor:pointer" onclick="stgOpenDir(\'' + x.d.id + '\')"><td class="cell-strong">' + stgEsc(x.c.n) + '</td>' + '<td><span class="stg-chip" style="background:' + T.bg + ';color:' + T.c + '">' + stgEsc(x.d.sigle) + '</span></td>' + '<td style="font-variant-numeric:tabular-nums;color:var(--neutral-600)">' + stgQlbl(x.c.s) + ' → ' + stgQlbl(Math.min(x.c.e, STG_NM - 1)) + '</td>' + '<td>' + stgEsc(x.c.own) + '</td><td style="font-variant-numeric:tabular-nums">' + stgEur(x.c.bud) + '</td>' + '<td><div class="stg-chips">' + ((x.c.ga || []).map(id => {
      const a = ax.find(z => z.id === id);
      return a ? '<span class="stg-chip">' + stgEsc(a.name) + '</span>' : '';
    }).join('') || '<span style="color:var(--neutral-300);font-weight:700">non rattaché</span>') + '</div></td>' + '<td class="right"><div class="prog" style="justify-content:flex-end"><div class="prog-track" style="width:80px"><div class="prog-fill" style="width:' + x.c.pct + '%;background:' + (x.c.pct >= 70 ? 'var(--state-success)' : x.c.pct >= 30 ? 'var(--brand-gold)' : 'var(--state-danger)') + '"></div></div><span class="prog-pct">' + x.c.pct + '%</span></div></td></tr>';
  }).join('') + '</tbody></table></div></div></div>'
  /* doublons */ + '<div class="stg-sec"><div><div class="stg-sec-t"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>Recouvrements entre directions</div>' + '<div class="stg-sec-sub" style="margin-bottom:12px">Chantiers de directions différentes partageant un même objet — à arbitrer en revue croisée avant lancement.</div></div>' + '<div class="stg-dup">' + (dups.length ? dups.map(p => {
    const TA = stgTone(p.a.d.tone),
      TB = stgTone(p.b.d.tone);
    const os = Math.max(p.a.c.s, p.b.c.s),
      oe = Math.min(p.a.c.e, p.b.c.e);
    const ov = Math.max(0, oe - os),
      sev = ov >= 12 ? 'high' : ov >= 6 ? 'mid' : 'low';
    const sevL = {
      high: 'Recouvrement fort',
      mid: 'Recouvrement notable',
      low: 'Recouvrement limité'
    }[sev];
    const bar = (c, col) => '<div class="tr"><i style="left:' + (c.s / STG_NM * 100).toFixed(2) + '%;width:' + (Math.max(1, c.e - c.s) / STG_NM * 100).toFixed(2) + '%;background:' + col + '"></i>' + (ov > 0 ? '<b style="left:' + (os / STG_NM * 100).toFixed(2) + '%;width:' + (ov / STG_NM * 100).toFixed(2) + '%"></b>' : '') + '</div>';
    const side = (x, T, al) => '<div class="sd ' + al + '"><div class="sd-h"><span class="sg" style="background:' + T.bg + ';color:' + T.c + '">' + stgEsc(x.d.sigle) + '</span>' + '<span class="sd-m">' + stgQlbl(x.c.s) + ' → ' + stgQlbl(Math.min(x.c.e, STG_NM - 1)) + '</span></div>' + '<div class="sd-n">' + stgEsc(x.c.n) + '</div>' + '<div class="sd-f"><span>' + stgEsc(x.c.own) + '</span><span class="dot"></span><span>' + stgEur(x.c.bud) + '</span><span class="dot"></span><span>' + x.c.pct + ' %</span></div></div>';
    return '<div class="card stg-dup-c sev-' + sev + '">' + '<div class="stg-dup-top"><span class="sev">' + sevL + '</span>' + '<span class="stg-dup-q">' + (ov > 0 ? 'Chevauchement ' + stgQlbl(os) + ' → ' + stgQlbl(Math.max(os, oe - 1)) : 'Fenêtres disjointes') + '</span>' + '<span class="stg-dup-b">' + stgEur(p.a.c.bud + p.b.c.bud) + ' cumulés</span></div>' + '<div class="stg-dup-body">' + side(p.a, TA, 'l') + '<div class="mid"><i style="background:' + TA.c + '"></i><span class="x"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg></span><i style="background:' + TB.c + '"></i></div>' + side(p.b, TB, 'r') + '</div>' + '<div class="stg-dup-tl"><div class="yl">' + Array.from({
      length: STG_NY
    }, (_, i) => '<span>' + (STG_Y0 + i) + '</span>').join('') + '</div>' + bar(p.a.c, TA.c) + bar(p.b.c, TB.c) + '</div>' + '<div class="stg-dup-foot"><span class="lb">Objets partagés</span><div class="stg-chips">' + p.shared.map(t => '<span class="stg-chip">' + stgEsc(t) + '</span>').join('') + '</div>' + '<button class="btn btn-secondary" onclick="stgToast(\'Recouvrement inscrit à l’ordre du jour de la revue croisée\')">Inscrire en revue croisée</button></div></div>';
  }).join('') : '<div class="card"><div class="stg-empty">Aucun recouvrement détecté entre les schémas directeurs.</div></div>') + '</div></div>';
}
/* ─── Comparateur ─── */
function stgOpenCompare() {
  const sel = (id, i) => {
    const e = document.getElementById(id);
    e.innerHTML = STG_DIRS.map(d => '<option value="' + d.id + '">' + stgEsc(d.sigle) + ' — ' + stgEsc(d.name) + '</option>').join('');
    e.value = (STG_DIRS[i] || STG_DIRS[0] || {}).id;
  };
  sel('scmp-a', 0);
  sel('scmp-b', 1);
  stgRenderCompare();
...
function stgRenderCompare() {
  const host = document.getElementById('scmp-body');
  if (!host) return;
  ...
  host.innerHTML = '<div class="stg-cmp">' + ids.map(id => {
    const d = stgDir(id);
    if (!d) return '<div class="stg-cmp-col"></div>';
    const T = stgTone(d.tone),
      m = stgMaturity(d);
    ...
    return '<div class="stg-cmp-col"><div class="stg-cmp-h" style="background:' + T.bg + '">' + '<div class="stg-sigle" style="width:38px;height:38px;font-size:12px;background:var(--neutral-0);color:' + T.c + '">' + stgEsc(d.sigle) + '</div>' + '<div><div class="n">' + stgEsc(d.name) + '</div><div class="s">' + stgEsc(d.dir) + ' · ' + stgEsc(d.version) + '</div></div></div>' + '<div style="padding:14px 16px;font-size:12.5px;line-height:1.55;color:var(--neutral-700);border-bottom:1px solid var(--neutral-100);min-height:88px">' + d.ambition + '</div>' + row('Score d’alignement', '<span style="color:' + T.c + '">' + stgScore(d) + '%</span>') + row('Statut', (STG_STATUS[d.status] || {
      lbl: '—'
    }).lbl) + row('Horizon', stgEsc(d.horizon)) + row('Effectif', d.fte + ' ETP') + row('Budget de fonctionnement', stgEur(d.budget)) + row('Budget schéma directeur', stgEur(bud)) + row('Axes propres', d.axes.length) + row('Chantiers', d.chantiers.length + ' · ' + chAvg + '% moyen') + row('Objectifs mesurables', d.okr.length) + row('Risques déclarés', d.risks.length) + row('Dernière revue', stgDate(d.review)) + '<div style="padding:12px 16px;border-top:1px solid var(--neutral-100)"><div style="font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--neutral-500);margin-bottom:9px">Contribution aux axes groupe</div>' + ax.map(a => {
      const v = d.align[a.id] || 0;
      return '<div class="stg-contrib-row" style="grid-template-columns:1fr 90px 38px;margin-bottom:7px"><div class="l">' + stgEsc(a.name) + '</div><div class="t" style="height:14px"><i style="width:' + v + '%;background:' + T.c + '"></i></div><div class="p">' + v + '%</div></div>';
    }).join('') + '</div>' + '<div style="padding:12px 16px;border-top:1px solid var(--neutral-100)"><div style="font-size:10.5px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--neutral-500);margin-bottom:9px">Maturité</div>' + '<div class="stg-chips">' + Object.keys(m).map(k => {
      const h = stgHeat(m[k]);
      return '<span class="stg-chip"' + (h.bg ? ' style="background:' + h.bg + ';color:' + h.c + '"' : '') + '>' + k + ' ' + h.t + '</span>';
    }).join('') + '</div></div>' + '</div>';
  }).join('') + '</div>';
}
/* ─── Export PDF 1 page ─── */
function stgExportPdf(id) {
  const d = stgDir(id || STG_CUR);
  if (!d) return;
  const T = stgTone(d.tone),
    ax = stgGAxes(),
    pc = m => m / STG_NM * 100;
  ...
  n.innerHTML = '<div class="stg-p-h"><div style="width:34px;height:34px;border-radius:6px;background:' + T.bg + ';color:' + T.c + ';display:flex;align-items:center;justify-content:center;font-weight:800;font-size:11pt">' + stgEsc(d.sigle) + '</div>' + '<div style="flex:1"><h1>' + stgEsc(d.name) + ' — schéma directeur ' + stgEsc(d.horizon) + '</h1>' + '<div class="s">' + stgEsc(d.dir) + ' · ' + stgEsc(d.version) + ' · ' + (STG_STATUS[d.status] || {
    lbl: ''
  }).lbl + ' · revue du ' + stgDate(d.review) + ' · alignement ' + stgScore(d) + '%</div></div></div>' + '<div class="stg-p-amb">' + d.ambition + '</div>' + '<div class="stg-p-tl">' + d.axes.map((a, i) => {
    const cs = d.chantiers.filter(c => c.lane === i);
    if (!cs.length) return '';
    return '<div class="stg-p-t" style="margin-top:6px">' + stgEsc(a.n) + '</div>' + cs.sort((x, y) => x.s - y.s).map(c => '<div class="r"><span>' + stgEsc(c.n) + '</span><div class="tr"><i style="left:' + pc(c.s).toFixed(1) + '%;width:' + pc(Math.max(1, c.e - c.s)).toFixed(1) + '%;background:' + stgTone(a.t).c + '"></i></div></div>').join('');
  }).join('') + '<div style="display:grid;grid-template-columns:40mm 1fr;gap:6px"><span></span><div style="display:grid;grid-template-columns:repeat(' + STG_NY + ',1fr);font-size:7pt;color:#5F5A52;font-weight:700">' + Array.from({
    length: STG_NY
  }, (_, i) => '<span>' + (STG_Y0 + i) + '</span>').join('') + '</div></div></div>' + '<div class="stg-p-cols"><div><div class="stg-p-t">Objectifs mesurables</div><div class="stg-p-l">' + (d.okr.map(o => line(stgEsc(o.t), o.pct + '%')).join('') || '<div><span>—</span></div>') + '</div>' + '<div class="stg-p-t" style="margin-top:9px">Contribution aux axes groupe</div><div class="stg-p-l">' + ax.map(a => line(stgEsc(a.name), (d.align[a.id] || 0) + '%')).join('') + '</div></div>' + '<div><div class="stg-p-t">Budget par horizon</div><div class="stg-p-l">' + Object.keys(d.budgets).sort().map(y => line(y, stgEur(d.budgets[y]))).join('') + line('<b>Total</b>', stgEur(Object.values(d.budgets).reduce((a, b) => a + b, 0))) + '</div>' + '<div class="stg-p-t" style="margin-top:9px">Risques stratégiques</div><div class="stg-p-l">' + (d.risks.map(r => line(stgEsc(r.n), stgEsc(r.p) + ' / ' + stgEsc(r.i))).join('') || '<div><span>—</span></div>') + '</div>' + '<div class="stg-p-t" style="margin-top:9px">Indicateurs</div><div class="stg-p-l">' + d.kpis.map(k => line(stgEsc(k.l), stgEsc(k.v))).join('') + '</div></div></div>' + '<div style="margin-top:9px;font-size:7.5pt;color:#8C8579">Starium · ' + stgEsc(d.sigle) + ' · document généré le ' + stgDate(new Date().toISOString().slice(0, 10)) + ' — ' + d.chantiers.length + ' chantiers, ' + stgEur(d.chantiers.reduce((s, c) => s + c.bud, 0)) + ' de charge projet.</div>';
...
(function stgMountModals() {
  ...
  const head = (id, ico, t, s, sub) => '<div class="modal-head"><div class="modal-head-ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round">' + ico + '</svg></div>' + '<div><div class="modal-title" id="' + t + '">' + s + '</div><div class="modal-sub">' + (sub || '') + '</div></div>' + '<button class="modal-close" onclick="stgClose(\'' + id + '\')">' + x + '</button></div>';
  ...
  const html = wrap('sdirModal', 'modal-plan', head('sdirModal', ICO_D, 'sdm-title', 'Nouvelle direction', 'Identité, périmètre et cadre de la stratégie') + '<div class="modal-body"><input type="hidden" id="sdm-id">' + '<div class="field-row"><div class="field"><label class="field-label">Sigle <span class="req">*</span></label><input class="input" id="sdm-sigle" placeholder="Ex : DRH"></div>' + '<div class="field"><label class="field-label">Nom complet <span class="req">*</span></label><input class="input" id="sdm-name" placeholder="Ex : Direction des Ressources Humaines"></div></div>' + '<div class="field-row"><div class="field"><label class="field-label">Directeur·rice / sponsor</label><input class="input" id="sdm-dir" placeholder="Ex : Claire Dubois"></div>' + '<div class="field"><label class="field-label">Rattachement</label><input class="input" id="sdm-parent" placeholder="Direction Générale"></div></div>' + '<div class="field"><label class="field-label">Périmètre &amp; missions</label><textarea class="textarea" id="sdm-scope" rows="2" placeholder="Ce que la direction porte au quotidien."></textarea></div>' + '<div class="field"><label class="field-label">Ambition stratégique</label><textarea class="textarea" id="sdm-ambition" rows="3" placeholder="En une phrase : où la direction veut être à la fin de l’horizon."></textarea></div>' + '<div class="field-row"><div class="field"><label class="field-label">Effectif (ETP)</label><input class="input" type="number" min="0" id="sdm-fte" placeholder="Ex : 34"></div>' + '<div class="field"><label class="field-label">Budget annuel (k€)</label><input class="input" type="number" min="0" id="sdm-budget" placeholder="Ex : 4100"></div></div>' + '<div class="field-row"><div class="field"><label class="field-label">Horizon de la stratégie</label><input class="input" id="sdm-horizon" placeholder="2026 → 2028"></div>' + '<div class="field"><label class="field-label">Date de dernière revue</label><input class="input" type="date" id="sdm-review"></div></div>' + '<div class="field-row"><div class="field"><label class="field-label">Statut</label><select class="nselect" id="sdm-status"><option value="brouillon">Brouillon</option><option value="revue">En revue</option><option value="valide">Validé</option></select></div>' + '<div class="field"><label class="field-label">Couleur / thème</label><div class="type-pills" id="sdm-tone">' + [['info', 'Bleu'], ['teal', 'Émeraude'], ['gold', 'Or'], ['purple', 'Violet'], ['success', 'Vert']].map(t => '<button class="type-pill" data-tone="' + t[0] + '" onclick="stgPickTone(this)"><div class="type-pill-name">' + t[1] + '</div></button>').join('') + '</div></div></div></div>' + '<div class="modal-foot"><button class="btn btn-secondary" style="margin-right:auto;color:var(--state-danger)" onclick="stgDeleteDir()" id="sdm-del-dir">Supprimer</button>' + '<button class="btn btn-secondary" onclick="stgClose(\'sdirModal\')">Annuler</button>' + '<button class="btn btn-primary" onclick="stgSaveDir()">Enregistrer</button></div>') + wrap('schantModal', '', head('schantModal', ICO_C, 'scm-title', 'Nouveau chantier', 'Une ligne du schéma directeur') + '<div class="modal-body">' + '<div class="field"><label class="field-label">Intitulé du chantier <span class="req">*</span></label><input class="input" id="scm-name" placeholder="Ex : Dématérialisation des notes de frais"></div>' + '<div class="field"><label class="field-label">Axe propre de rattachement</label><select class="nselect" id="scm-lane"></select></div>' + '<div class="field-row"><div class="field"><label class="field-label">Début</label><select class="nselect" id="scm-s"></select></div>' + '<div class="field"><label class="field-label">Fin</label><select class="nselect" id="scm-e"></select></div></div>' + '<div class="field-row"><div class="field"><label class="field-label">Pilote</label><input class="input" id="scm-own" placeholder="Ex : Nadia Cherif"></div>' + '<div class="field"><label class="field-label">Budget (k€)</label><input class="input" type="number" min="0" id="scm-bud" placeholder="Ex : 350"></div></div>' + '<div class="field"><label class="field-label">Avancement — <span id="scm-pctv">0%</span></label><input type="range" min="0" max="100" step="5" value="0" id="scm-pct" oninput="document.getElementById(\'scm-pctv\').textContent=this.value+\'%\'" style="width:100%"></div>' + '<div class="field"><label class="field-label">Contribution aux axes du groupe</label><div class="stg-chips" id="scm-ga"></div></div>' + '<div class="field"><label class="field-label">Jalons structurants</label><textarea class="textarea" id="scm-ms" rows="3" placeholder="Une ligne par jalon, au format : T2 2027 : Bascule entité pilote"></textarea></div>' + '</div><div class="modal-foot"><button class="btn btn-secondary" id="scm-del" style="margin-right:auto;color:var(--state-danger)" onclick="stgDelChantier()">Supprimer</button>' + '<button class="btn btn-secondary" onclick="stgClose(\'schantModal\')">Annuler</button>' + '<button class="btn btn-primary" onclick="stgSaveChantier()">Enregistrer</button></div>') + wrap('sblockModal', '', head('sblockModal', ICO_T, 'sbm-title', 'Ajouter un texte', 'Note ou pièce du schéma directeur') + '<div class="modal-body"><input type="hidden" id="sbm-kind">' + '<div class="field"><label class="field-label">Titre <span class="req">*</span></label><input class="input" id="sbm-t" placeholder="Ex : Principes directeurs"></div>' + '<div class="field"><label class="field-label" id="sbm-b-label">Contenu</label><textarea class="textarea" id="sbm-b" rows="7" placeholder="Texte libre. Pour une image, cette zone sert de légende — le visuel se dépose ensuite sur la carte."></textarea></div>' + '</div><div class="modal-foot"><button class="btn btn-secondary" onclick="stgClose(\'sblockModal\')">Annuler</button>' + '<button class="btn btn-primary" onclick="stgSaveBlock()">Enregistrer</button></div>') + wrap('sokrModal', '', head('sokrModal', ICO_O, 'som-title', 'Nouvel objectif mesurable', 'Résultat attendu et indicateur de suivi') + '<div class="modal-body">' + '<div class="field"><label class="field-label">Objectif <span class="req">*</span></label><input class="input" id="som-t" placeholder="Ex : Clôture mensuelle en 5 jours ouvrés"></div>' + '<div class="field-row"><div class="field"><label class="field-label">Responsable</label><input class="input" id="som-who" placeholder="Ex : Pierre Moreau"></div>' + '<div class="field"><label class="field-label">Indicateur cible</label><input class="input" id="som-kpi" placeholder="Ex : 5 jours"></div></div>' + '<div class="field"><label class="field-label">Valeur actuelle</label><input class="input" id="som-cur" placeholder="Ex : 9 jours"></div>' + '<div class="field"><label class="field-label">Avancement — <span id="som-pctv">0%</span></label><input type="range" min="0" max="100" step="5" value="0" id="som-pct" oninput="document.getElementById(\'som-pctv\').textContent=this.value+\'%\'" style="width:100%"></div>' + '</div><div class="modal-foot"><button class="btn btn-secondary" onclick="stgClose(\'sokrModal\')">Annuler</button>' + '<button class="btn btn-primary" onclick="stgSaveOkr()">Ajouter</button></div>') + wrap('srevModal', '', head('srevModal', ICO_R, 'srm-title', 'Nouvelle revue stratégique', 'Versionner le schéma directeur') + '<div class="modal-body">' + '<div class="field-row"><div class="field"><label class="field-label">Version <span class="req">*</span></label><input class="input" id="srm-v" placeholder="v3.2"></div>' + '<div class="field"><label class="field-label">Date</label><input class="input" type="date" id="srm-d"></div></div>' + '<div class="field-row"><div class="field"><label class="field-label">Instance</label><input class="input" id="srm-by" placeholder="CODIR"></div>' + '<div class="field"><label class="field-label">Issue de la revue</label><select class="nselect" id="srm-st"><option value="valide">Validé</option><option value="revue">En revue</option><option value="brouillon">Renvoyé en brouillon</option></select></div></div>' + '<div class="field"><label class="field-label">Décisions &amp; commentaires</label><textarea class="textarea" id="srm-n" rows="4" placeholder="Ce qui a été arbitré, ce qui reste ouvert."></textarea></div>' + '</div><div class="modal-foot"><button class="btn btn-secondary" onclick="stgClose(\'srevModal\')">Annuler</button>' + '<button class="btn btn-primary" onclick="stgSaveReview()">Enregistrer la revue</button></div>') + wrap('scmpModal', 'modal-plan', head('scmpModal', ICO_X, 'scmp-title', 'Comparer deux directions', 'Ambition, moyens, alignement et maturité côte à côte') + '<div class="modal-body"><div class="field-row" style="margin-bottom:18px">' + '<div class="field" style="margin-bottom:0"><label class="field-label">Direction A</label><select class="nselect" id="scmp-a" onchange="stgRenderCompare()"></select></div>' + '<div class="field" style="margin-bottom:0"><label class="field-label">Direction B</label><select class="nselect" id="scmp-b" onchange="stgRenderCompare()"></select></div></div>' + '<div id="scmp-body"></div></div>' + '<div class="modal-foot"><button class="btn btn-secondary" onclick="stgClose(\'scmpModal\')">Fermer</button></div>');
  ...
  window.stgOpenDirModal = function (id) {
    om(id);
    if (dd) dd.style.display = id ? 'inline-flex' : 'none';
  };
})();
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', stgRenderDirs);else stgRenderDirs();
})(); } catch (e) { __ds_ns.__errors.push({ path: "ui_kits/app/modules/strategie-consol.js", error: String((e && e.message) || e) }); }

// ui_kits/app/modules/strategie.js
try { (() => {
...
const STG_SEED = [{
  ...
  chantiers: [{
    ...
    ms: [{
      m: 11,
      l: 'Attestation régulateur'
    }]
  }, {
    id: 'c5',
    n: 'SOC & détection 24/7',
    lane: 2,
    s: 9,
    e: 23,
    own: 'Alice Bernard',
    ...
    ms: [{
      m: 12,
      l: 'Première promotion'
    }]
  }],
  okr: [{
    t: 'Disponibilité des services critiques',
    who: 'Marc Dupont',
    kpi: '99,9 % de disponibilité',
    cur: '99,72 %',
    pct: 78
  }, {
    t: 'Réduire la dette technique de 30 %',
    who: 'Julien Tan',
    kpi: '-30 % de composants obsolètes',
    cur: '-13 %',
    pct: 45
  }, {
    t: 'Sécurité by design sur tous les cadrages',
    who: 'Alice Bernard',
    kpi: '100 % des projets',
    cur: '62 %',
    pct: 60
  }, {
    t: 'Time-to-market des évolutions < 6 semaines',
    who: 'Sophie Leroy',
    kpi: '6 semaines',
    cur: '11 semaines',
    pct: 35
  ...
  risks: [{
    n: 'Pénurie de compétences cloud',
    p: 'Élevée',
    i: 'Fort',
    own: 'CD',
    lvl: 'danger'
  }, {
    n: 'Dépendance à un intégrateur unique',
    p: 'Moyenne',
    i: 'Fort',
    own: 'MD',
    lvl: 'warning'
  }, {
    n: 'Retard réglementaire DORA',
    p: 'Moyenne',
    i: 'Critique',
    own: 'AB',
    lvl: 'danger'
  }, {
    n: 'Adhésion métier à la plateforme data',
    p: 'Faible',
    i: 'Moyen',
    own: 'JT',
    lvl: 'info'
  ...
  kpis: [{
    ...
    l: 'Coût IT / CA',
    v: '3,1 %',
    d: 'médiane secteur 3,4 %'
  }, {
    l: 'Satisfaction interne',
    v: '7,8/10',
    d: '+0,6 vs 2025'
  }, {
    l: 'Incidents majeurs',
  ...
  blocks: [{
    k: 'text',
    t: 'Principes directeurs',
    b: "1 — Cloud d'abord, exceptions documentées et arbitrées en comité d'architecture.\n2 — Une seule source de vérité par domaine de données.\n3 — Sécurité intégrée au cadrage, jamais en contrôle final.\n4 — Aucun nouveau développement sans propriétaire métier identifié."
  }, {
    k: 'image',
    t: 'Architecture cible 2028',
    b: 'Schéma d’architecture applicative et d’infrastructure cible',
    src: ''
  }, {
    k: 'text',
    t: 'Trajectoire de décommissionnement',
    b: "37 applications au périmètre. 12 décommissionnées d'ici fin 2026, 18 d'ici 2027, le solde intégré au socle cible. Chaque sortie est conditionnée à la reprise des données et à l'accord du métier propriétaire."
  }],
  reviews: [{
    v: 'v3.1',
    d: '2026-05-02',
    by: 'CODIR',
    st: 'valide',
    n: "Ajout du chantier Archivage & conformité légale. Budget 2028 révisé à la baisse (-0,4 M€)."
  }, {
    v: 'v3.0',
    d: '2025-11-14',
    by: 'CODIR',
    st: 'valide',
    n: "Extension de l'horizon à 2028 et intégration de l'axe Compétences IT."
  }, {
    v: 'v2.0',
    d: '2025-03-20',
    by: 'Comité d’architecture',
    st: 'valide',
    n: "Recentrage sur le socle et la cybersécurité après l'audit externe."
  }]
}, {
  id: 'daf',
  sigle: 'DAF',
  ...
  axes: [{
    n: 'Pilotage de la performance',
    t: 'info'
  }, {
    n: 'Industrialisation des processus',
    t: 'gold'
  }, {
    n: 'Maîtrise des risques financiers',
    t: 'teal'
  }],
  align: {
    ax1: 60,
  ...
  chantiers: [{
    ...
    ms: [{
      m: 9,
      l: 'Budget 2027 en nouveau format'
    }]
  }, {
    id: 'd2',
    n: 'Dématérialisation facture fournisseur',
    lane: 1,
    s: 2,
    e: 16,
    own: 'Nadia Cherif',
    ...
    ms: [{
      m: 10,
      l: 'Obligation e-invoicing'
    }]
  }, {
    id: 'd3',
    n: 'Consolidation ERP finance',
    lane: 1,
    s: 12,
    e: 32,
    own: 'Nadia Cherif',
    ...
    ms: [{
      m: 24,
      l: 'Bascule entité pilote'
    }]
  }, {
    id: 'd4',
    n: 'Contrôle interne & conformité',
    lane: 2,
    s: 6,
    e: 22,
    own: 'Hugo Petit',
    ...
    id: 'd5',
    n: 'Pilotage de la dette & trésorerie',
    lane: 0,
    s: 14,
    e: 26,
    own: 'Pierre Moreau',
  ...
  okr: [{
    t: 'Clôture mensuelle en 5 jours ouvrés',
    who: 'Pierre Moreau',
    kpi: '5 jours',
    cur: '9 jours',
    pct: 40
  }, {
    t: '90 % des factures fournisseurs dématérialisées',
    who: 'Nadia Cherif',
    kpi: '90 %',
    cur: '51 %',
    pct: 52
  }, {
    t: 'Écart prévision / réalisé budgétaire < 3 %',
    who: 'Hugo Petit',
    kpi: '3 %',
    cur: '6,8 %',
    pct: 30
  ...
  risks: [{
    n: 'Retard e-invoicing réglementaire',
    p: 'Moyenne',
    i: 'Critique',
    own: 'NC',
    lvl: 'danger'
  }, {
    n: 'Qualité des données de gestion',
    p: 'Élevée',
    i: 'Moyen',
    own: 'HP',
    lvl: 'warning'
  }, {
    n: 'Charge de la double tenue pendant l’ERP',
    p: 'Élevée',
    i: 'Fort',
    own: 'NC',
    lvl: 'danger'
  ...
  reviews: [{
    ...
    by: 'CODIR',
    st: 'revue',
    n: "Chiffrage de la consolidation ERP en cours d'arbitrage — décision attendue en septembre."
  }, {
    v: 'v1.0',
    d: '2025-09-09',
    by: 'CODIR',
    st: 'valide',
    n: 'Première version du schéma directeur finance.'
  }]
}];
let STG_DIRS = [],
  STG_CUR = null,
  ...
  STG_CH = null;
try {
  const s = JSON.parse(localStorage.getItem('starium_dirstrat'));
  STG_DIRS = s && s.length ? s : JSON.parse(JSON.stringify(STG_SEED));
} catch (e) {
  STG_DIRS = JSON.parse(JSON.stringify(STG_SEED));
}
function stgSave() {
  try {
    localStorage.setItem('starium_dirstrat', JSON.stringify(STG_DIRS));
  } catch (e) {}
}
function stgReset() {
  STG_DIRS = JSON.parse(JSON.stringify(STG_SEED));
...
function stgDir(id) {
  return STG_DIRS.find(d => d.id === id) || null;
}
function stgCur() {
  return stgDir(STG_CUR);
}
function stgTone(t) {
  return STG_TONES[t] || STG_TONES.info;
}
function stgEsc(s) {
...
function stgDate(iso) {
  if (!iso) return '—';
  const d = new Date(iso);
  return d.getDate() + ' ' + ['janv', 'févr', 'mars', 'avr', 'mai', 'juin', 'juil', 'août', 'sept', 'oct', 'nov', 'déc'][d.getMonth()] + '. ' + d.getFullYear();
}
function stgQlbl(m) {
  return 'T' + (Math.floor(m % 12 / 3) + 1) + ' ' + (STG_Y0 + Math.floor(m / 12));
}
function stgGAxes() {
  return typeof AXES !== 'undefined' && AXES.length ? AXES : [{
    id: 'ax1',
    name: 'Axe 1'
  }, {
    id: 'ax2',
...
function stgSvgRing(pct, color) {
  const r = 22,
    c = 2 * Math.PI * r;
  return '<svg width="52" height="52" viewBox="0 0 52 52"><circle cx="26" cy="26" r="' + r + '" fill="none" stroke="var(--neutral-200)" stroke-width="6"/>' + '<circle cx="26" cy="26" r="' + r + '" fill="none" stroke="' + color + '" stroke-width="6" stroke-linecap="round" stroke-dasharray="' + c.toFixed(1) + '" stroke-dashoffset="' + (c * (1 - pct / 100)).toFixed(1) + '" transform="rotate(-90 26 26)"/></svg>';
}
/* ─── Scores calculés ─── */
function stgScore(d) {
  const ax = stgGAxes();
...
function stgMaturity(d) {
  ...
  const bTot = Object.values(d.budgets).reduce((a, b) => a + b, 0),
    cTot = d.chantiers.reduce((s, c) => s + c.bud, 0);
...
function stgTab(name) {
  ...
  const ha = document.getElementById('vs-head-actions');
  if (ha) ha.dataset.tab = name;
  document.querySelectorAll('#vs-head-actions [data-only]').forEach(b => {
    b.style.display = b.dataset.only === name ? '' : 'none';
  });
...
function stgRenderDirs() {
  const host = document.getElementById('vs-dir-grid');
  if (!host) return;
  const q = STG_Q.trim().toLowerCase();
  const list = STG_DIRS.filter(d => !q || (d.name + ' ' + d.sigle + ' ' + d.dir).toLowerCase().includes(q));
  const cnt = document.getElementById('vs-dir-count');
  if (cnt) cnt.textContent = String(STG_DIRS.length);
  host.innerHTML = list.map(d => {
    const T = stgTone(d.tone),
      sc = stgScore(d),
      st = STG_STATUS[d.status] || STG_STATUS.brouillon;
    const done = d.chantiers.filter(c => c.pct >= 100).length;
    return '<div class="card stg-card" onclick="stgOpenDir(\'' + d.id + '\')">' + '<div class="stg-card-head"><div class="stg-sigle" style="background:' + T.bg + ';color:' + T.c + '">' + stgEsc(d.sigle) + '</div>' + '<div class="stg-card-id"><div class="stg-card-name">' + stgEsc(d.name) + '</div><div class="stg-card-sub">' + stgEsc(d.dir) + ' · ' + stgEsc(d.parent) + '</div></div>' + '<div class="stg-ring">' + stgSvgRing(sc, T.c) + '<div class="stg-ring-v" style="color:' + T.c + '">' + sc + '</div></div></div>' + '<p class="stg-card-scope">' + stgEsc(d.scope) + '</p>' + '<div class="stg-mgrid">' + '<div class="stg-mcell"><div class="l">Horizon</div><div class="v">' + stgEsc(d.horizon) + '</div></div>' + '<div class="stg-mcell"><div class="l">Chantiers</div><div class="v">' + d.chantiers.length + ' <span style="font-weight:600;color:var(--neutral-500);font-size:11.5px">dont ' + done + ' livré' + (done > 1 ? 's' : '') + '</span></div></div>' + '<div class="stg-mcell"><div class="l">Effectif</div><div class="v">' + d.fte + ' ETP</div></div>' + '<div class="stg-mcell"><div class="l">Budget</div><div class="v">' + stgEur(d.budget) + '</div></div>' + '</div>' + '<div class="stg-card-foot"><span class="badge ' + st.cls + '">' + st.lbl + ' · ' + stgEsc(d.version) + '</span>' + '<span class="stg-meta">Revue ' + stgDate(d.review) + '</span></div></div>';
  }).join('') + '<button class="stg-add" onclick="stgOpenDirModal()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>Ajouter une direction</button>';
}
function stgSearch(v) {
  STG_Q = v;
  stgRenderDirs();
}
...
function stgOpenDir(id) {
  ...
  const bc = document.getElementById('ds-bc');
  if (bc) bc.textContent = d.sigle;
}
function stgBackDirs() {
  if (typeof showView === 'function') showView('vision');
  stgTab('directions');
}
const STG_DTABS = [['axes', 'Axes stratégiques'], ['objectifs', 'Objectifs'], ['alignement', 'Alignement'], ['alertes', 'Alertes'], ['historique', 'Historique']];
let STG_DTAB = 'axes';
function stgDetTab(n) {
  STG_DTAB = n;
  stgRenderDetail();
}
...
function stgAlerts(d) {
  ...
  stgGAxes().forEach(a => {
    ...
    const v = d.align[a.id] || 0;
    if (!n) add(v > 0 ? 'warning' : 'danger', 'Axe groupe non couvert — ' + a.name, v > 0 ? 'Contribution déclarée à ' + v + ' % mais aucun chantier rattaché à cet axe.' : 'Aucune contribution déclarée et aucun chantier rattaché.');else if (v < 45) add('warning', 'Contribution faible — ' + a.name, v + ' % déclarés pour ' + n + ' chantier' + (n > 1 ? 's' : '') + ' rattaché' + (n > 1 ? 's' : '') + '.');
  });
  d.chantiers.forEach(c => {
    if (c.e <= STG_NOW && c.pct < 100) add('danger', 'Chantier en retard — ' + c.n, 'Fin prévue ' + stgQlbl(Math.max(0, c.e - 1)) + ', avancement ' + c.pct + ' %. Pilote ' + c.own + '.');else if (c.s <= STG_NOW && c.pct === 0) add('warning', 'Chantier non démarré — ' + c.n, 'Fenêtre ouverte depuis ' + stgQlbl(c.s) + ' sans avancement déclaré.');
  ...
  d.axes.forEach((x, i) => {
    if (!d.chantiers.filter(c => c.lane === i).length) add('info', 'Axe propre sans chantier — ' + x.n, 'Axe déclaré mais aucune ligne du schéma directeur ne le porte.');
  });
  const bTot = Object.values(d.budgets).reduce((x, y) => x + y, 0),
    cTot = d.chantiers.reduce((x, c) => x + c.bud, 0);
...
function stgRenderDetail() {
  const d = stgCur(),
    root = document.getElementById('ds-root');
  if (!d || !root) return;
  const T = stgTone(d.tone),
    sc = stgScore(d),
    st = STG_STATUS[d.status] || STG_STATUS.brouillon;
  const cTot = d.chantiers.reduce((s, c) => s + c.bud, 0);
  const AL = stgAlerts(d),
    nCrit = AL.filter(x => x.lvl === 'danger').length;
  ...
  let pane = '';
  if (STG_DTAB === 'axes') {
    pane = '<div class="stg-sec"><div class="stg-sec-head"><div>' + secT('<rect x="3" y="4" width="18" height="17" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/><line x1="9" y1="10" x2="9" y2="21"/>', 'Schéma directeur ' + stgEsc(d.horizon), d.chantiers.length + ' chantiers · ' + stgEur(cTot) + ' de charge projet · jalons structurants en losange') + '</div>' + '<button class="btn btn-secondary" onclick="stgOpenChantier()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>Ajouter un chantier</button></div>' + '<div class="card stg-tl">' + stgTimeline(d) + '</div></div>' + '<div class="stg-sec"><div class="stg-sec-t" style="margin-bottom:12px">Axes propres de la direction</div><div class="card stg-contrib">' + d.axes.map((a, i) => {
      const cs = d.chantiers.filter(c => c.lane === i);
      const p = cs.length ? Math.round(cs.reduce((s, c) => s + c.pct, 0) / cs.length) : 0;
      const t = stgTone(a.t);
      return '<div class="stg-contrib-row"><div class="l" title="' + stgEsc(a.n) + '">' + stgEsc(a.n) + '</div><div class="t"><i style="width:' + p + '%;background:' + t.c + '"></i></div><div class="p">' + p + '%</div></div>' + '<div style="font-size:11px;color:var(--neutral-500);font-weight:600;margin:-6px 0 2px 0">' + (cs.length ? cs.length + ' chantier' + (cs.length > 1 ? 's' : '') : 'aucun chantier') + '</div>';
    }).join('') + '</div></div>' + '<div class="stg-sec"><div class="stg-sec-head"><div>' + secT('<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>', 'Notes &amp; pièces du schéma directeur', 'Textes de cadrage, principes, schémas d’architecture — libre à la direction.') + '</div>' + '<div style="display:flex;gap:8px"><button class="btn btn-secondary" onclick="stgOpenBlock(\'text\')">Ajouter un texte</button><button class="btn btn-secondary" onclick="stgOpenBlock(\'image\')">Ajouter une image</button></div></div>' + '<div class="stg-blocks">' + stgBlocks(d) + '</div></div>';
  }
  if (STG_DTAB === 'objectifs') {
    pane = '<div class="stg-sec"><div class="stg-sec-head"><div>' + secT('<circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/>', 'Objectifs &amp; résultats mesurables', d.okr.length + ' objectif' + (d.okr.length > 1 ? 's' : '') + ' suivi' + (d.okr.length > 1 ? 's' : '') + ' par la direction') + '</div>' + '<button class="btn btn-secondary" onclick="stgOpenOkr()">Ajouter un objectif</button></div>' + '<div class="card tablecard"><div class="table-wrap"><table class="dt"><thead><tr><th>Objectif</th><th>Responsable</th><th>Indicateur cible</th><th>Actuel</th><th>Avancement</th><th class="right"></th></tr></thead><tbody>' + (d.okr.length ? d.okr.map((o, i) => '<tr><td class="cell-strong">' + stgEsc(o.t) + '</td><td>' + stgEsc(o.who) + '</td><td>' + stgEsc(o.kpi) + '</td><td style="font-variant-numeric:tabular-nums;font-weight:700">' + stgEsc(o.cur) + '</td>' + '<td><div class="prog"><div class="prog-track"><div class="prog-fill" style="width:' + o.pct + '%;background:' + (o.pct >= 70 ? 'var(--state-success)' : o.pct >= 40 ? 'var(--brand-gold)' : 'var(--state-danger)') + '"></div></div><span class="prog-pct">' + o.pct + '%</span></div></td>' + '<td class="right"><button class="dots-btn" title="Supprimer" onclick="stgDelOkr(' + i + ')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg></button></td></tr>').join('') : '<tr><td colspan="6"><div class="stg-empty">Aucun objectif renseigné pour cette direction.</div></td></tr>') + '</tbody></table></div></div></div>' + '<div class="stg-sec"><div class="stg-sec-t" style="margin-bottom:12px">Budget prévisionnel par horizon</div><div class="card stg-contrib">' + Object.keys(d.budgets).sort().map(y => {
      const mx = Math.max.apply(null, Object.values(d.budgets)) || 1;
      return '<div class="stg-contrib-row" style="grid-template-columns:60px 1fr 70px"><div class="l">' + y + '</div><div class="t"><i style="width:' + Math.round(d.budgets[y] / mx * 100) + '%;background:' + T.c + '"></i></div><div class="p">' + stgEur(d.budgets[y]) + '</div></div>';
    }).join('') + '<div style="display:flex;justify-content:space-between;font-size:12px;font-weight:700;color:var(--neutral-600);padding-top:10px;border-top:1px solid var(--neutral-100)"><span>Total horizon</span><span>' + stgEur(Object.values(d.budgets).reduce((a, b) => a + b, 0)) + '</span></div>' + '<div style="display:flex;justify-content:space-between;font-size:12px;font-weight:600;color:var(--neutral-500);padding-top:8px"><span>Charge des chantiers</span><span>' + stgEur(cTot) + '</span></div>' + '</div></div>';
  }
  if (STG_DTAB === 'alignement') {
    const M = stgMaturity(d);
    pane = '<div class="stg-sec"><div>' + secT('<circle cx="12" cy="12" r="10"/><path d="M12 2a15.3 15.3 0 0 1 0 20"/><path d="M2 12h20"/>', 'Contribution aux axes du groupe', 'Score global d’alignement : ' + sc + ' %. La contribution déclarée est confrontée aux chantiers réellement rattachés.') + '</div>' + '<div class="card stg-contrib" style="margin-top:12px">' + stgGAxes().map(a => {
      const v = d.align[a.id] || 0;
      const cs = d.chantiers.filter(c => (c.ga || []).includes(a.id));
      return '<div class="stg-contrib-row"><div class="l" title="' + stgEsc(a.name) + '">' + stgEsc(a.name) + '</div><div class="t"><i style="width:' + v + '%;background:' + (v >= 70 ? 'var(--state-success)' : v >= 45 ? 'var(--brand-gold)' : 'var(--state-danger)') + '"></i></div><div class="p">' + v + '%</div></div>' + '<div style="font-size:11px;color:var(--neutral-500);font-weight:600;margin:-6px 0 2px 0">' + (cs.length ? cs.map(c => stgEsc(c.n)).join(' · ') : 'aucun chantier rattaché') + '</div>';
    }).join('') + '</div></div>' + '<div class="stg-sec"><div class="stg-sec-t" style="margin-bottom:12px">Maturité du schéma directeur</div><div class="card stg-contrib">' + Object.keys(M).map(k => '<div class="stg-contrib-row"><div class="l">' + k + '</div><div class="t"><i style="width:' + M[k] + '%;background:' + (M[k] >= 70 ? 'var(--state-success)' : M[k] >= 40 ? 'var(--brand-gold)' : 'var(--state-danger)') + '"></i></div><div class="p">' + M[k] + '%</div></div>').join('') + '</div></div>';
  }
  if (STG_DTAB === 'alertes') {
    const cls = {
        danger: 'bdg-danger',
    ...
    pane = '<div class="stg-sec"><div>' + secT('<path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>', 'Alertes de désalignement &amp; d’exécution', AL.length ? AL.length + ' alerte' + (AL.length > 1 ? 's' : '') + ' dont ' + nCrit + ' critique' + (nCrit > 1 ? 's' : '') + ' — calculées à partir des chantiers, du budget et des revues.' : 'Aucune alerte : le schéma directeur est complet et à jour.') + '</div>' + '<div class="card tablecard" style="margin-top:12px"><div class="table-wrap"><table class="dt"><thead><tr><th style="width:130px">Niveau</th><th>Alerte</th><th>Détail</th></tr></thead><tbody>' + (AL.length ? AL.map(x => '<tr><td><span class="badge ' + cls[x.lvl] + '">' + lbl[x.lvl] + '</span></td><td class="cell-strong">' + stgEsc(x.t) + '</td><td style="color:var(--neutral-600)">' + stgEsc(x.b) + '</td></tr>').join('') : '<tr><td colspan="3"><div class="stg-empty">Aucune alerte détectée sur ce schéma directeur.</div></td></tr>') + '</tbody></table></div></div></div>' + '<div class="stg-sec"><div class="stg-sec-t" style="margin-bottom:12px">Risques stratégiques déclarés</div>' + '<div class="card tablecard"><div class="table-wrap"><table class="dt"><thead><tr><th>Risque</th><th>Prob.</th><th>Impact</th><th class="right">Propriétaire</th></tr></thead><tbody>' + (d.risks.length ? d.risks.map(r => '<tr><td><span class="badge ' + (r.lvl === 'danger' ? 'bdg-danger' : r.lvl === 'warning' ? 'bdg-warn' : 'bdg-info') + '">' + stgEsc(r.n) + '</span></td><td>' + stgEsc(r.p) + '</td><td>' + stgEsc(r.i) + '</td><td class="right"><span class="stg-chip">' + stgEsc(r.own) + '</span></td></tr>').join('') : '<tr><td colspan="4"><div class="stg-empty">Aucun risque stratégique déclaré.</div></td></tr>') + '</tbody></table></div></div></div>';
  }
  if (STG_DTAB === 'historique') {
    pane = '<div class="stg-sec"><div class="stg-sec-head"><div>' + secT('<path d="M3 3v5h5"/><path d="M3.05 13A9 9 0 1 0 6 5.3L3 8"/><polyline points="12 7 12 12 15 14"/>', 'Revues stratégiques &amp; versions', d.reviews.length + ' version' + (d.reviews.length > 1 ? 's' : '') + ' · version courante ' + stgEsc(d.version) + ' (' + st.lbl.toLowerCase() + ')') + '</div>' + '<button class="btn btn-secondary" onclick="stgOpenReview()">Nouvelle revue</button></div>' + '<div class="card stg-rev">' + d.reviews.map(r => {
      const s = STG_STATUS[r.st] || STG_STATUS.valide;
      return '<div class="stg-rev-item"><div><div class="stg-rev-v">' + stgEsc(r.v) + '</div><div class="stg-rev-d">' + stgDate(r.d) + '</div></div>' + '<div><div class="stg-rev-n">' + stgEsc(r.n) + '</div><div class="stg-rev-w">' + stgEsc(r.by) + '</div></div>' + '<span class="badge ' + s.cls + '">' + s.lbl + '</span></div>';
    }).join('') + '</div></div>';
  }
  root.innerHTML = '<div class="pg-head"><div><h1>' + stgEsc(d.sigle) + ' — Schéma directeur</h1><p class="np-sub">' + stgEsc(d.name) + ' · ' + stgEsc(d.horizon) + ' · ' + stgEsc(d.version) + '</p></div>' + '<div class="head-actions">' + '<button class="btn btn-secondary" onclick="stgBackDirs()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>Toutes les directions</button>' + '<button class="btn btn-secondary" onclick="stgExportPdf(\'' + d.id + '\')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round"><path d="M6 9V2h12v7"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>Export PDF 1 page</button>' + '<button class="btn btn-secondary" onclick="stgOpenReview()"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>Nouvelle revue</button>' + '<button class="btn btn-modifier" onclick="stgOpenDirModal(\'' + d.id + '\')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4z"/></svg>Modifier la direction</button>' + '</div></div>'
  /* hero */ + '<div class="card stg-hero"><div class="stg-hero-sigle" style="background:' + T.bg + ';color:' + T.c + '">' + stgEsc(d.sigle) + '</div>' + '<div class="stg-hero-id"><h2>' + stgEsc(d.name) + '</h2>' + '<div class="stg-hero-meta"><span class="badge ' + st.cls + '">' + st.lbl + '</span><span class="stg-hero-sep"></span>' + '<span class="stg-meta">' + stgEsc(d.dir) + ' · directeur·rice</span><span class="stg-hero-sep"></span>' + '<span class="stg-meta">Rattachée à ' + stgEsc(d.parent) + '</span><span class="stg-hero-sep"></span>' + '<span class="stg-meta">' + d.fte + ' ETP · ' + stgEur(d.budget) + '</span><span class="stg-hero-sep"></span>' + '<span class="stg-meta">Dernière revue ' + stgDate(d.review) + '</span></div>' + '<p class="stg-ambition" style="margin-top:16px">' + d.ambition + '</p>' + '<p class="stg-sec-sub" style="margin-top:10px">' + stgEsc(d.scope) + '</p></div>' + '<div class="stg-ring" style="width:72px;height:72px"><svg width="72" height="72" viewBox="0 0 52 52" style="width:72px;height:72px">' + stgSvgRing(sc, T.c).replace(/^<svg[^>]*>/, '').replace(/<\/svg>$/, '') + '</svg>' + '<div class="stg-ring-v" style="color:' + T.c + ';font-size:17px;flex-direction:column"><div>' + sc + '</div><div style="font-size:8.5px;font-weight:700;color:var(--neutral-500);text-transform:uppercase;letter-spacing:.06em">align.</div></div></div></div>'
  /* kpis */ + '<div class="stg-kpis">' + d.kpis.map(k => '<div class="stg-kpi"><div class="l">' + stgEsc(k.l) + '</div><div class="v">' + stgEsc(k.v) + '</div><div class="d">' + stgEsc(k.d) + '</div></div>').join('') + '</div>'
  /* sous-onglets, calqués sur la Vision groupe */ + '<div class="bud-subtabs" id="ds-subtabs">' + STG_DTABS.map(t => '<button class="bud-subtab' + (STG_DTAB === t[0] ? ' active' : '') + '" onclick="stgDetTab(\'' + t[0] + '\')">' + t[1] + (t[0] === 'alertes' && AL.length ? '<span class="stg-tabn' + (nCrit ? ' crit' : '') + '">' + AL.length + '</span>' : '') + (t[0] === 'objectifs' && d.okr.length ? '<span class="stg-tabn">' + d.okr.length + '</span>' : '') + '</button>').join('') + '</div>' + '<div class="ds-pane">' + pane + '</div>';
}
/* ─── Timeline du schéma directeur ─── */
function stgTimeline(d) {
  const pc = m => m / STG_NM * 100;
  let h = '<div class="stg-tl-row" style="--stg-ny:' + STG_NY + '"><div></div><div class="stg-tl-years" style="--stg-ny:' + STG_NY + '">' + Array.from({
    length: STG_NY
  }, (_, i) => '<div class="stg-tl-year">' + (STG_Y0 + i) + '</div>').join('') + '</div></div>';
  d.axes.forEach((a, i) => {
    const cs = d.chantiers.filter(c => c.lane === i);
    if (!cs.length) return;
    const t = stgTone(a.t);
    ...
    cs.sort((x, y) => x.s - y.s).forEach(c => {
      h += '<div class="stg-tl-row stg-tl-lane" style="--stg-nq:12"><div><div class="stg-tl-name" onclick="stgOpenChantier(\'' + c.id + '\')">' + stgEsc(c.n) + '</div><div class="stg-tl-nsub">' + stgEsc(c.own) + ' · ' + stgEur(c.bud) + '</div></div>' + '<div class="stg-tl-track" style="--stg-nq:12">' + '<div class="stg-tl-bar" style="left:' + pc(c.s).toFixed(2) + '%;width:' + pc(Math.max(1, c.e - c.s)).toFixed(2) + '%;background:' + t.c + '" onclick="stgOpenChantier(\'' + c.id + '\')" title="' + stgEsc(c.n) + ' — ' + stgQlbl(c.s) + ' → ' + stgQlbl(c.e) + ' · ' + c.pct + '%">' + '<i style="width:' + c.pct + '%"></i><span>' + c.pct + '%</span></div>' + (c.ms || []).map(m => '<div class="stg-tl-ms" style="left:' + pc(m.m).toFixed(2) + '%" title="' + stgEsc(m.l) + ' — ' + stgQlbl(m.m) + '"></div>').join('') + '<div class="stg-tl-now" style="left:' + pc(STG_NOW).toFixed(2) + '%"></div>' + '</div></div>';
    });
  });
  h += '<div class="stg-tl-legend">' + d.axes.map(a => '<span class="k"><i style="background:' + stgTone(a.t).c + '"></i>' + stgEsc(a.n) + '</span>').join('') + '<span class="k"><span class="dia"></span>Jalon structurant</span><span class="k"><i style="background:var(--state-danger);width:2px;height:12px;border-radius:0"></i>Aujourd’hui</span></div>';
  return h;
}
/* ─── Blocs texte / image ─── */
function stgBlocks(d) {
  ...
  return d.blocks.map((b, i) => {
    const acts = '<div class="stg-block-acts"><button title="Modifier" onclick="stgOpenBlock(\'' + b.k + '\',' + i + ')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></button>' + '<button title="Supprimer" onclick="stgDelBlock(' + i + ')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg></button></div>';
    if (b.k === 'image') {
      return '<div class="card stg-block">' + acts + '<div class="stg-block-t">' + stgEsc(b.t) + '</div>' + (b.src ? '<img src="' + b.src + '" alt="' + stgEsc(b.t) + '">' : '<div class="stg-drop" onclick="stgPickImage(' + i + ')" ondragover="event.preventDefault();this.classList.add(\'over\')" ondragleave="this.classList.remove(\'over\')" ondrop="stgDropImage(event,' + i + ')">' + '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>' + stgEsc(b.b || 'Déposer une image') + '<span style="font-weight:600;color:var(--neutral-400)">Glisser-déposer ou cliquer</span></div>') + '</div>';
    }
...
function stgPickImage(i) {
  ...
  inp.onchange = () => {
    const f = inp.files[0];
    if (f) stgReadImage(f, i);
  };
  inp.click();
}
function stgDropImage(ev, i) {
  ev.preventDefault();
...
function stgReadImage(f, i) {
  ...
  r.onload = () => {
    const d = stgCur();
    if (!d) return;
    d.blocks[i].src = r.result;
    stgSave();
    stgRenderDetail();
    stgToast('Image ajoutée au schéma directeur');
  };
  r.readAsDataURL(f);
}
function stgDelBlock(i) {
  const d = stgCur();
  if (!d) return;
  d.blocks.splice(i, 1);
  stgSave();
  stgRenderDetail();
  stgToast('Bloc supprimé');
}
function stgDelOkr(i) {
  const d = stgCur();
  if (!d) return;
  d.okr.splice(i, 1);
  stgSave();
  stgRenderDetail();
  stgToast('Objectif supprimé');
}
/* ─── Modales ─── */
function stgOpen(id) {
...
function stgOpenDirModal(id) {
  ...
  const set = (k, v) => {
    document.getElementById('sdm-' + k).value = v;
  };
  set('sigle', d ? d.sigle : '');
  set('name', d ? d.name : '');
  set('dir', d ? d.dir : '');
  set('parent', d ? d.parent : 'Direction Générale');
  set('scope', d ? d.scope : '');
  set('fte', d ? d.fte : '');
  set('budget', d ? Math.round(d.budget / 1000) : '');
  set('horizon', d ? d.horizon : '2026 → 2028');
  set('status', d ? d.status : 'brouillon');
  set('review', d ? d.review : '');
...
function stgSaveDir() {
  ...
  const sigle = g('sigle'),
    name = g('name');
  if (!sigle || !name) {
    stgToast('Sigle et nom sont requis');
    return;
  }
  const tone = (document.querySelector('#sdm-tone .type-pill.sel') || {
    dataset: {}
  }).dataset.tone || 'info';
  const id = g('id'),
    d = id ? stgDir(id) : null;
  const p = {
    ...
    name,
    dir: g('dir') || '—',
    parent: g('parent') || 'Direction Générale',
    scope: g('scope'),
    fte: parseInt(g('fte'), 10) || 0,
    budget: (parseInt(g('budget'), 10) || 0) * 1000,
    horizon: g('horizon') || '2026 → 2028',
    status: g('status'),
    review: g('review') || '',
    tone,
    ambition: g('ambition') || 'Ambition à formuler.'
  };
  if (d) {
    Object.assign(d, p);
    stgToast('Direction mise à jour');
  } else {
    const nid = sigle.toLowerCase().replace(/[^a-z0-9]/g, '') + '-' + Date.now().toString(36).slice(-4);
    STG_DIRS.push(Object.assign({
      id: nid,
      version: 'v0.1',
      axes: [{
        n: 'Axe 1 — à nommer',
      ...
      reviews: [{
        ...
        by: 'Création',
        st: 'brouillon',
        n: 'Création du schéma directeur.'
      }]
    }, p));
...
function stgDeleteDir() {
  const id = document.getElementById('sdm-id').value;
  if (!id) return;
  const d = stgDir(id);
  if (!d) return;
  if (!confirm('Supprimer la direction « ' + d.sigle + ' » et son schéma directeur ?')) return;
  STG_DIRS = STG_DIRS.filter(x => x.id !== id);
  stgSave();
  stgClose('sdirModal');
  stgToast('Direction supprimée');
  stgBackDirs();
}
function stgOpenChantier(cid) {
  ...
  const opts = Array.from({
    length: STG_NM + 1
  }, (_, m) => '<option value="' + m + '">' + stgQlbl(Math.min(m, STG_NM - 1)) + '</option>').join('');
  document.getElementById('scm-s').innerHTML = opts;
  document.getElementById('scm-e').innerHTML = opts;
  document.getElementById('scm-s').value = c ? c.s : 0;
  document.getElementById('scm-e').value = c ? c.e : 11;
...
function stgSaveChantier() {
  ...
  if (!n) {
    stgToast('Nom du chantier requis');
    return;
  }
  ...
  if (c) Object.assign(c, p);else d.chantiers.push(Object.assign({
    id: 'c' + Date.now().toString(36),
    prj: []
  }, p));
  stgSave();
  stgClose('schantModal');
  stgRenderDetail();
...
function stgDelChantier() {
  const d = stgCur();
  if (!d || !STG_CH) return;
  d.chantiers = d.chantiers.filter(x => x.id !== STG_CH);
  stgSave();
  stgClose('schantModal');
  stgRenderDetail();
  stgToast('Chantier supprimé');
}
...
function stgSaveBlock() {
  ...
  const t = document.getElementById('sbm-t').value.trim(),
    b = document.getElementById('sbm-b').value;
  if (!t) {
    stgToast('Titre requis');
    return;
  }
  if (STG_BLK == null) d.blocks.push({
    k,
    t,
    b,
    src: ''
  });else {
    d.blocks[STG_BLK].t = t;
    d.blocks[STG_BLK].b = b;
  }
  stgSave();
  stgClose('sblockModal');
  stgRenderDetail();
  stgToast('Bloc enregistré');
}
function stgOpenOkr() {
  ['som-t', 'som-who', 'som-kpi', 'som-cur'].forEach(k => document.getElementById(k).value = '');
  document.getElementById('som-pct').value = 0;
  document.getElementById('som-pctv').textContent = '0%';
  stgOpen('sokrModal');
}
function stgSaveOkr() {
  const d = stgCur();
  ...
  if (!t) {
    stgToast('Titre de l’objectif requis');
    return;
  }
  d.okr.push({
    t,
    who: document.getElementById('som-who').value.trim() || '—',
    kpi: document.getElementById('som-kpi').value.trim() || '—',
    cur: document.getElementById('som-cur').value.trim() || '—',
    pct: parseInt(document.getElementById('som-pct').value, 10) || 0
  });
  stgSave();
  stgClose('sokrModal');
  stgRenderDetail();
  stgToast('Objectif ajouté');
}
...
function stgSaveReview() {
  ...
  if (!v) {
    stgToast('Numéro de version requis');
    return;
  }
  const st = document.getElementById('srm-st').value;
  d.reviews.unshift({
    v,
    d: document.getElementById('srm-d').value,
    by: document.getElementById('srm-by').value.trim() || '—',
    st,
    n: document.getElementById('srm-n').value.trim() || 'Revue sans commentaire.'
  });
  d.version = v;
  d.status = st;
  d.review = document.getElementById('srm-d').value;
  stgSave();
  stgClose('srevModal');
  stgRenderDetail();
  stgToast('Revue ' + v + ' enregistrée');
}
})(); } catch (e) { __ds_ns.__errors.push({ path: "ui_kits/app/modules/strategie.js", error: String((e && e.message) || e) }); }
