...
function initCanvas(){
  ...
  const pat = svgEl('pattern',{id:'atlas-grid', width:'48', height:'48', patternUnits:'userSpaceOnUse'});
  pat.appendChild(svgEl('circle',{cx:'2', cy:'2', r:'1.4', fill:'#E4DFD5'}));
  defs.appendChild(pat);
  const mkShadow=(id,dy,std,color,op)=>{ const f=svgEl('filter',{id, x:'-70%', y:'-70%', width:'240%', height:'240%'}); f.appendChild(svgEl('feDropShadow',{dx:0, dy, stdDeviation:std, 'flood-color':color, 'flood-opacity':op})); defs.appendChild(f); };
  mkShadow('atlas-shadow', 3, 4, '#0E0E10', 0.16);
  mkShadow('atlas-glow', 0, 7, '#E8A317', 0.6);

  viewportG = svgEl('g',{id:'viewport'});
  const gridG = svgEl('g',{id:'grid-layer'});
  gridG.appendChild(svgEl('rect',{x:-4000, y:-4000, width:10000, height:10000, fill:'url(#atlas-grid)'}));
  edgesG = svgEl('g',{id:'edges-layer'});
  nodesG = svgEl('g',{id:'nodes-layer'});
  viewportG.appendChild(gridG); viewportG.appendChild(edgesG); viewportG.appendChild(nodesG);
...
function renderGraph(){
  ...
  edges.forEach((e,i)=>{
    ...
    hit.addEventListener('click', (ev)=>{ ev.stopPropagation(); atShowEdgeDetail(e.id); });
    hit.addEventListener('mouseenter', (ev)=>{ edgeTooltipShow(e, ev); });
    hit.addEventListener('mousemove', (ev)=>{ edgeTooltipMove(ev); });
    hit.addEventListener('mouseleave', ()=>{ edgeTooltipHide(); });
  ...
  NODES.filter(n=>ids.has(n.id)).forEach(n=>{
    ...
    const halo = svgEl('circle',{class:'node-halo', r:30, fill:'none', stroke:'var(--gold, #E8A317)', 'stroke-width':2.5});
    g.appendChild(halo);
    const circle = svgEl('circle',{class:'node-circle'+(opacityClass?' '+opacityClass:''), r:23, fill:tm.bg, stroke:tm.color});
...
/* ============================================================
   ...
   ============================================================ */
const FLOW_LABEL = {applicatif:'Applicatif', metier:'Métier', technique:'Technique', fournisseur:'Fournisseur'};
function atShowEdgeDetail(edgeId){
  const e = EDGES.find(x=>x.id===edgeId); if(!e) return;
  const sN = NODE_BY_ID[e.s], tN = NODE_BY_ID[e.t];
  const stm = TYPE_META[sN.type], ttm = TYPE_META[tN.type];
...
function emClose(){ document.getElementById('em-overlay').classList.remove('open'); }
...
function edgeTooltipHide(){ document.getElementById('edge-tooltip').classList.remove('show'); }
