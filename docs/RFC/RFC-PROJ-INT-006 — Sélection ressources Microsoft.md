...
| GET | *à définir après spike* | Plans Planner exploitables pour le contexte choisi |
...
* Mock Graph : listes vides, erreurs permission, isolation **client** (pas de données d’un autre `clientId`).

## 6. Récapitulatif

* Sélection ressources = **facade** au-dessus de Graph ; le **contrat exact** des URLs et DTO suit le spike.
