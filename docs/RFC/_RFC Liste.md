...
| **RFC-ORG-002** | Lien `ClientUser` ↔ `Resource` HUMAN | 📝 Draft | Extension `ClientUser.resourceId` (FK HUMAN, même `clientId`), API + audit + UI admin membres (libellés métier) ; prérequis calcul `OWN`/`SCOPE` ; plan [_Plan Org & licences](./_Plan%20de%20d%C3%A9ploement%20Orgnisation%20et%20licences.md) ; voir [RFC](./RFC-ORG-002%20%E2%80%94%20Lien%20ClientUser%20%E2%86%94%20Resource%20HUMAN.md) |
| **RFC-ORG-003** | Propriété organisationnelle des ressources | 📝 Draft | `ownerOrgUnitId` (ou table dédiée) sur entités métier, validations, UI sélecteur d’unité, audit ; alimente filtrage `SCOPE` ; voir [RFC](./RFC-ORG-003%20%E2%80%94%20Propri%C3%A9t%C3%A9%20organisationnelle%20des%20ressources.md) |
...
| **RFC-ACL-016** | Résolution du scope organisationnel | 📝 Draft | `OrganizationScopeService`, verdict `NONE`/`OWN`/`SCOPE`/`ALL` + `reasonCodes`, dépend ORG-002/003 + ACL-015 ; voir [RFC](./RFC-ACL-016%20%E2%80%94%20R%C3%A9solution%20du%20scope%20organisationnel.md) |
