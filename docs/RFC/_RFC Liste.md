...
| **RFC-ACL-015** | Permissions `OWN` / `SCOPE` / `ALL` | 🟡 Partielle (socle) | Package `packages/rbac-permissions` (`satisfiesPermission`, seed rows), guards + diagnostics + `GET /me/permissions` (`uiPermissionHints`), seed modules budgets/projects/procurement + permissions scoped ; **hors** : filtrage métier par scope (→ 016/018) ; voir [RFC](./RFC-ACL-015%20%E2%80%94%20Permissions%20OWN%20SCOPE%20ALL.md) |
| **RFC-ACL-016** | Résolution du scope organisationnel | 🟡 Service livré — branché au moteur lecture (018) | `OrganizationScopeService` (`apps/api/src/common/organization/`), verdict `NONE`/`OWN`/`SCOPE`/`ALL` + `reasonCodes` typés (`ALL_RBAC_OVERRIDE`, `OWN_SELF_MATCH`, `SCOPE_DIRECT_MATCH`, `SCOPE_DESCENDANT_MATCH`, `SCOPE_OWNER_ORG_UNIT_INACTIVE`, `SCOPE_OUT_OF_SUBTREE`, `SCOPE_NO_MEMBERSHIPS`, `SCOPE_NO_RESOURCE_LINK`, `OWN_NO_RESOURCE_LINK`, `OWN_NO_HINT`, `OWN_MISMATCH`, `MISSING_OWNER_ORG_UNIT`) ; BFS in-memory sur arbre actif + cache requête (`resolvedOrgScopeContext`, `resolvedOrgUnitTreeByClient`) ; **OWN** = `subjectResourceId === ClientUser.resourceId` (self HUMAN) ; **SCOPE** = descendants des memberships actives ; **ALL** = `hasAllOverride` pré-calculé par l’appelant (ex. moteur 018 pour `read_all` / legacy `*.read`) ; consommé par [RFC-ACL-018](./RFC-ACL-018%20%E2%80%94%20Moteur%20de%20d%C3%A9cision%20d%27acc%C3%A8s%20unifi%C3%A9.md) ; **visible dans le diagnostic** si [RFC-ACL-019](./RFC-ACL-019%20%E2%80%94%20Diagnostic%20enrichi%20organisation%20et%20acc%C3%A8s.md) activé (`ACCESS_DIAGNOSTICS_ENRICHED`) ; voir [RFC](./RFC-ACL-016%20%E2%80%94%20R%C3%A9solution%20du%20scope%20organisationnel.md) |
...
| **RFC-ACL-018** | Moteur de décision d’accès unifié | ✅ Implémentée (V1 — lecture pilote Projets) | Module `apps/api/src/modules/access-decision/` : `AccessDecisionService` (`decide`, `assertAllowed`, `filterResourceIdsByAccess`), `loadAccessResources`, `evaluateReadRbacIntent`, `membership-access-gates`, `ResourceAccessDecisionGuard` + `@RequireAccess` (export ; pilote via **service** `ProjectsService`) ; `sharingFloorAllows = floorAllowed` ; pas de pré-filtre org avant ACL ; intents `write`/`admin` refusés côté moteur jusqu’à RFC-020 ; verdict `AccessDecisionResult` → RFC-019 ; voir [RFC](./RFC-ACL-018%20%E2%80%94%20Moteur%20de%20d%C3%A9cision%20d%27acc%C3%A8s%20unifi%C3%A9.md) |
| **RFC-ACL-019** | Diagnostic enrichi (organisation + propriété + politique) | ✅ Implémentée (V1 — flag) | Opt-in `ACCESS_DIAGNOSTICS_ENRICHED` (`true` \| `1` strictement) ; **read** = `AccessDecisionService.decide` sous garde-fous ; `organizationScopeCheck` / `resourceOwnershipCheck` / `resourceAccessPolicyCheck` ; `evaluationMode` sur les six checks ; self `ORGANIZATION_SCOPE` / `RESOURCE_OWNERSHIP` / `RESOURCE_ACCESS_POLICY` ; write\|admin informatifs uniquement ; voir [RFC](./RFC-ACL-019%20%E2%80%94%20Diagnostic%20enrichi%20organisation%20et%20acc%C3%A8s.md) |
| **RFC-ACL-020** | Intégration modules métier (ownership + scope) | 📝 Draft | Portage budgets/projets/contrats/fournisseurs/vision/documents sur moteur + ownership, tests anti-fuite ; voir [RFC](./RFC-ACL-020%20%E2%80%94%20Int%C3%A9gration%20modules%20m%C3%A9tier%20ownership%20et%20scope.md) |
| **RFC-ACL-021** | Cockpit « modèle d’accès » admin client | 📝 Draft | KPI santé droits, alertes, actions correctives, UI `/client/administration/access-model` (indicatif) ; voir [RFC](./RFC-ACL-021%20%E2%80%94%20Cockpit%20mod%C3%A8le%20d%27acc%C3%A8s%20admin%20client.md) |
| **RFC-ACL-022** | Migration, backfill et feature flags | 📝 Draft | Scripts idempotents, rapports d’écarts, activation progressive par module ; voir [RFC](./RFC-ACL-022%20%E2%80%94%20Migration%20backfill%20et%20feature%20flags.md) |
