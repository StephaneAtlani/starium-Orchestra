...
| **RFC-STRAT-005** | Stratégie par direction et vision stratégique | ✅ Implémentée (MVP) | Référentiel `StrategicDirection` (client-scoped), `StrategicObjective.directionId`, `GET /api/strategic-vision/kpis/by-direction`, filtre `directionId`/`unassigned=true` sur `/alerts`, cockpit global/par direction/non affecté ; `Project` reste relié via `StrategicLink` (`PROJECT`), sans `directionId` direct sur projet |
