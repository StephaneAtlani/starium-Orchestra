...
| RFC | Nom | État | Commentaire |
| --- | --- | --- | --- |
| **RFC-TEAM-001** | Synchronisation des collaborateurs depuis AD DS | ✅ Implémentée (MVP) | Graph/Entra ; `USER_LINK_REQUIRED` ; `link-platform-user` (+ DELETE) ; UI rattachement via **Membres** `/client/members` (pas `/teams/collaborators`) ; voir [RFC](./RFC-TEAM-001%20%E2%80%94%20Synchronisation%20des%20collaborateurs%20depuis%20AD%20DS.md) |
...
| **RFC-CAPA-001** | Gestion de la capacité des équipes et des collaborateurs | ✅ Implémentée (lots 1-5, UI MVP) | Module `capacity` ; centre = **`WorkTeam`**, personnes = **`Resource` HUMAN** ; API `/api/capacity/*` ; perms `capacity.*` ; UI MVP `/teams/capacity/*` ; reste UI → **[RFC-FE-CAPA-001](./RFC-FE-CAPA-001%20%E2%80%94%20UI%20UX%20Capacit%C3%A9%20%E2%80%94%20compl%C3%A9ments%20post-MVP.md)** ; voir [RFC](./RFC-CAPA-001%20%E2%80%94%20Gestion%20de%20la%20capacit%C3%A9%20des%20%C3%A9quipes%20et%20des%20collaborateurs) |
| **RFC-FE-CAPA-001** | UI/UX Capacité — page unique & compléments | 🟡 En cours | **1 page** `/teams/capacity` (Pilotage \| Affectations \| Réglages) ; redirects anciennes routes ; contrat FE nested + J/H string ; reste : encarts risque/plan, polish DS ; voir [RFC](./RFC-FE-CAPA-001%20%E2%80%94%20UI%20UX%20Capacit%C3%A9%20%E2%80%94%20compl%C3%A9ments%20post-MVP.md) |
...
| **RFC-FE-TEAM-002** | UI Collaborateurs | ✅ Implémentée (MVP FE) | Routes `/teams/collaborators*` → redirect `/client/members` ; rattachement ADDS dans édition membre ; helpers API `features/teams/collaborators` conservés ; voir [RFC](./RFC-FE-TEAM-002%20%E2%80%94%20UI%20Collaborateurs.md) §0 |
...
| 14c′  | **RFC-PROJ-DOC-002** | Silo Documents + rattachements | Onglet Documents (mock), upload/download STARIUM, picker Points | ✅ P0–P2 | [RFC](./RFC-PROJ-DOC-002%20—%20Silo%20Documents%20projet%2C%20upload%20et%20rattachements.md) — supersède DOC-FE-001 ; NTH.3 |
...
| 14d‴  | **RFC-PROJ-013-3** | Cohérence Point projet | Lots 0–E ; C9 → 013-4/013-5+. [Note D1–D7](./RFC-PROJ-013-3%20—%20Note%20arbitrage%20D1-D7.md) | 📝 Draft amendé | [RFC](./RFC-PROJ-013-3%20—%20Cohérence%20Point%20projet%20(CR,%20snapshot,%20lecture,%20UX).md) |
...
| 14d⁵  | **RFC-PROJ-013-5** | Spec écrans Points projet | Catalogue **20 écrans** (historique) ; parcours 8 écrans → **013-10** | 📝 Draft | [RFC](./RFC-PROJ-013-5%20—%20Spécification%20fonctionnelle%20écrans%20Points%20projet%20(catalogue%2001-20).md) |
| 14d⁶  | **RFC-PROJ-013-6** | Animer la séance (09) | Conduite live : présence, ODJ, Présentation/Décision, timers, close-conduct | ✅ Implémenté | [RFC](./RFC-PROJ-013-6%20—%20Animer%20la%20séance%20(écran%2009).md) |
| 14d⁷  | **RFC-PROJ-013-7** | Listes / Séries / Préparation | Socle listes + séries + figer ; écarts CDC → **013-10** | ✅ Socle · 📝 alignement CDC | [RFC](./RFC-PROJ-013-7%20—%20Listes%20par%20état%20Séries%20et%20Préparation%20(écrans%2001-08).md) |
...
| 14d¹⁰ | **RFC-PROJ-013-10** | CDC parcours 8 écrans | Fidélité visuelle create→CR ; P2 ✅ ; P3 convocation (modale + e-mail `.ics`) **en cours** | 📝 Draft · **active** | [RFC](./RFC-PROJ-013-10%20—%20CDC%20parcours%208%20écrans%20Points%20projet%20(fidélité%20visuelle).md) |
...
| 9 | **RFC-PROJ-INT-009** | Sync documents → Teams | One-way Graph Drive, mapping `ProjectDocumentMicrosoftSync` | ✅ Implémenté (backend) | `POST .../microsoft-link/sync-documents` ; **UI pilotage** : bouton depuis **Options projet** (RFC-PROJ-OPT-001) ; statuts / cible Drive : [RFC-PROJ-INT-FE-009](./RFC-PROJ-INT-FE-009%20—%20UI%20cible%20documents%20Teams%20(SharePoint%20OneDrive)%20et%20sync.md) ; lecture fichiers `STARIUM` via stockage documents client (domaine `projets`) |
| 9b | **RFC-PROJ-INT-FE-009** | UI cible documents Teams + sync | Carte Documents (SharePoint/OneDrive) : résolution `filesFolder` canal, libellés, dissocier cible, statuts silo | 📝 Draft | [RFC](./RFC-PROJ-INT-FE-009%20—%20UI%20cible%20documents%20Teams%20(SharePoint%20OneDrive)%20et%20sync.md) — complète INT-009 / OPT-001 ; backlog V11.14 |
| 10 | **RFC-PROJ-INT-010** | Provisioning Teams (mise à disposition M365) | Création équipe + canaux par défaut (options module, création ou vie projet) | 🟡 MVP Teams + lot 2 livrés ; **lot 5 modulaire ❌** (sync 2026-09-16) | [RFC](./RFC-PROJ-INT-010%20—%20Provisioning%20Teams%20et%20canaux%20par%20défaut.md) — livré : Team + canaux, opt-in, `/projects/options`, queue/stale/retry ; **reste** : cases Planner / dossier docs / sync tâches + Prisma `request*` + Graph + UI (§18.3) |
...
| 31a-4b | **RFC-PROJ-ARB-001** | Grille d’arbitrage décideur | Quatre questions vérifiables (date imposée, risque d’inaction, valeur 18 mois, dépendance) + capacité JH ; MUST_DO hors pool ; matrice cycle ; pas d’auto-décision | 📝 Draft | [RFC](./RFC-PROJ-ARB-001%20—%20Grille%20d'arbitrage%20d%C3%A9cideur%20(date%20risque%20valeur%20d%C3%A9pendance%20capacit%C3%A9).md) — remplace le stub [INTAKE-005](./RFC-PROJ-INTAKE-005%20—%20Scoring%20multicrit%C3%A8re%20demandes.md) |
...
| Ordre | RFC | Nom | Description | État | Commentaire |
| ----- | --- | --- | --- | --- | --- |
| 31a-5 | **RFC-PROJ-INTAKE-001** | Demandes projet et workflow de validation configurable | Entité `ProjectRequest`, validation manuelle (validateur désigné), routage (pool cycle `GovernanceCycleItem` / brouillon projet / backlog / manuel), paramètres CLIENT_ADMIN + `defaultGovernanceCycleId`, notifications RFC-038, conversion transactionnelle vers `Project` (`DRAFT`) | 🟢 Implémenté (MVP + 2026-06-09) | [RFC](./RFC-PROJ-INTAKE-001%20%E2%80%94%20Demandes%20projet%20et%20workflow%20de%20validation%20configurable.md) — module `project_requests`, ACL `PROJECT_REQUEST`, UI `/projects/requests` · **UX active** → [INTAKE-002](./RFC-PROJ-INTAKE-002%20—%20CDC%20Demandes%20de%20projet%20(circuit%20configurable%20et%20fidélité%20visuelle).md) |
| 31a-6 | **RFC-PROJ-INTAKE-002** | CDC Demandes de projet (circuit configurable et fidélité visuelle) | Circuit seuils/exempt, machine à états, journal, UI A1–A7, form A2 5 sections + portfolio/SWOT, seed 8 DP | 🟢 Implémenté (P0–P5 + form étendu 2026-09-14) | [RFC](./RFC-PROJ-INTAKE-002%20—%20CDC%20Demandes%20de%20projet%20(circuit%20configurable%20et%20fidélité%20visuelle).md) — migrations `…140000` / `…150000` / `…160000` ; ODJ Cycles + PLA = partiel ; P6–P9 stubs ; config PATCH = admin client |
| 31a-7 | **RFC-PROJ-INTAKE-003** | Chiffrage détaillé depuis demande | Ventilation CAPEX/OPEX optionnelle ; enveloppe Budget à la conversion sans lignes | 📝 Draft (stub P6) | [RFC](./RFC-PROJ-INTAKE-003%20—%20Chiffrage%20détaillé%20depuis%20demande.md) |
| 31a-8 | **RFC-PROJ-INTAKE-004** | Planning skeleton depuis demande | Interdit au demandeur ; skeleton phase/jalon + plan cadrage à la conversion | 📝 Draft (stub P7) | [RFC](./RFC-PROJ-INTAKE-004%20—%20Planning%20skeleton%20depuis%20demande.md) |
| 31a-9 | **RFC-PROJ-INTAKE-005** | Scoring multicritère demandes | ~~Axes configurables~~ → **redirigé** vers grille fixe décideur | 📝 Redirigé | [RFC](./RFC-PROJ-INTAKE-005%20—%20Scoring%20multicritère%20demandes.md) → [**ARB-001**](./RFC-PROJ-ARB-001%20—%20Grille%20d'arbitrage%20d%C3%A9cideur%20(date%20risque%20valeur%20d%C3%A9pendance%20capacit%C3%A9).md) |
| 31a-10 | **RFC-PROJ-INTAKE-006** | Portail externe demandes | Invité authentifié / magic-link ; `source=EXTERNAL` ; pas d’anonyme | 📝 Draft (stub P9) | [RFC](./RFC-PROJ-INTAKE-006%20—%20Portail%20externe%20demandes.md) |
...
| **RFC-FE-BUD-031** | Portefeuille budgets UI (refonte mockup) | ✅ Lot A implémenté | [RFC](./RFC-FE-BUD-031%20%E2%80%94%20Portefeuille%20budgets%20UI%20(refonte%20mockup).md) — `/budgets` portefeuille (KPI consol 5 cellules, cartes/table colorées, toggle URL, export CSV, `components/portfolio`) ; **Lot B fiche** → [RFC-FE-BUD-032](./RFC-FE-BUD-032%20%E2%80%94%20Fiche%20budget%20cockpit%20(refonte%20pr%C3%A9sentation%20%26%20fonctionnalit%C3%A9s).md) |
| **RFC-FE-BUD-032** | Fiche budget cockpit (refonte présentation & fonctionnalités) | ✅ Implémenté (lots 1→4 + F + polish mockup) | [RFC](./RFC-FE-BUD-032%20%E2%80%94%20Fiche%20budget%20cockpit%20(refonte%20pr%C3%A9sentation%20%26%20fonctionnalit%C3%A9s).md) — `/budgets/[budgetId]` : `PageHeader`, bande 6 KPI, Vue d’ensemble (graphique **Réalisé vs prévu** 12 mois), outils Version figée / Comparaisons / Prévisionnel / Réaffectations / Accès, saisie **Engagement / commande** · **Consommé / Facture**, export CSV client, dénouement engagé à la facturation |
...
| **RFC-BUD-040** | Unification atterrissage, prévision et forecast | ✅ Implémenté (2026-08-28) | [RFC](./RFC-BUD-040%20%E2%80%94%20Unification%20atterrissage%2C%20pr%C3%A9vision%20et%20forecast.md) — moteur `BudgetLandingService`, KPI **Atterrissage**, onglet **Prévisionnel** conservé, fusion `/reporting` → fiche Comparaisons |
| **RFC-BUD-041** | Gouvernance mid-year, PA (prévision d'atterrissage) et arbitrage | 🟡 Partiel (lots 1–5) · lot 6 ouvert | [RFC](./RFC-BUD-041%20%E2%80%94%20Ajouts%20structurels%20en%20cours%20d'exercice%20et%20gouvernance%20pr%C3%A9visionnel-atterrissage.md) — glossaire **PA** vs **atterrissage** vs **plan 12 mois** ; cycle version figée → scénario arbitré → comparaison → **activation** ; intentions A/B/C ; `?onglet=pa` hors tablist |
| **RFC-018** | Budget Data Import (moteur) | ✅ Backend · 🟡 UI wizard | [RFC](./RFC-018%20%E2%80%94%20Budget%20Data%20Import.md) — analyze / preview / execute / mappings / anti-doublon ; wizard `/budgets/[budgetId]/import` ; produit & hub → **RFC-BUD-043** |
| **RFC-BUD-043** | Import CSV/Excel — centre de gestion (Configuration → Imports) | 🟡 Partiel (L1–L2) | [RFC](./RFC-BUD-043%20%E2%80%94%20Import%20budg%C3%A9taire%20CSV%20Excel%20%E2%80%94%20centre%20de%20gestion.md) — hub `/budgets/imports` (Profils / Historique / Aide CSV), API jobs + template, profils `importPurpose` / `defaultBudgetId`, wizard `?profileId=` ; L3 KPI / timeline hors lot |
| **RFC-BUD-044** | Automatisation des imports budgétaires | 📝 Draft | [RFC](./RFC-BUD-044%20%E2%80%94%20Automatisation%20des%20imports%20budg%C3%A9taires.md) — cron, queue BullMQ, SFTP/webhook (lots), notifications RFC-038 ; dépend RFC-BUD-043 |
...
> **Roadmap** (beta → Fin 2026 / 2027) : [ROADMAP.md](../ROADMAP.md).  
> **Backlog** (reste à faire) : [BACKLOG.md](../BACKLOG.md).
...
| **RFC-PROJ-010-B** | **Vue inverse BudgetLine → projets + KPI cockpit** (implémentée MVP ; dashboard hors V1) |
...
| **RFC-STRAT-010** | Vision stratégique V1 — Plan de tests et trajectoire de delivery | ✅ Implémentée (V1) | Layout cockpit, StrategicLink PROJECT (panneau + Aligner non-alignés), invalidations KPI/alertes, isolation cross-client, `audit:ui-ids` / `audit:modals` |
| **RFC-STRAT-011** | CDC Stratégie de direction et Consolidé groupe (fidélité mock) | ✅ Implémentée | [RFC](./RFC-STRAT-011%20%E2%80%94%20CDC%20Strat%C3%A9gie%20de%20direction%20et%20Consolid%C3%A9%20groupe%20(fid%C3%A9lit%C3%A9%20mock).md) — portefeuille, fiche, consolidé, sponsor/caps, options `allowSelfValidation` ; pièces document S3 ; KPI DnD + synchro OKR ; axes retenus Alignement ; sans CTA revue croisée |
...
| **RFC-ADM-001** | Référentiels plateforme (Admin Studio) | ❌ Abandonnée | Listes génériques retirées — remplacé par **RFC-ADM-002** — [RFC](./RFC-ADM-001%20%E2%80%94%20R%C3%A9f%C3%A9rentiels%20plateforme%20(Admin%20Studio).md) |
| **RFC-ADM-002** | Catalogue référentiels conformité (plateforme) | ✅ Implémentée (MVP) | `ComplianceFramework` `clientId=null`, CRUD `/api/platform/compliance/frameworks`, import CISO `/api/platform/compliance/ciso-libraries`, activation client `GET …/catalog` + `POST …/activate`, UI `/admin/compliance-frameworks` — [RFC](./RFC-ADM-002%20%E2%80%94%20Catalogue%20r%C3%A9f%C3%A9rentiels%20conformit%C3%A9%20(plateforme).md) |
| **RFC-COMP-001** | Pilotage conformité (cible produit) | 📝 Draft | Cible GRC complète ; livraison immédiate via **001-A** — [RFC](./RFC-COMP-001%20%E2%80%94%20Pilotage%20de%20la%20conformit%C3%A9.md) · [écarts MVP](./RFC-COMP-001-ecarts-mvp.md) |
...
| **RFC-COMP-002** | Campagnes et dossier d’audit (V2) | 🚧 V2.1 (campagnes + instantanés) | CRUD campagnes, snapshot JSON, UI revue — [RFC](./RFC-COMP-002%20%E2%80%94%20Campagnes%20et%20dossier%20d'audit.md) |
| **RFC-COMP-003** | CDC Conformité (fidélité mock) | ✅ UX.0–3 · suite COMP-002 | Fiche `/compliance/frameworks/[id]` + overview API + remédiation — [RFC](./RFC-COMP-003%20%E2%80%94%20CDC%20Conformit%C3%A9%20(fid%C3%A9lit%C3%A9%20mock).md) |
