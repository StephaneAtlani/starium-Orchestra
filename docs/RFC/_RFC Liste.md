...
| 10 | **RFC-PROJ-INT-010** | Provisioning Teams | Création équipe + canaux par défaut (options module, création ou vie projet) | ✅ Implémenté (MVP) | [RFC](./RFC-PROJ-INT-010%20—%20Provisioning%20Teams%20et%20canaux%20par%20défaut.md) — settings/templates client (`/projects/options`), queue BullMQ + worker, routes `.../microsoft-teams/provision*`, case création projet, cohabitation INT-007 ; suites : tests controller/FE intégrés, `aria-live` provisioning |
