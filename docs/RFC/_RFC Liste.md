...
| 10 | **RFC-PROJ-INT-010** | Provisioning Teams | Création équipe + canaux par défaut (options module, création ou vie projet) | 🟡 MVP Teams livré (lot 1 gate OK) ; lot 5 modulaire planifié | [RFC](./RFC-PROJ-INT-010%20—%20Provisioning%20Teams%20et%20canaux%20par%20défaut.md) — MVP : Team + canaux, opt-in création ; référentiel `/projects/options` (table + dialog) ; **lot 5** : cases Planner / dossier documents / sync tâches |
