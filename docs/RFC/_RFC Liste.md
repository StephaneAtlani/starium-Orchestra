...
| **RFC-TEAM-009** | Temps réalisé (socle API + FE liste) | ✅ Implémentée (socle) | `ResourceTimeEntry`, `/api/resource-time-entries` ; RBAC **`resources.read`** / **`resources.update`** ; pas de fichier RFC Markdown dédié — détail [API.md](../API.md) (section temps réalisé), [ARCHITECTURE.md](../ARCHITECTURE.md) (module `resource-time-entries`) ; alignement **RFC-TEAM-020** |
...
| **RFC-FE-TEAM-005** | UI Affectations & staffing projet | ✅ Implémentée (MVP FE) | `apps/web/src/features/teams/team-assignments/` ; `/teams/assignments`, `/projects/[projectId]/staffing` (onglet **Charge**) ; filtres / body **`resourceId`** (TEAM-020) ; `/teams/time-entries` + `features/teams/resource-time-entries/` (RFC-TEAM-009) ; consomme TEAM-007 / TEAM-008 ; `activity_types.read` requis pour les selects / écritures côté UI ; encart collaborateur & `projectTeamRoleId` → P1 ; voir [RFC](./RFC-FE-TEAM-005%20%E2%80%94%20UI%20Affectations%20%26%20staffing%20projet.md) |
