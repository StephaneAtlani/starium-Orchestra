...
| **RFC-ACL-015** | Permissions `OWN` / `SCOPE` / `ALL` | 📝 Draft | Registre permissions RBAC par périmètre (`read_own`, `read_scope`, `read_all`, etc.), seeds + profils, compatibilité codes existants ; voir [RFC](./RFC-ACL-015%20%E2%80%94%20Permissions%20OWN%20SCOPE%20ALL.md) |
| **RFC-ACL-016** | Résolution du scope organisationnel | 📝 Draft | `OrganizationScopeService`, verdict `NONE`/`OWN`/`SCOPE`/`ALL` + `reasonCodes` ; ORG-002 (lien HUMAN) **MVP livré** ; dépend encore ORG-003 + ACL-015 ; voir [RFC](./RFC-ACL-016%20%E2%80%94%20R%C3%A9solution%20du%20scope%20organisationnel.md) |
| **RFC-ACL-017** | Politique d’accès ressource (DEFAULT / RESTRICTIVE / SHARING) | 📝 Draft | `ResourceAccessPolicy`, compatibilité sémantique ACL restrictive vs partage explicite, migration défaut `DEFAULT` ; voir [RFC](./RFC-ACL-017%20%E2%80%94%20Politique%20d%27acc%C3%A8s%20ressource.md) |
