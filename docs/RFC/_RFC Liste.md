...
| **RFC-ACL-016** | Résolution du scope organisationnel | 🟡 Service livré — non appliqué aux décisions d’accès | `OrganizationScopeService` (`apps/api/src/common/organization/`), verdict `NONE`/`OWN`/`SCOPE`/`ALL` + `reasonCodes` typés (`ALL_RBAC_OVERRIDE`, `OWN_SELF_MATCH`, `SCOPE_DIRECT_MATCH`, `SCOPE_DESCENDANT_MATCH`, `SCOPE_OWNER_ORG_UNIT_INACTIVE`, `SCOPE_OUT_OF_SUBTREE`, `SCOPE_NO_MEMBERSHIPS`, `SCOPE_NO_RESOURCE_LINK`, `OWN_NO_RESOURCE_LINK`, `OWN_NO_HINT`, `OWN_MISMATCH`, `MISSING_OWNER_ORG_UNIT`) ; BFS in-memory sur arbre actif + cache requête (`resolvedOrgScopeContext`, `resolvedOrgUnitTreeByClient`) ; **OWN** = `subjectResourceId === ClientUser.resourceId` (self HUMAN) ; **SCOPE** = descendants des memberships actives ; **ALL** = `hasAllOverride` pré-calculé par l’appelant (aucun couplage RBAC) ; enforcement réel = [RFC-ACL-018](./RFC-ACL-018%20%E2%80%94%20Moteur%20de%20d%C3%A9cision%20cible.md), diagnostic = [RFC-ACL-019](./RFC-ACL-019%20%E2%80%94%20Diagnostic%20enrichi%20organisation.md) ; voir [RFC](./RFC-ACL-016%20%E2%80%94%20R%C3%A9solution%20du%20scope%20organisationnel.md) |
| **RFC-ACL-017** | Politique d’accès ressource (DEFAULT / RESTRICTIVE / SHARING) | ✅ Implémentée (V1) | Prisma `ResourceAccessPolicy` ; `evaluateResourceAccessDecision` ; `GET` + `PATCH .../access-policy` ; réponse liste `accessPolicy` + `effectiveAccessMode` ; audit `resource_access_policy.changed` ; UI éditeur ; hors périmètre : moteur unifié RFC-ACL-018 ; [RFC](./RFC-ACL-017%20%E2%80%94%20Politique%20d%27acc%C3%A8s%20ressource.md) |
| **RFC-ACL-018** | Moteur de décision d’accès unifié | 📝 Draft | `AccessDecisionService`, guard/decorators, ordre licence → module → RBAC → org → policy/ACL ; voir [RFC](./RFC-ACL-018%20%E2%80%94%20Moteur%20de%20d%C3%A9cision%20d%27acc%C3%A8s%20unifi%C3%A9.md) |
