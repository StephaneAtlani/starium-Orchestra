...
| **RFC-ACL-014** | Conformité modèle Rôles+Groupes+ACL (5 couches) | 📝 Draft | Garde-fou backend `last ADMIN lockout` (409, `force=true` `PLATFORM_ADMIN` only) ; diagnostic self-service `GET /api/access-diagnostics/effective-rights/me` ouvert `CLIENT_USER` (anti-fuite cross-client) ; `<AccessExplainerPopover />` partout sur les fiches métier ; page d’aide `/client/help/access-model` + `docs/ACCESS-MODEL.md` ; `/me/permissions` enrichi `roles[]` (id/name/scope) ; renommage UI « Permissions » → « Accès à la ressource » ; aucune migration ; voir [RFC](./RFC-ACL-014%20%E2%80%94%20Conformit%C3%A9%20mod%C3%A8le%20R%C3%B4les%2C%20Groupes%20et%20ACL.md) |
