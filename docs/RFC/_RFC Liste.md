...
| 10 | **RFC-PROJ-INT-010** | Provisioning Teams | Création équipe + canaux par défaut (options module, création ou vie projet) | 🟡 MVP Teams + **lot 2 backend** (fiabilisation queue / stale / resolve / retry) ; lot 5 modulaire planifié | [RFC](./RFC-PROJ-INT-010%20—%20Provisioning%20Teams%20et%20canaux%20par%20défaut.md) — MVP : Team + canaux, opt-in création ; référentiel `/projects/options` ; maintenance stale BullMQ ; **lot 5** : cases Planner / dossier documents / sync tâches |
...
| **RFC-STRAT-011** | CDC Stratégie de direction et Consolidé groupe (fidélité mock) | ✅ Implémentée | [RFC](./RFC-STRAT-011%20%E2%80%94%20CDC%20Strat%C3%A9gie%20de%20direction%20et%20Consolid%C3%A9%20groupe%20(fid%C3%A9lit%C3%A9%20mock).md) — portefeuille `.stg-card`, fiche schéma, consolidé, PDF print ; **sponsor** create/update/submit/adapt/archive ; flags `canEditContent`… ; CTA Partager / Nouvelle version / Archiver ; options `allowSelfValidation` |
