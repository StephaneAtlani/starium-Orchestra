...
| **RFC-CAPA-001** | Gestion de la capacité des équipes et des collaborateurs | ✅ Implémentée (lots 1-5, UI MVP) | Module `capacity` ; centre = **`WorkTeam`**, personnes = **`Resource` HUMAN** ; API `/api/capacity/*` ; perms `capacity.read` / `settings.manage` / `members.manage` / `allocations.manage` ; UI `/teams/capacity/*` + switch fiche projet ; écarts documentés (ACL RISK/PLAN, contrat FE nested `workTeam`/`resource`, tests isolation) ; socle TEAM-011 ; voir [RFC](./RFC-CAPA-001%20%E2%80%94%20Gestion%20de%20la%20capacit%C3%A9%20des%20%C3%A9quipes%20et%20des%20collaborateurs) |
