...
- Select « Modèle ODJ » actif à la création de point (seed ODJ depuis payload template — option B)

### Livré (option C) — lien permanent équipe ↔ modèle de point

- Champ `ProjectGovernanceCircle.prepareTemplateId` → `ProjectReviewPrepareTemplate` (`ON DELETE SET NULL`)
- API teams : `prepareTemplateId` en create/update ; réponse `prepareTemplateName` / `prepareTemplateTypeCode` ; COPIL/COPROJ n’acceptent qu’un modèle de même `typeCode` ; isolation client
- UI : select « Modèle de point » dans l’éditeur d’équipes ; auto-application du modèle lié à l’ouverture de **Préparer** ; libellé du modèle lié affiché (disabled) à la création

**V2 candidate** : rôles additionnels (signature CR / relances) ; templates RASCI par type d’équipe ; wire complet création (option B).
...
| Technique | Enrichir cercles + API teams + convene ; RASCI R1 (`identityKey`) ; UI onglet + modale DS + matrice ; lien modèle prepare (C) |
| Statut | ✅ Implémentée (Vague A + Vague B + option C) |
---
