...
| `--neutral-500` | `#8C8579` | Bordures légères — **pas** placeholder ni texte de champ |
| `--color-input-text` | `#0E0E10` | Valeur saisie (`Input`, `Textarea`, `Select` sélectionné) — `--brand-ink` |
| `--color-input-placeholder` | `#14130F` | Placeholder / select vide — alias `--color-text-muted` |
| `--neutral-600` | `#5F5A52` | Texte tertiaire, badges neutres — **pas** le token principal du texte secondaire |
| `--neutral-700` | `#403D38` | Texte tertiaire, labels discrets — **pas** le token principal du texte secondaire |
| `--neutral-800` | `#26241F` | Texte tertiaire, labels discrets — **pas** le token principal du texte secondaire |
| `--neutral-900` | `#14130F` | Texte muted / métadonnées (`--color-text-muted`, `--fg-3`, `.starium-text-muted`) |
...
color: var(--neutral-900); /* ou var(--fg-3) — texte secondaire lisible */
padding: 10px 14px;
...
| Artefact | Rôle |
|----------|------|
| `apps/web/src/styles/tokens.css` | Tokens DS + `--ds-*` (rayon, ombre, KPI) + `--control-*` (boutons, chips, segmented, switch) |
| `apps/web/src/app/globals.css` | Pont shadcn, `.starium-card`, `.starium-module`, `.starium-kpi-card`, `.starium-kpi-strip`, `.starium-filter-bar` / `.starium-filter-chip`, `.starium-tab-group` / `.starium-tab-btn`, `.starium-panel`, `.starium-projects-table`, remap `@theme` |
| `apps/web/src/components/ui/button-variants.ts` | Variantes CVA boutons (pilule encre / outline / ghost / destructive) |
...
| [FRONTEND_UI-UX.md](../FRONTEND_UI-UX.md) §2.1 / §5 | Règles « pas de cadre dans cadre », charte contrôles, patterns cockpit |
