...
├── contracts
├── licenses
├── access-control (RFC-ACL-005 — `ResourceAcl`, `GET|PUT|POST|DELETE /api/resource-acl/...` ; **GET** = **CLIENT_ADMIN** + `ActiveClientGuard` ; **mutations** = RFC-ACL-014 Option A `ActiveClientOrPlatformContextGuard` + `ClientAdminOrPlatformAdminGuard` + query `force` réservée `PLATFORM_ADMIN` ; lockout + audits `resource_acl.*` ; `AccessControlModule` dans **`AppModule`** — pas dans `CommonModule` ; `forwardRef` vers diagnostics ; branchement routes métier = RFC-ACL-006)
