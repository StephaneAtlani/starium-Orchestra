...
│   │   ├── governance-cycles   (RFC-PROJ-CYCLE-001 backend B1–B8 ; RFC-FE-PROJ-CYCLE-001 UI `/cycles` — arbitrage CODIR)
│   │   └── ...
│   └── prisma / PostgreSQL
│
└── web/   → Next.js frontend
    ...
    ├── features/governance-cycles   (`/cycles`, `/cycles/[cycleId]` — liste, détail, matrice arbitrage ; RFC-FE-PROJ-CYCLE-001)
...
**Module `governance-cycles` (RFC-PROJ-CYCLE-001 — lots B1–B8 ; RFC-FE-PROJ-CYCLE-001 UI)** : API `/api/governance-cycles` — CRUD cycles + CRUD items sous `/api/governance-cycles/:id/items` (PATCH édition vs arbitrage séparés : body mixte → **400** ; scores en groupe édition). Scoring `priorityScore` : formule RFC §4.5 ; recalcul create systématique ; recalcul update **uniquement** si le body contient une clé score (`hasScorePatch` / `hasOwnProperty`, y compris `null` pour effacement) ; `priorityScore` jamais accepté en body write. Tri liste `priorityScore desc` (`nulls: 'last'`). Deux contrats summary : **embarqué** (3 compteurs sur liste/détail cycle) et **KPI global** `GET /api/governance-cycles/:id/summary` (`GovernanceCycleGlobalSummaryDto` — compteurs par statut, totaux budget/capacité, moyenne `priorityScore`, items à haut risque ; ≤ 3 requêtes Prisma parallèles ; isolation `clientId`). **PATCH cycle** : diff réel (no-op sans Prisma ni audit si valeurs inchangées) ; transition **`TO_ARBITRATE`** → validation CODIR (`validatedAt`, `validatedByUserId` obligatoire, refus si items `CANDIDATE`) ; transition **`CLOSED`** → clôture (`closedAt`, refus si cycle vide ou items `CANDIDATE` / `TO_ARBITRATE`). Permissions `governance_cycles.read|create|update|delete|arbitrate` ; module RBAC `governance_cycles`. Audits cycle : `governance_cycle.created|updated|archived|validated|closed` ; audits item : `governance_cycle_item.created|updated|deleted|decision_changed` (snapshot inclut les 6 champs score). **Hors scope livré** : `GET …/by-project/:projectId` (RFC-PROJ-CYCLE-002 / B9). Code API : `apps/api/src/modules/governance-cycles/` (`governance-cycle-scoring.util.ts`, specs DTO, 78 tests module). **UI** : `apps/web/src/features/governance-cycles/` — routes `/cycles`, `/cycles/[cycleId]` ; TanStack Query avec `clientId` dans les query keys ; liste (synthèse page + `useQueries` summary), détail multi-onglets, matrice arbitrage sans recalcul `priorityScore` côté React ; add-item V1 `PROJECT`/`BUDGET`/`MANUAL`. Détail API : [docs/API.md](API.md) §5.8.
