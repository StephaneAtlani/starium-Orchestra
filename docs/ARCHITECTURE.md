...
- **RBAC codes (RFC-ACL-015 — socle)** : `EffectivePermissionsService` (`apps/api/src/common/services/effective-permissions.service.ts`) agrège les `Permission.code` des rôles en un **Set brut** (cache par requête). Les guards `ModuleAccessGuard` et `PermissionsGuard` (`apps/api/src/common/guards/`), le diagnostic d’accès, la recherche et le chatbot appellent **`satisfiesPermission`** du package workspace **`@starium-orchestra/rbac-permissions`** (`packages/rbac-permissions/`) — équivalences **documentées et restrictives** avant RFC-016/018 (ex. `*.read_all` peut ouvrir un décorateur legacy `*.read` sur les modules catalogue ; `read_scope` / `read_own` **non**). `GET /api/me/permissions` renvoie ces codes bruts + `uiPermissionHints` (affichage seul). Détail : [RFC-ACL-015](RFC/RFC-ACL-015%20%E2%80%94%20Permissions%20OWN%20SCOPE%20ALL.md), [ACCESS-MODEL.md](ACCESS-MODEL.md).
