...
**Scénarios projet (RFC-PROJ-SC-001…007, RFC-FE-PROJ-SC-001, RFC-FE-PROJ-SC-002)** : API `GET|POST /api/projects/:projectId/scenarios`, `GET|PATCH /api/projects/:projectId/scenarios/:scenarioId`, sous-routes financier / ressources / planning / capacité / risques / sélection (`project-scenarios` côté Nest). UI : liste `/projects/[projectId]/scenarios` ; **cockpit comparaison** `/projects/[projectId]/scenarios/cockpit` (`apps/web/src/features/projects/scenario-cockpit/`), données via détail scénario (synthèses agrégées), isolation client identique au reste du module.
