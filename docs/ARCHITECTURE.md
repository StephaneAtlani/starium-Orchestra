...
│   │   ├── governance-cycles   (RFC-PROJ-CYCLE-001 — cycles de pilotage / arbitrage CODIR ; lots B1–B4 : CRUD cycles)
│   │   └── ...
│   └── prisma / PostgreSQL
│
...
**Module `governance-cycles` (RFC-PROJ-CYCLE-001 — lots B1–B4)** : API `/api/governance-cycles` — liste paginée `{ items, total, limit, offset }`, CRUD cycle, archivage logique `DELETE` → `status = ARCHIVED` (**204** sans corps). Champ `summary` (`itemsCount`, `acceptedItemsCount`, `deferredItemsCount`) calculé côté serveur sur `GovernanceCycleItem`. Permissions `governance_cycles.read|create|update|delete|arbitrate` ; module RBAC `governance_cycles` ; seed + profils dans `default-profiles.json`. Mêmes guards que le métier (`JwtAuthGuard`, `ActiveClientGuard`, `ModuleAccessGuard`, `PermissionsGuard`) ; `clientId` jamais en DTO write. Audits : `governance_cycle.created|updated|archived`. **Hors scope livré** : routes items, scoring, `GET …/:id/summary` global, `GET …/by-project/:projectId` (RFC-002). Code : `apps/api/src/modules/governance-cycles/`. Détail API : [docs/API.md](API.md) §5.8.
