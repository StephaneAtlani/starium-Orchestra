...
**Module `governance-cycles` (RFC-PROJ-CYCLE-001 B1–B9 ; RFC-PROJ-CYCLE-002 ; RFC-FE-PROJ-CYCLE-001 UI)** : API `/api/governance-cycles` — CRUD cycles + CRUD items sous `/api/governance-cycles/:id/items` (PATCH édition vs arbitrage séparés : body mixte → **400** ; scores en groupe édition). Scoring `priorityScore` : formule RFC §4.5 ; recalcul create systématique ; recalcul update **uniquement** si le body contient une clé score (`hasScorePatch` / `hasOwnProperty`, y compris `null` pour effacement) ; `priorityScore` jamais accepté en body write. Tri liste `priorityScore desc` (`nulls: 'last'`). Deux contrats summary : **embarqué** (3 compteurs sur liste/détail cycle) et **KPI global** `GET /api/governance-cycles/:id/summary` (`GovernanceCycleGlobalSummaryDto`). **Intégration projet** : `GET /api/governance-cycles/by-project/:projectId` (read-only, cycles non archivés, `periodLabel` API) — **RFC-PROJ-CYCLE-002**. **PATCH cycle** : diff réel (no-op sans Prisma ni audit si valeurs inchangées) ; transitions `TO_ARBITRATE` / `CLOSED`. Permissions `governance_cycles.read|create|update|delete|arbitrate` ; module RBAC `governance_cycles`. Audits cycle/item RFC §6. Code API : `apps/api/src/modules/governance-cycles/` (88 tests module). **UI** : `apps/web/src/features/governance-cycles/` — routes `/cycles`, `/cycles/[cycleId]` ; bloc fiche projet `ProjectGovernanceCyclesPresenceBlock` sur `/projects/[id]` (masqué sans `governance_cycles.read`, query `enabled: false`). Détail API : [docs/API.md](API.md) §5.8.
