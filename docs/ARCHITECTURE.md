...
├── work-teams (RFC-TEAM-005 — `WorkTeam`, rattachements, périmètres managers ; permissions `teams.*`)
├── activity-types (RFC-TEAM-006 — `ActivityType`, taxonomie charge / staffing ; permissions `activity_types.*`)
├── team-assignments (RFC-TEAM-007 — `TeamResourceAssignment`, staffing planifié ; permissions `team_assignments.*`)
├── projects
...
**Module `work-teams` (RFC-TEAM-005)** : API `/api/work-teams` (CRUD équipes, `GET /work-teams/tree` — réponse `{ nodes }`, archive/restore), membres sous `/api/work-teams/:id/members`, `/api/manager-scopes/:managerCollaboratorId` (+ `preview`), route inverse `GET /api/collaborators/:id/work-teams`. Permissions `teams.read`, `teams.update`, `teams.manage_scopes` ; même pipeline guards que le reste du métier (`JwtAuthGuard`, `ActiveClientGuard`, `ModuleAccessGuard`, `PermissionsGuard`). Toutes les lectures/écritures filtrent par `clientId` du contexte client actif. Module Prisma `teams` activé par client via `ClientModule` (seed). Code : `apps/api/src/modules/work-teams/`.
...
**Module `team-assignments` (RFC-TEAM-007 / RFC-TEAM-008)** : API `/api/team-resource-assignments` — liste paginée `{ items, total, limit, offset }`, CRUD, `POST …/:id/cancel` (annulation logique idempotente). **RFC-TEAM-008** : routes projet-scopées `/api/projects/:projectId/resource-assignments` (contrôleur dans `projects/`, même service `TeamAssignmentsService`, mêmes permissions `team_assignments.read` / `team_assignments.manage` — pas de `projects.*` sur ces handlers). Permissions `team_assignments.read`, `team_assignments.manage` ; mêmes guards que ci-dessus. Modèle Prisma `TeamResourceAssignment` (liens `Collaborator`, `ActivityType`, `Project?`, `ProjectTeamRole?`) ; isolation stricte par `clientId`. Module Prisma `team_assignments` activé par client via `ClientModule` (seed). Code : `apps/api/src/modules/team-assignments/`, `apps/api/src/modules/projects/project-resource-assignments.controller.ts`. Détail API : [docs/API.md](API.md) (sections Équipes — affectations ressources et affectations projet-scopées).
