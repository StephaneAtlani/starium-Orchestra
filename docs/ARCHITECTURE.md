...
│   │   ├── meetings   (RFC-MEET-001 — réunions de gouvernance CODIR/COPIL/COPRO ; **surcouche** : ne duplique aucune donnée métier ; lot A livré)
│   │   └── ...
│   └── prisma / PostgreSQL
│
└── web/   → Next.js frontend
    ...
    ├── features/capacity (RFC-CAPA-001 — UI `/teams/capacity/*`, API `/api/capacity`)
...
- **Catalogue conformité plateforme (RFC-ADM-002)** : cadres `ComplianceFramework` à `clientId = null` — CRUD `/api/platform/compliance/frameworks` + import CISO `/api/platform/compliance/ciso-libraries` (`PLATFORM_ADMIN`) ; UI `/admin/compliance-frameworks`. Les clients lisent le catalogue (`GET /api/compliance/frameworks/catalog`) et activent une **copie** isolée (`POST /api/compliance/frameworks/activate`).
- **Évaluation conformité client (RFC-COMP-001-A)** : module `apps/api/src/modules/compliance/` — `PUT /api/compliance/requirements/:id/status` (upsert + `assertEvaluationTransition`), `POST /api/compliance/evidence` (`kind` dérivé V1), `GET /api/compliance/dashboard` (`applicableCount` `A`, `compliancePercent` = `C/A` ou `null`). UI client : `/compliance/dashboard`, `/compliance/frameworks`, `/compliance/requirements` (modale évaluer + preuves + CTA risque). Fidélité mock / suite UX : [RFC-COMP-003](RFC/RFC-COMP-003%20—%20CDC%20Conformité%20(fidélité%20mock).md). Isolation : toujours `clientId` du scope actif.
...
Le **prévisionnel mensuel** par ligne (`BudgetLinePlanningMonth` indexé 1–12, champs `planningMode` / `planningTotalAmount` sur `BudgetLine`) et le calcul d’**atterrissage** (consommé + engagé + prévision restante) sont décrits dans [RFC-023 — Budget Prévisionnel (Planning & Atterrissage)](./RFC/RFC-023%20%E2%80%94%20Budget%20Pr%C3%A9visionnel%20(Planning%20%26%20Atterrissage).md). Depuis [RFC-BUD-040](./RFC/RFC-BUD-040%20%E2%80%94%20Unification%20atterrissage%2C%20pr%C3%A9vision%20et%20forecast.md), l’atterrissage canonique est **`BudgetLine.landingAmount`** (persisté par `BudgetLandingService` dans `apps/api/src/modules/budget-landing/`) ; **`forecastAmount`** reste synchronisé comme alias de transition. La logique d’alignement des mois sur `BudgetExercise.startDate` est partagée via le package **`@starium-orchestra/budget-exercise-calendar`** (`packages/budget-exercise-calendar/`).
...
├── capacity (RFC-CAPA-001 — calendrier / allocations / dashboards ; centre = `WorkTeam` ; permissions `capacity.*` ; module catalogue `capacity`)
├── meetings (RFC-MEET-001 — réunions de gouvernance ; modèles de rituel, périmètre multi-projets, sections ; permissions `meetings.*` ; module catalogue `meetings`)
...
**Socle alertes / notifications in-app / email async (RFC-038)** : modèles Prisma `Alert`, `Notification`, `EmailDelivery` ; modules Nest `alerts`, `notifications`, `queue` (BullMQ + **Redis** : `REDIS_HOST`, `REDIS_PORT`, `REDIS_PASSWORD` optionnel), `email` ; worker applicatif **sans serveur HTTP** : `apps/api/src/worker/main.ts` (`pnpm start:worker` depuis `apps/api` ; service Compose **`api-worker`** en prod). API : `GET /api/alerts`, `POST /api/alerts/evaluate` (recalcul budget/projet/contrats, `alerts.update`), `PATCH /api/alerts/:id/resolve`, `PATCH /api/alerts/:id/dismiss` ; `GET /api/notifications` (réponse paginée + champ `unread` pour badge), `PATCH /api/notifications/:id/read`, `PATCH /api/notifications/read-all`. Même pipeline guards que le métier (`JwtAuthGuard`, `ActiveClientGuard`, `ModuleAccessGuard`, `PermissionsGuard`) ; isolation stricte `clientId` et pour les notifications **filtrage `userId` courant**. **Socle RBAC implicite** : `EffectivePermissionsService` injecte `notifications.read`, `notifications.update`, `alerts.read` à tout utilisateur authentifié dans un contexte client ; `alerts.update` (résolution / rejet / recalcul) via rôle seed « Client admin — alertes ». **Modules socle** `notifications` et `alerts` : dans `ModuleAccessGuard`, non désactivables via `ClientModule` ni masquage profil (RFC-ACL-004) — seul `Module.isActive = false` (plateforme) bloque. Triggers : `AlertsTriggerService` (budget, projet, contrats) + `AlertsTriggerSchedulerService` (cron `ALERTS_TRIGGER_CRON_EXPRESSION`, défaut `0 * * * *` UTC) ; `resolveStaleByRule` pour résolution auto des alertes obsolètes. Seed prod : `ensureAlertsNotificationsBaselineRole` + `ensureAlertsAndNotificationsModulesAndPermissions` (`ALLOW_PROD_SEED=true`). UI : cloche dans `apps/web/src/components/shell/workspace-header.tsx`, panel alertes critiques minimal sur `/dashboard`. Ne pas confondre avec `GET /api/strategic-vision/alerts` (alertes métier vision). Détail : [RFC-038](RFC/RFC-038%20%E2%80%94%20Socle%20alertes%20et%20emails%20async.md), [API.md](API.md) §5.7.

> ⚠️ **`WorkerModule` est un contexte applicatif racine distinct de `AppModule`.** Un module `@Global()` n'est global **que dans le contexte qui l'importe** : ce qu'`AppModule` importe n'est pas visible du worker. Tout module global dont un provider du worker dépend — directement ou **par transitivité** — doit donc être importé **explicitement** dans `apps/api/src/worker/worker.module.ts`. Oubli constaté en production (2026-07-27) : `AuthCommonModule` manquant, `MicrosoftModule → AuthModule → UsersModule → CollaboratorsModule` ne résolvant plus `EmailReservationService` — l'API démarrait, le service `api-worker` non. Le `typecheck` et les tests ne couvrent pas ce cas : **ajouter un import au worker se vérifie en démarrant `node dist/worker/main.js`**.
...
**Module `capacity` (RFC-CAPA-001)** : API `/api/capacity` — calendrier mensuel client, exceptions membre, équipe primaire (`Resource.primaryCapacityWorkTeamId` → `WorkTeam`), allocations + dashboards ; `consumesCapacity` sur `Project` / `ProjectRisk` / `ActionPlan`. Centre = **`WorkTeam`** ; personnes = **`Resource` HUMAN**. Permissions `capacity.read` / `capacity.settings.manage` / `capacity.members.manage` / `capacity.allocations.manage` ; module catalogue **`capacity`**. Isolation stricte par client actif. UI `/teams/capacity/*` (`apps/web/src/features/capacity/`). Détail : [docs/API.md](API.md) (section Capacité), [RFC-CAPA-001](RFC/RFC-CAPA-001%20—%20Gestion%20de%20la%20capacité%20des%20équipes%20et%20des%20collaborateurs). Code : `apps/api/src/modules/capacity/`.
...
**Module `meetings` (RFC-MEET-001 — lot A livré)** : API `/api/meetings` + `/api/meeting-templates` — réunions de gouvernance (CODIR, COPIL, COPRO, revue projet, revue budgétaire, comité risques, arbitrage, point de crise, post-mortem). **Architecture en surcouche, trois règles non négociables** : (1) aucune donnée métier dupliquée — risques, phases, avancement, alertes, budget consommé et capacité sont **lus** via les services existants et figés dans `snapshotPayload` à la seule finalisation ; (2) la trace **par projet** reste dans `ProjectReview` (`MeetingProject.projectReviewId`) ; (3) la trace **portefeuille** reste dans `GovernanceCycleInstance`. Le module ne possède en propre que ce qui n'existe nulle part : modèle de rituel, périmètre multi-projets, appel, sections, deck, points bloquants. Modèles Prisma : `MeetingTemplate`, `MeetingTemplateSection`, `Meeting`, `MeetingProject`, `MeetingAttendee`, `MeetingSectionInstance`, `MeetingAgendaItem`, `MeetingDecision`, `MeetingBlocker`, `MeetingAttachment` (+ `EmailDelivery.meetingId`). Cycle de vie `PREPARING → SCHEDULED → IN_PROGRESS → FINALIZED`, `CANCELLED` tant que non finalisée ; transition invalide → **409 `MEETING_INVALID_TRANSITION`**. Catalogue **fermé** de 16 types de section (seul `FREE_TEXT` est instanciable plusieurs fois) ; 9 modèles système livrés **par client** au seed, non supprimables et non modifiables mais **masquables** (`isHidden`) — pour les adapter, on les duplique. **Isolation** : `clientId` dérivé du client actif (jamais du body), tout projet inscrit passe par `AccessDecisionService` (intent `read`, refus explicite **403** jamais silencieux), et `MeetingProject` référence `Project` par **FK composite `(clientId, projectId)`** — la base refuse la référence inter-clients. Permissions : `meetings.read` / `create` / `update` / `conduct` / `templates.manage` ; module catalogue **`meetings`**. Migration `20260727120000_rfc_meet_001_meetings` (**purement additive**). Tests : 6 suites / 55 tests (`apps/api/src/modules/meetings/tests/` + `lib/`). **Reste à livrer** : `GET …/deck` (agrégation des sections), appel en écriture et convocations, pont `ProjectReview`, décisions et arbitrages (dont portée `MACRO_TASK`), registre des points bloquants, compte rendu et exports, **et l'intégralité du frontend**. Détail API : [docs/API.md](API.md) (section Réunions de gouvernance) ; RFC : [RFC-MEET-001](RFC/RFC-MEET-001%20%E2%80%94%20Module%20R%C3%A9unions%20de%20gouvernance%20(CODIR%2C%20COPIL%2C%20COPRO).md) · [RFC-FE-MEET-001](RFC/RFC-FE-MEET-001%20%E2%80%94%20UI%20R%C3%A9unions%20%E2%80%94%20pr%C3%A9paration%2C%20conduite%20et%20pr%C3%A9sentation.md). Code : `apps/api/src/modules/meetings/`.
...
**Module `microsoft` (RFC-PROJ-INT-003 / RFC-PROJ-INT-004 / RFC-PROJ-INT-005 / RFC-PROJ-INT-010)** : OAuth délégué Microsoft 365, jetons stockés chiffrés (`MicrosoftConnection`). **`MicrosoftModule`** importe **`AccessDecisionModule`** pour résoudre `ResourceAccessDecisionGuard` sur `ProjectMicrosoftTeamsProvisioningController` (RFC-ACL-025). **Transport Microsoft Graph** : `MicrosoftGraphService` ([RFC-PROJ-INT-004](RFC/RFC-PROJ-INT-004%20—%20Microsoft%20Graph%20Service.md)) — requêtes vers `https://graph.microsoft.com/v1.0`, Bearer via `MicrosoftOAuthService.ensureFreshAccessToken` (ex. `requestForConnection`) ; **pas d’endpoint générique « Graph »** côté API : le client appelle des routes métier qui appellent le service. **Lien projet Microsoft** ([RFC-PROJ-INT-007](RFC/RFC-PROJ-INT-007%20—%20Lien%20projet%20Microsoft.md), [RFC-PROJ-INT-008](RFC/RFC-PROJ-INT-008%20—%20Sync%20t%C3%A2ches%20vers%20Planner.md), [RFC-PROJ-INT-009](RFC/RFC-PROJ-INT-009%20—%20Sync%20documents%20vers%20Teams.md)) : `GET|PUT /api/projects/:projectId/microsoft-link` (champ `useMicrosoftPlannerBuckets` pour importer les buckets Planner à la place des buckets Starium — voir RFC-PROJ-OPT-001 ; `plannerPlanId` obligatoire si activation manuelle `isEnabled=true`), `POST .../sync-tasks`, `POST .../sync-documents` — contrôleurs dans `microsoft/`, isolation `projectId` + client actif ; la sync documents importe `ProjectsModule` pour `ProjectDocumentContentService` (fichiers STARIUM sur disque). **Provisioning Teams** ([RFC-PROJ-INT-010](RFC/RFC-PROJ-INT-010%20—%20Provisioning%20Teams%20et%20canaux%20par%20défaut.md)) : référentiel client `GET|PUT /api/projects/options/microsoft-teams-provisioning` + CRUD templates canaux ; exécution asynchrone `GET|POST /api/projects/:projectId/microsoft-teams/provision*` — worker BullMQ **`ProjectMicrosoftTeamsProvisioningProcessor`** (timeout job 8 min, `AbortController`, heartbeat poll Graph) + **`ProjectMicrosoftTeamsProvisioningStaleMaintenanceService`** (job scheduler toutes les 5 min, runs `PENDING|IN_PROGRESS` sans heartbeat > 15 min) ; constantes partagées **`project-microsoft-teams-provisioning.constants.ts`** (codes erreur, allowlist retry, `jobId` `_r{retryCount}`) ; enqueue via **`QueueService`** (backoff 30 s). Guards ACL projet ; opt-in création projet via `provisionMicrosoftTeams` ; cohabite avec le rattachement manuel INT-007. **État** : MVP **Team + canaux** + fiabilisation lot 2 livrés ; **lot 5** (cases modulaires Planner / dossier documents / sync tâches) **non livré** (sync code 2026-09-16). Routes publiques existantes : `/api/microsoft/auth/url`, `/api/microsoft/auth/callback` (sans JWT ; `clientId` issu du `state` signé validé), `/api/microsoft/connection` ; **configuration plateforme** `GET|PATCH /api/platform/microsoft-settings` (redirect URI, scopes, timeouts — `PlatformAdminGuard`) ; **identifiants d’app Azure par client Starium** `GET|PUT /api/clients/active/microsoft-oauth` (`projects.update`, client admin ou équivalent). Repli **environnement** si la DB est vide. UI client : **Administration client** → `/client/administration/microsoft-365` (accès aligné sur l’API : **client admin** ou **`projects.update`** avec module Projets activé) ; référentiel Teams : **`/projects/options`** (section Équipes Microsoft : orchestrateur + table + dialog canaux) ; UI plateforme : `/admin/microsoft-settings` (un utilisateur non–platform-admin y est **redirigé** vers `/client/administration/microsoft-365`). Code : `apps/api/src/modules/microsoft/`, `apps/web/src/features/microsoft-365/`, `apps/web/src/features/projects/options/`, `apps/web/src/app/(protected)/admin/microsoft-settings/`.
