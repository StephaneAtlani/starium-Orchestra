...
- **RBAC codes (RFC-ACL-015 — socle)** : `EffectivePermissionsService` (`apps/api/src/common/services/effective-permissions.service.ts`) agrège les `Permission.code` des rôles en un **Set brut** (cache par requête). Les guards `ModuleAccessGuard` et `PermissionsGuard` (`apps/api/src/common/guards/`), le diagnostic d’accès, la recherche et le chatbot appellent **`satisfiesPermission`** du package workspace **`@starium-orchestra/rbac-permissions`** (`packages/rbac-permissions/`) — équivalences **documentées et restrictives** avant RFC-016/018 (ex. `*.read_all` peut ouvrir un décorateur legacy `*.read` sur les modules catalogue ; `read_scope` / `read_own` **non**). **`AccessDecisionService`** ([RFC-ACL-018](RFC/RFC-ACL-018%20%E2%80%94%20Moteur%20de%20d%C3%A9cision%20d%27acc%C3%A8s%20unifi%C3%A9.md), module `access-decision`) applique en plus l’intention lecture scoped + org + matrice policy/ACL sur les chemins métier branchés (V1 : lecture/liste **Projets**). `GET /api/me/permissions` renvoie ces codes bruts + `uiPermissionHints` (affichage seul). Détail : [RFC-ACL-015](RFC/RFC-ACL-015%20%E2%80%94%20Permissions%20OWN%20SCOPE%20ALL.md), [ACCESS-MODEL.md](ACCESS-MODEL.md).
...
├── access-decision (**RFC-ACL-018** — `AccessDecisionService`, `assertAllowed`, `filterResourceIdsByAccess`, `loadAccessResources`, `evaluateReadRbacIntent`, `membership-access-gates` ; ordre licence → abonnement → module → RBAC intent → `OrganizationScopeService` → matrice 017 avec **`sharingFloorAllows = floorAllowed`** ; `ResourceAccessDecisionGuard` + `@RequireAccess` exportés ; pilote **lecture/liste Projets** via `ProjectsModule` ; intents `write`/`admin` refusés côté moteur en V1)
├── access-diagnostics (RFC-ACL-011 — diagnostic consolidé 6 couches ; admin `GET /api/access-diagnostics/effective-rights` + plateforme `GET /api/platform/clients/:clientId/access-diagnostics/effective-rights` ; **RFC-ACL-014** — self-service `GET /api/access-diagnostics/effective-rights/me` ; **RFC-ACL-019** — enrichissement **opt-in** (`ACCESS_DIAGNOSTICS_ENRICHED` = `true`|`1` strictement) : `finalDecision` **read** alignée sur `AccessDecisionService.decide`, blocs `organizationScopeCheck` / `resourceOwnershipCheck` / `resourceAccessPolicyCheck`, `evaluationMode` sur les six checks + contrôles self `ORGANIZATION_SCOPE` / `RESOURCE_OWNERSHIP` / `RESOURCE_ACCESS_POLICY` ; sans flag, contrat JSON inchangé ; audit `access_diagnostic.self_outcome` ; registre `RESOURCE_ACCESS_DIAGNOSTIC_REGISTRY` ; whitelist `resourceType` V1 = `PROJECT`/`BUDGET`/`CONTRACT`/`SUPPLIER`/`STRATEGIC_OBJECTIVE` ; refus générique `DIAGNOSTIC_SCOPE_MISMATCH` hors périmètre — anti-fuite stricte)
