...
│   │   ├── governance-cycles   (RFC-PROJ-CYCLE-001 — cycles de pilotage / arbitrage CODIR ; lots B1–B7 : CRUD cycles + items + scoring + KPI global)
│   │   └── ...
│   └── prisma / PostgreSQL
│
...
**Module `governance-cycles` (RFC-PROJ-CYCLE-001 — lots B1–B7)** : API `/api/governance-cycles` — CRUD cycles + CRUD items sous `/api/governance-cycles/:id/items` (PATCH édition vs arbitrage séparés : body mixte → **400** ; scores en groupe édition). Scoring `priorityScore` : formule RFC §4.5 ; recalcul create systématique ; recalcul update **uniquement** si le body contient une clé score (`hasScorePatch` / `hasOwnProperty`, y compris `null` pour effacement) ; `priorityScore` jamais accepté en body write. Tri liste `priorityScore desc` (`nulls: 'last'`). Deux contrats summary : **embarqué** (3 compteurs sur liste/détail cycle) et **KPI global** `GET /api/governance-cycles/:id/summary` (`GovernanceCycleGlobalSummaryDto` — compteurs par statut, totaux budget/capacité, moyenne `priorityScore`, items à haut risque ; ≤ 3 requêtes Prisma parallèles ; isolation `clientId`). Permissions `governance_cycles.read|create|update|delete|arbitrate` ; module RBAC `governance_cycles`. Audits cycle : `governance_cycle.*` ; audits item : `governance_cycle_item.created|updated|deleted|decision_changed` (snapshot inclut les 6 champs score). **Hors scope livré** : `GET …/by-project/:projectId` (RFC-PROJ-CYCLE-002). Code : `apps/api/src/modules/governance-cycles/` (`governance-cycle-scoring.util.ts`, specs DTO). Détail API : [docs/API.md](API.md) §5.8.
