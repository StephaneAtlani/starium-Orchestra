...
/**
 * Données démo conformité **client** : copie locale ISO / NIS2 + statuts + preuve.
 * Le catalogue plateforme (clientId null) est géré par `ensurePlatformComplianceCatalog`.
 */
export async function ensureDemoCompliance(prisma: PrismaClient, clientId: string): Promise<void> {
  let iso = await prisma.complianceFramework.findFirst({
    where: { clientId, name: "ISO 27001", version: "2022" },
  });
  if (!iso) {
    iso = await prisma.complianceFramework.create({
      data: {
        clientId,
        name: "ISO 27001",
        version: "2022",
    ...
    iso = await prisma.complianceFramework.update({
      where: { id: iso.id },
      data: { isActive: true },
    });
  }

  let nis2 = await prisma.complianceFramework.findFirst({
    where: { clientId, name: "NIS2", version: "directive" },
  });
  if (!nis2) {
    nis2 = await prisma.complianceFramework.create({
      data: {
        clientId,
        name: "NIS2",
        version: "directive",
    ...
    nis2 = await prisma.complianceFramework.update({
      where: { id: nis2.id },
      data: { isActive: true },
    });
  }
