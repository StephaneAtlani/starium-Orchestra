...
const IV_LENGTH = 12;
const KEY_LENGTH = 32;
const SALT = 'starium-ms-oauth-v1';

/**
 * Chiffrement AES-256-GCM des jetons Microsoft au repos.
 * Préférer `MICROSOFT_TOKEN_ENCRYPTION_KEY` (64 caractères hex = 32 octets).
 */
@Injectable()
export class MicrosoftTokenCryptoService {
  ...
  constructor(private readonly config: ConfigService) {
    const envKey = this.config.get<string>('MICROSOFT_TOKEN_ENCRYPTION_KEY')?.trim();
    if (envKey) {
