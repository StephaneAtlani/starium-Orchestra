import {
  Controller,
  Get,
  Param,
  Post,
  StreamableFile,
  UploadedFile,
  UseGuards,
  UseInterceptors,
} from '@nestjs/common';
import { JwtAuthGuard } from '../auth/guards/jwt-auth.guard';
import { ActiveClientGuard } from '../../common/guards/active-client.guard';
import { ModuleAccessGuard } from '../../common/guards/module-access.guard';
import { PermissionsGuard } from '../../common/guards/permissions.guard';
import { RequirePermissions } from '../../common/decorators/require-permissions.decorator';
import { ActiveClientId } from '../../common/decorators/active-client.decorator';
import { RequestUserId } from '../../common/decorators/request-user.decorator';
import { RequestMeta } from '../../common/decorators/request-meta.decorator';
import type { AuditContext } from '../budget-management/types/audit-context';
import { PlatformMaxFileInterceptor } from '../platform-upload/platform-max-file.interceptor';
import { StrategicDirectionStrategyDocumentsService } from './strategic-direction-strategy-documents.service';

@Controller('strategic-direction-strategies/:strategyId/documents')
@UseGuards(JwtAuthGuard, ActiveClientGuard, ModuleAccessGuard, PermissionsGuard)
export class StrategicDirectionStrategyDocumentsController {
  constructor(private readonly documents: StrategicDirectionStrategyDocumentsService) {}

  @Get()
  @RequirePermissions('strategic_direction_strategy.read')
  list(
    @ActiveClientId() clientId: string | undefined,
    @Param('strategyId') strategyId: string,
    @RequestUserId() userId: string | undefined,
  ) {
    return this.documents.list(clientId!, strategyId, userId);
  }

  @Post('upload')
  @RequirePermissions('strategic_direction_strategy.update')
  @UseInterceptors(PlatformMaxFileInterceptor)
  upload(
    @ActiveClientId() clientId: string | undefined,
    @Param('strategyId') strategyId: string,
    @UploadedFile() file: Express.Multer.File,
    @RequestUserId() actorUserId: string | undefined,
    @RequestMeta() meta: { ipAddress?: string; userAgent?: string; requestId?: string },
  ) {
    const context: AuditContext = { actorUserId, meta };
    return this.documents.upload(clientId!, strategyId, file, context);
  }

  @Get(':documentId/download')
  @RequirePermissions('strategic_direction_strategy.read')
  async download(
    @ActiveClientId() clientId: string | undefined,
    @Param('strategyId') strategyId: string,
    @Param('documentId') documentId: string,
    @RequestUserId() userId: string | undefined,
  ) {
    const { stream, contentType, filename } = await this.documents.getDownloadStream(
      clientId!,
      strategyId,
      documentId,
      userId,
    );
    const safe = filename.replace(/["\r\n]/g, '_').slice(0, 200);
    return new StreamableFile(stream, {
      type: contentType,
      disposition: `inline; filename="${safe}"`,
    });
  }
}
